...
int KeyMapper::getKeysym(const std::string& name)
	...
	if(result==NoSymbol)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown key name \"%s\"",name.c_str());
...
std::string KeyMapper::getName(int keysym)
	...
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown key symbol %d",keysym);
...
KeyMapper::QualifiedKey KeyMapper::getQualifiedKey(const std::string& name)
	...
	while(true)
		...
		if(pref-nPtr==5&&strncasecmp(nPtr,"Shift",5)==0)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown modifier key name %s",std::string(nPtr,pref).c_str());
