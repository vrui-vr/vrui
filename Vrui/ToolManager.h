...
class ToolManager:public Plugins::FactoryManager<ToolFactory>
	...
	public:
	class ToolListEntry // Class to store tools in a list
		{
		...
		bool essential; // Flag if the tool is essential, i.e., should not be destroyed interactively
		
		/* Constructors and destructors: */
		public:
		ToolListEntry(Tool* sTool,bool sEssential =false)
			:tool(sTool),essential(sEssential)
			{
			}
		
		/* Methods: */
		operator const Tool*(void) const // Returns the pointer to the list entry's tool
			{
			return tool;
			}
		operator Tool*(void) // Ditto
			{
			return tool;
			}
		};
	
	typedef std::vector<ToolListEntry> ToolList; // Data type for list of tools
	...
	void finishToolCreation(bool confirmed); // Called when user explicitly confirms or cancels tool creation; flag tells whether creation was confirmed or canceled
	...
	bool isToolEssential(Tool* tool) const; // Returns true if the given tool is essential, i.e., should not be deleted interactively
	};
