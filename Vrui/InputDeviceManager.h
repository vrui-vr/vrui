...
class InputDeviceManager
	...
	typedef Misc::HashTable<InputDevice*,ONTransform> HandleTransformMap; // Hash table mapping input devices to their handle transformations
	...
	HandleTransformMap handleTransformMap; // Map from input devices to their handle transformations
	bool predictDeviceStates; // Flag to enable device state prediction for latency mitigation
	TimePoint predictionTime; // Time point to which to predict the state of all input devices during update
	...
	void addHandleTransform(InputDevice* device,const ONTransform& handleTransform); // Sets a handle transformation for the given input device
	...
	void setPredictionTime(const TimePoint& newPredictionTime); // Enables device state prediction and sets the prediction time point for the current frame
	void setPredictionTimeNow(void); // Sets the current device state prediction time to the current time by sampling the real-time clock; leaves prediction disabled
	const TimePoint& getPredictionTime(void) const // Returns the current device state prediction time point
		{
	...
	const ONTransform& getHandleTransform(InputDevice* device) const // Returns the handle transformation for the given input device; returns identity transformation if none is defined
		{
		/* Find the input device in the handle transform map: */
		HandleTransformMap::ConstIterator htmIt=handleTransformMap.findEntry(device);
		if(!htmIt.isFinished())
			return htmIt->getDest();
		else
			return ONTransform::identity;
		}
	};
