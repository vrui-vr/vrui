...
struct ToolManagerToolCreationState
	...
	GLMotif::Button* okButton; // Pointer to the confirmation button
	
	/* Private methods: */
	void okButtonSelectCallback(Misc::CallbackData* cbData);
	void cancelButtonSelectCallback(Misc::CallbackData* cbData);
...
void ToolManagerToolCreationState::okButtonSelectCallback(Misc::CallbackData* cbData)
	{
	/* Notify the tool manager: */
	getToolManager()->finishToolCreation(true);
	}

void ToolManagerToolCreationState::cancelButtonSelectCallback(Misc::CallbackData* cbData)
	{
	/* Notify the tool manager: */
	getToolManager()->finishToolCreation(false);
	}
...
void ToolManagerToolCreationState::createProgressDialog(void)
	...
	confirmButtonsMargin->setAlignment(GLMotif::Alignment::RIGHT);
	
	GLMotif::RowColumn* confirmButtonsBox=new GLMotif::RowColumn("ConfirmButtonsBox",confirmButtonsMargin,false);
	confirmButtonsBox->setOrientation(GLMotif::RowColumn::HORIZONTAL);
	confirmButtonsBox->setPacking(GLMotif::RowColumn::PACK_GRID);
	confirmButtonsBox->setNumMinorWidgets(1);
	
	okButton=new GLMotif::Button("OkButton",confirmButtonsBox,"OK");
	okButton->setEnabled(!(requireButtons||requireValuators));
	okButton->getSelectCallbacks().add(this,&ToolManagerToolCreationState::okButtonSelectCallback);
	
	GLMotif::Button* cancelButton=new GLMotif::Button("CancelButton",confirmButtonsBox,"Cancel");
	cancelButton->getSelectCallbacks().add(this,&ToolManagerToolCreationState::cancelButtonSelectCallback);
	
	confirmButtonsBox->manageChild();
	
	confirmButtonsMargin->manageChild();
...
ToolManagerToolCreationState::ToolManagerToolCreationState(const InputDeviceManager& inputDeviceManager,const InputDeviceFeature& sFirstFeature)
	:firstFeature(sFirstFeature),
	 ...
	 progressDialog(0),progressBox(0),buttonBox(0),valuatorBox(0),cancelLine(0),okButton(0)
	{
	}
...
GLMotif::PopupMenu* ToolManager::createToolSubmenu(const Plugins::Factory& factory)
	...
	for(Plugins::Factory::ClassList::const_iterator chIt=factory.childrenBegin();chIt!=factory.childrenEnd();++chIt)
		...
		if(factory==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Factory class %s is not a Vrui tool factory class",(*chIt)->getClassName());
...
void ToolManager::addClassToMenu(ToolFactory* newFactory)
	...
	for(std::vector<const ToolFactory*>::reverse_iterator aIt=ancestors.rbegin();aIt!=ancestors.rend();++aIt)
		...
		if(ancestorCascade!=0)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Base class name \"%s\" already exists as concrete class",(*aIt)->getClassName());
		}
...
void ToolManager::removeClassFromMenu(ToolFactory* factory)
	...
	if(menu!=0)
		...
		if(factoryButton!=0)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Removed class's button not found");
		}
	else
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Removed class's sub-menu not found");
	}
...
ToolManager::ToolManager(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& sConfigFileSection)
	...
	StringList toolSearchPaths;
	configFileSection->updateValue("./toolSearchPaths",toolSearchPaths);
	...
	StringList toolClassNames;
	configFileSection->updateValue("./toolClassNames",toolClassNames);
	...
	if(!toolSelectionMenuFactory->isDerivedFrom("MenuTool"))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Tool selection menu tool class is not a menu tool class");
	...
	if(menuToolLayout.getNumButtons()!=1||menuToolLayout.getNumValuators()!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Tool selection menu tool class has wrong input layout");
	...
	if(killZoneType=="Box")
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown kill zone type \"%s\"",killZoneType.c_str());
	}
...
ToolManager::~ToolManager(void)
	...
	for(ToolList::iterator tIt=tools.begin();tIt!=tools.end();++tIt)
		...
		#endif
		tIt->tool->getFactory()->destroyTool(*tIt);
...
void ToolManager::releaseClass(ToolFactory* factory)
	...
	for(ToolList::iterator tIt=tools.begin();tIt!=tools.end();++tIt)
		if(tIt->tool->getFactory()==factory)
...
void ToolManager::loadToolBinding(const char* toolSectionName)
	...
	ToolAssignmentList bindings;
	toolSection.updateValue("./bindings",bindings);
	...
	for(ToolAssignmentList::iterator bIt=bindings.begin();bIt!=bindings.end();++bIt)
		for(std::vector<int>::iterator fiIt=bIt->featureIndices.begin();fiIt!=bIt->featureIndices.end();++fiIt)
			...
			if(openFeature.isValid())
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Feature %s on input device %s is already assigned",featureName.c_str(),bIt->device->getDeviceName());
				}
			}
	
	/* Check if there are enough assignments: */
	if(buttonSlotIndex<layout.getNumButtons())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Not enough button bindings; got %d, need %d",buttonSlotIndex,layout.getNumButtons());
	if(valuatorSlotIndex<layout.getNumValuators())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Not enough valuator bindings; got %d, need %d",valuatorSlotIndex,layout.getNumValuators());
...
void ToolManager::finishToolCreation(bool confirmed)
	{
	/* Bail out if there is no tool creation state: */
	if(toolCreationState==0)
		return;
	
	/* Check if tool creation was confirmed and successful: */
	if(confirmed&&toolCreationState->isComplete())
		{
	...
	/* Remove the motion callback from an existing tool selection device: */
	if(toolCreationState->toolSelectionDevice!=0)
		{
		toolCreationState->toolSelectionDevice->getTrackingCallbacks().remove(this,&ToolManager::toolCreationDeviceMotionCallback);
		toolCreationState->toolSelectionDevice=0;
...
Tool* ToolManager::createTool(ToolFactory* factory,const ToolInputAssignment& tia,const Misc::ConfigurationFileSection* cfg)
	...
	// try
		...
		/* Check if the tool is essential: */
		bool essential=false;
		if(cfg!=0)
			cfg->updateValue("./essential",essential);
		
		/* Add tool to list: */
		tools.push_back(ToolListEntry(newTool,essential));
		++state;
...
bool ToolManager::isToolEssential(Tool* tool) const
	{
	/* Find the tool in the list: */
	for(ToolList::const_iterator tIt=tools.begin();tIt!=tools.end();++tIt)
		if(tIt->tool==tool)
			return tIt->essential;
	return false;
	}
