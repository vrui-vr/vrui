...
/******************************************
Declaration of Vrui kernel interface types:
******************************************/
...
/* Manage intra-cluster communication: */
...
bool isHeadNode(void); // Returns true if the cluster node on which the caller is running is the head node
int getNodeIndex(void); // Returns the index of the cluster node on which the caller is running; head node has index 0
int getNumNodes(void); // Returns the number of cluster nodes, including the head node
...
Cluster::MulticastPipe* openPipe(void); // Opens a pipe for 1-to-n communication from the head node to all other cluster nodes (returns 0 if called in a non-cluster environment)
...
/* Manage the central scene graph: */
SceneGraphManager* getSceneGraphManager(void); // Returns the scene graph manager
...
/* Shortcuts to elements and methods of EnvironmentDefinition: */
...
Point calcFloorPoint(const Point& position); // Returns a point on the environment's "floor" directly under the given physical-space position
...
void setNavigationTransformation(const Point& center,Scalar radius,const Vector& up,const Vector& right); // Sets the navigation transformation such that a model inside the given sphere appears in the middle of the environment, and the given directions point "up" and "right," respectively
void setNavigationTransformation(const Box& box,const Vector& up,const Vector& right); // Sets the navigation transformation such that a model inside the given box appears in the middle of the environment, and the given directions point "up" and "right," respectively
