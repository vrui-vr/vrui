...
void Glyph::setGlyphType(const char* newGlyphType)
	...
	if(strcasecmp(newGlyphType,"None")!=0)
		...
		if(strcasecmp(newGlyphType,"Cone")==0)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid glyph type %s",newGlyphType);
		enabled=true;
		}
	else
		{
...
void Glyph::configure(const Misc::ConfigurationFileSection& configFileSection,const char* glyphTypeTagName,const char* glyphMaterialTagName)
	...
	configFileSection.updateValue(glyphMaterialTagName,glyphMaterial);
	}
...
GlyphRenderer::GlyphRenderer(GLfloat sGlyphSize,const std::string& cursorImageFileName,unsigned int sCursorNominalSize)
	:GLObject(false),
	 glyphSize(sGlyphSize),
	 cursorNominalSize(sCursorNominalSize)
	{
	/* Load the cursor image file: */
	cursorImage=Images::readCursorFile(cursorImageFileName.c_str(),cursorNominalSize,&cursorHotspot);
...
void GlyphRenderer::initContext(GLContextData& contextData) const
	...
	for(int glyphType=Glyph::CONE;glyphType<Glyph::GLYPHS_END;++glyphType)
		...
		if(glyphType==Glyph::CURSOR)
			...
			glVertex2f(-float(cursorHotspot[0])*scale,-float(cis[1]-1-cursorHotspot[1])*scale);
			glTexCoord2f(tcMax[0],tcMin[1]);
			glVertex2f(float(cis[0]-1-cursorHotspot[0])*scale,-float(cis[1]-1-cursorHotspot[1])*scale);
			glTexCoord2f(tcMax[0],tcMax[1]);
			glVertex2f(float(cis[0]-1-cursorHotspot[0])*scale,float(cursorHotspot[1])*scale);
			glTexCoord2f(tcMin[0],tcMax[1]);
			glVertex2f(-float(cursorHotspot[0])*scale,float(cursorHotspot[1])*scale);
			glEnd();
			glBindTexture(GL_TEXTURE_2D,0);
			glPopAttrib();
			glEndList();
