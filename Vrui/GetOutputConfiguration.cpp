...
OutputConfiguration getOutputConfiguration(Display* display,int screen,const char* outputName)
	...
	result.domain=IRect(ISize(DisplayWidth(display,screen),DisplayHeight(display,screen)));
					...
					if(crtcInfo!=0)
						...
						for(int outputIndex=0;outputIndex<crtcInfo->noutput&&(!haveMatch||vruiVerbose);++outputIndex)
							...
							if(firstOutput||matchesName)
								...
								result.domain=IRect(IOffset(crtcInfo->x,crtcInfo->y),ISize(crtcInfo->width,crtcInfo->height));
	...
	if(result.sizeMm[0]==0||result.sizeMm[1]==0)
		...
		for(int i=0;i<2;++i)
			result.sizeMm[i]=(254*result.domain.size[i]+dpi*5)/(dpi*10);
		}
