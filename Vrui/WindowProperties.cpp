...
WindowProperties::WindowProperties(void)
	:nonlinear(false),depthBufferSize(16),numAuxBuffers(0),stencilBufferSize(0)
	{
	for(int i=0;i<3;++i)
		colorBufferSize[i]=8;
	colorBufferSize[3]=0;
...
void WindowProperties::setNonlinear(bool newNonlinear)
	{
	nonlinear=newNonlinear;
	}
...
void WindowProperties::merge(const WindowProperties& other)
	{
	/* Enable non-linear compressed colors if anybody requested them: */
...
void WindowProperties::setContextProperties(GLContext::Properties& contextProperties) const
	{
	/* Copy over the property values: */
	for(int i=0;i<4;++i)
		contextProperties.colorBufferSize[i]=colorBufferSize[i];
	contextProperties.nonlinear=nonlinear;
	contextProperties.depthBufferSize=depthBufferSize;
	contextProperties.stencilBufferSize=stencilBufferSize;
	contextProperties.numAuxBuffers=numAuxBuffers;
	for(int i=0;i<4;++i)
		contextProperties.accumBufferSize[i]=accumBufferSize[i];
	}
