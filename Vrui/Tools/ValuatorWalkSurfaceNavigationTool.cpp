...
ValuatorWalkSurfaceNavigationToolFactory::Configuration::Configuration(void)
	:activationToggle(true),
	 ...
	 valuatorMoveFollowsDevice(false),valuatorViewFollowFactor(1),
...
void ValuatorWalkSurfaceNavigationToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	{
	cfs.updateValue("./activationToggle",activationToggle);
	...
	Geometry::Vector<Scalar,2> vms=cfs.retrieveValue("./valuatorMoveSpeed",Geometry::Vector<Scalar,2>(valuatorMoveSpeeds));
	for(int i=0;i<2;++i)
		valuatorMoveSpeeds[i]=vms[i];
	cfs.updateValue("./valuatorMoveFollowsDevice",valuatorMoveFollowsDevice);
	cfs.updateValue("./valuatorViewFollowFactor",valuatorViewFollowFactor);
	...
	centerViewDirection.orthogonalize(getUpDirection());
	...
	cfs.updateValue("./valuatorSnapRotate",valuatorSnapRotate);
	valuatorRotateSpeed=Math::rad(cfs.retrieveValue("./valuatorRotateSpeed",Math::deg(valuatorRotateSpeed)));
...
void ValuatorWalkSurfaceNavigationToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	{
	cfs.storeValue("./activationToggle",activationToggle);
	...
	cfs.storeValue("./valuatorMoveSpeed",Geometry::Vector<Scalar,2>(valuatorMoveSpeeds));
	cfs.storeValue("./valuatorMoveFollowsDevice",valuatorMoveFollowsDevice);
	cfs.storeValue("./valuatorViewFollowFactor",valuatorViewFollowFactor);
	...
	cfs.storeValue("./valuatorSnapRotate",valuatorSnapRotate);
	cfs.storeValue("./valuatorRotateSpeed",Math::deg(valuatorRotateSpeed));
	cfs.storeValue("./fallAcceleration",fallAcceleration);
	cfs.storeValue("./jetpackAcceleration",jetpackAcceleration);
...
void ValuatorWalkSurfaceNavigationTool::frame(void)
	...
	if(isActive())
		...
		viewDir.orthogonalize(getUpDirection());
		...
		Vector valuatorMoveDir;
		if(configuration.valuatorMoveFollowsDevice)
			{
			/* Follow the valuator device's pointing direction: */
			valuatorMoveDir=getValuatorDeviceRayDirection(1);
			valuatorMoveDir.orthogonalize(getUpDirection());
			}
		else
			{
			/* Follow the central viewing direction: */
			valuatorMoveDir=configuration.centerViewDirection;
			}
		
		/* Blend the base moving direction and the view direction: */
		valuatorMoveDir=valuatorMoveDir*(Scalar(1)-configuration.valuatorViewFollowFactor)+viewDir*configuration.valuatorViewFollowFactor;
		Scalar vmdMag=valuatorMoveDir.mag();
		if(vmdMag!=Scalar(0))
			{
			Vector valuatorStrafeDir=valuatorMoveDir^getUpDirection();
			Scalar vsdMag=valuatorStrafeDir.mag();
			...
			moveDir+=valuatorStrafeDir*(vmx/vsdMag)+valuatorMoveDir*(vmy/vmdMag);
		...
		if(jetpack!=Scalar(0))
			{
			moveDir+=getValuatorDeviceRayDirection(3)*jetpack;
			newMoving=true;
			}
...
InputDeviceFeatureSet ValuatorWalkSurfaceNavigationTool::getSourceFeatures(const InputDeviceFeature& forwardedFeature)
	...
	if(i==input.getNumValuatorSlots())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Forwarded feature not found");
...
InputDevice* ValuatorWalkSurfaceNavigationTool::getSourceDevice(const InputDevice* forwardedDevice)
	...
	if(i==numValuatorDevices)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Forwarded device not found");
...
InputDeviceFeatureSet ValuatorWalkSurfaceNavigationTool::getForwardedFeatures(const InputDeviceFeature& sourceFeature)
	...
	if(slotIndex<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Source feature not found");
