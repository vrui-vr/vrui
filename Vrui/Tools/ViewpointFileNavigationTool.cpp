...
void ViewpointFileNavigationToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	...
	cfs.updateValue("./showGui",showGui);
	cfs.updateValue("./showKeyframes",showKeyframes);
	cfs.updateString("./pauseFileName",pauseFileName);
	cfs.updateValue("./autostart",autostart);
	}
...
void ViewpointFileNavigationToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.retrieveValue("./showGui",showGui);
	cfs.retrieveValue("./showKeyframes",showKeyframes);
	cfs.storeString("./pauseFileName",pauseFileName);
	cfs.retrieveValue("./autostart",autostart);
	}
	...
	try
		...
		if(Misc::hasCaseExtension(fileName,".view"))
			...
			if(strcmp(header,vruiViewpointFileHeader)==0)
				...
				ControlPoint v;
				viewpointFile->read(v.center.getComponents(),3);
				v.size=Math::log(viewpointFile->read<Scalar>()); // Sizes are interpolated logarithmically
				viewpointFile->read(v.forward.getComponents(),3);
				viewpointFile->read(v.up.getComponents(),3);
