...
void ScreenMouseToolFactory::Configuration::load(const Misc::ConfigurationFileSection& cfs)
	{
	/* Get parameters: */
	cfs.updateString("./screenName",screenName);
	cfs.updateValue("./deadZone",deadZone);
	cfs.updateValue("./exponent",exponent);
	cfs.updateValue("./velocityFactors",velocityFactors);
	}
...
void ScreenMouseToolFactory::Configuration::save(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.storeValue("./deadZone",deadZone);
	cfs.storeValue("./exponent",exponent);
	cfs.storeValue("./velocityFactors",velocityFactors);
	}
...
void ScreenMouseTool::initialize(void)
	...
	if(screen==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Screen %s not found",config.screenName.c_str());
