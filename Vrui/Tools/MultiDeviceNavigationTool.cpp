...
void MultiDeviceNavigationToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	{
	cfs.updateValue("./translationFactor",translationFactor);
	cfs.updateValue("./minRotationScalingDistance",minRotationScalingDistance);
	cfs.updateValue("./rotationFactor",rotationFactor);
	cfs.updateValue("./scalingFactor",scalingFactor);
	cfs.updateValue("./mutualExclusion",mutualExclusion);
	}

void MultiDeviceNavigationToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	{
	cfs.storeValue("./translationFactor",translationFactor);
	cfs.storeValue("./minRotationScalingDistance",minRotationScalingDistance);
	cfs.storeValue("./rotationFactor",rotationFactor);
	cfs.storeValue("./scalingFactor",scalingFactor);
	cfs.storeValue("./mutualExclusion",mutualExclusion);
	}
...
void MultiDeviceNavigationTool::frame(void)
	...
	if(isActive())
		...
		if(numActiveDevices>0)
			...
			nav.leftMultiply(t);
			nav.renormalize();
			setNavigationTransformation(nav,centroid);
			}
