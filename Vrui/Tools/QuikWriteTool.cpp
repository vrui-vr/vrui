...
void QuikWriteToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	...
	cfs.updateValue("./squareSize",squareSize);
	cfs.updateValue("./initialSquareDist",initialSquareDist);
	cfs.updateValue("./backgroundColor",backgroundColor);
	cfs.updateValue("./foregroundColor",foregroundColor);
	cfs.updateValue("./drawPoint",drawPoint);
	cfs.updateValue("./pointColor",pointColor);
	cfs.updateValue("./pointSize",pointSize);
	}
...
void QuikWriteToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.storeValue("./squareSize",squareSize);
	cfs.storeValue("./initialSquareDist",initialSquareDist);
	cfs.storeValue("./backgroundColor",backgroundColor);
	cfs.storeValue("./foregroundColor",foregroundColor);
	cfs.storeValue("./drawPoint",drawPoint);
	cfs.storeValue("./pointColor",pointColor);
	cfs.storeValue("./pointSize",pointSize);
	}
...
void QuikWriteTool::initialize(void)
	...
	if(config.useDevice)
		...
		if(interactionDevice==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Interaction device %s not found",config.deviceName.c_str());
		}
	else
