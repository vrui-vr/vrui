...
MeasurementToolFactory::MeasurementToolFactory(ToolManager& toolManager)
	...
	cfs.updateValue("./defaultMeasurementMode",defaultMeasurementMode);
	cfs.updateValue("./defaultCoordinateMode",defaultCoordinateMode);
	cfs.updateValue("./markerSize",markerSize);
	cfs.updateValue("./saveMeasurements",saveMeasurements);
	cfs.updateString("./measurementFileName",measurementFileName);
...
void MeasurementTool::configure(const Misc::ConfigurationFileSection& configFileSection)
	...
	configFileSection.updateValue("./measurementMode",measurementMode);
	switch(measurementMode)
		{
		case MeasurementToolFactory::POSITION:
			numMeasurementPoints=1;
	...
	configFileSection.updateValue("./coordinateMode",coordinateMode);
	if(coordinateMode==MeasurementToolFactory::USER&&userTransform==0)
		coordinateMode=MeasurementToolFactory::NAVIGATIONAL;
	...
	configFileSection.updateValue("./points",savedPoints);
...
void MeasurementTool::storeState(Misc::ConfigurationFileSection& configFileSection) const
	...
	configFileSection.storeValue("./measurementMode",measurementMode);
	configFileSection.storeValue("./coordinateMode",coordinateMode);
	...
	if(numPoints>0)
		...
		configFileSection.storeValue("./points",savedPoints);
		}
	}
...
void MeasurementTool::buttonCallback(int,InputDevice::ButtonCallbackData* cbData)
	...
	if(cbData->newButtonState) // Button has just been pressed
		...
		if(factory->saveMeasurements&&numPoints==numMeasurementPoints&&isHeadNode())
			{
