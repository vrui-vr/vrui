...
void TouchpadButtonsToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	...
	cfs.updateValue("./centerRadius",centerRadius);
	cfs.updateValue("./useCenterButton",useCenterButton);
	cfs.updateValue("./drawOnTouch",drawOnTouch);
	if(drawOnTouch)
		{
		cfs.updateValue("./touchpadTransform",touchpadTransform);
		cfs.updateValue("./touchpadRadius",touchpadRadius);
		cfs.updateValue("./touchpadColor",touchpadColor);
		}
	}
...
void TouchpadButtonsToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.storeValue("./centerRadius",centerRadius);
	cfs.storeValue("./useCenterButton",useCenterButton);
	cfs.storeValue("./drawOnTouch",drawOnTouch);
	if(drawOnTouch)
		{
		cfs.storeValue("./touchpadTransform",touchpadTransform);
		cfs.storeValue("./touchpadRadius",touchpadRadius);
		cfs.storeValue("./touchpadColor",touchpadColor);
		}
	}
...
int TouchpadButtonsTool::calcButtonIndex(void) const
	...
	Scalar x(getValuatorState(0));
	Scalar y(getValuatorState(1));
	Scalar r2=x*x+y*y;
	if(r2>=Math::sqr(configuration.centerRadius))
		{
		/* Calculate the angle of the touchpad contact: */
		Scalar angle=Math::atan2(-x,y);
		if(angle<Scalar(0))
			angle+=Scalar(2)*Math::Constants<Scalar>::pi;
		Scalar anglePerButton=Scalar(2)*Math::Constants<Scalar>::pi/double(configuration.numButtons);
		
		/* Find the touched perimeter button: */
		result=int(Math::floor(angle/anglePerButton+Scalar(0.5)))%configuration.numButtons;
		}
