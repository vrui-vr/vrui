...
void WiggleScreenshotToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	...
	cfs.updateValue("./numFrames",numFrames);
	angleIncrement=Math::rad(cfs.retrieveValue("./angleIncrement",Math::deg(angleIncrement)));
	}
...
void WiggleScreenshotToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.storeValue("./numFrames",numFrames);
	cfs.storeValue("./angleIncrement",Math::deg(angleIncrement));
	}
