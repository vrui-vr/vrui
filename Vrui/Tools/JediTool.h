...
class JediToolFactory:public ToolFactory
	...
	std::string lightsaberImageFileName; // Name of image file containing light saber texture; relative to Vrui's Texture directory
...
class JediTool:public PointingTool,public GLObject,public TransparentObject,public ALObject
	{
	friend class JediToolFactory;
	
	/* Embedded classes: */
	private:
	struct GLDataItem:public GLObject::DataItem
		{
		/* Elements: */
		public:
	...
	struct ALDataItem:public ALObject::DataItem
		...
		ALuint sources[numSources]; // Sound sources mixed together to create a light saber sound
		ALfloat gains[numSources]; // Independent gain factors for the sound sources
		ALuint buffers[numSources]; // Buffers containing component light saber sounds
		bool playing; // Flag if the sound sources are currently playing
	...
	ONTransform hiltTransform; // Transformation from input device's coordinate frame to light saber hilt
	...
	ONTransform transform; // Light saber transformation on current frame
