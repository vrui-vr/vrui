...
SketchingToolFactory::SketchingToolFactory(ToolManager& toolManager)
	...
	cfs.updateValue("./detailSize",detailSize);
	cfs.updateValue("./brushAxis",brushAxis);
	cfs.updateString("./curvesFileName",curvesFileName);
...
void SketchingTool::configure(const Misc::ConfigurationFileSection& configFileSection)
	...
	configFileSection.updateValue("./sketchMode",sketchMode);
	
	/* Read current line width and color: */
	configFileSection.updateValue("./lineWidth",newLineWidth);
	configFileSection.updateValue("./color",newColor);
...
void SketchingTool::storeState(Misc::ConfigurationFileSection& configFileSection) const
	...
	configFileSection.storeValue("./sketchMode",sketchMode);
	
	/* Write current line width and color: */
	configFileSection.storeValue("./lineWidth",newLineWidth);
	configFileSection.storeValue("./color",newColor);
	...
	try
		...
		for(unsigned int sketchObjectIndex=0;sketchObjectIndex<numSketchObjects;++sketchObjectIndex)
			...
			if(sot=="Curve")
				...
				throw Misc::makeStdErr(0,"Unrecognized sketch object type %s",sot.c_str());
