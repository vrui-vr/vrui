...
ScreenshotToolFactory::ScreenshotToolFactory(ToolManager& toolManager)
	...
	cfs.updateString("./screenshotFileName",screenshotFileName);
	cfs.updateValue("./windowIndex",windowIndex);
	cfs.updateValue("./screenSize",screenSize);
	cfs.updateValue("./horizontal",horizontal);
	horizontal.normalize();
	cfs.updateValue("./vertical",vertical);
	vertical.normalize();
	cfs.updateValue("./useMainViewer",useMainViewer);
	cfs.updateValue("./monoEyePosition",monoEyePosition);
	cfs.updateValue("./eyeOffset",eyeOffset);
...
void ScreenshotTool::initialize(void)
	...
	if(isMaster())
		...
		if(getMainPipe()!=0)
			{
			/* Send the screen center, screen rectangle, and eye position to the cluster: */
		...
		{
		/* Receive the eye position and screen rectangle from the head node: */
