...
void MouseCameraToolFactory::Configuration::read(const Misc::ConfigurationFileSection& cfs)
	...
	cfs.updateValue("./windowIndex",windowIndex);
	...
	cfs.updateValue("./keepScreenLevel",keepScreenLevel);
	cfs.updateValue("./fovScale",fovScale);
	cfs.updateValue("./showScreenCenter",showScreenCenter);
	cfs.updateValue("./showFrustum",showFrustum);
	}
...
void MouseCameraToolFactory::Configuration::write(Misc::ConfigurationFileSection& cfs) const
	...
	cfs.storeValue("./windowIndex",windowIndex);
	...
	cfs.storeValue("./keepScreenLevel",keepScreenLevel);
	cfs.storeValue("./fovScale",fovScale);
	cfs.storeValue("./showScreenCenter",showScreenCenter);
	cfs.storeValue("./showFrustum",showFrustum);
	}
...
void MouseCameraTool::applyCameraState(void)
	...
	if(lockToMainViewer)
		...
		Vector headX=configuration.keepScreenLevel?(headY^getUpDirection()):(headY^getMainViewer()->getUpDirection());
		Rotation headRot=Rotation::fromBaseVectors(headX,headY)*Rotation::rotateX(Math::rad(Scalar(90)));
		ONTransform headTransform(headPos-Point::origin,headRot);
		
		/* Position the screen: */
		screen->setSize(screenSize[0],screenSize[1]);
		ONTransform screenT=headTransform;
		screenT*=ONTransform::translate(Vector(-Math::div2(screenSize[0]),-Math::div2(screenSize[1]),-eyeScreenDist/configuration.fovScale));
		screenT.renormalize();
		screen->setTransform(screenT);
		
		/* Position the viewer: */
		ONTransform viewerT=headTransform;
		viewer->detachFromDevice(viewerT); // This looks a tad weird, but is perfectly cromulent
		viewer->setEyes(Vector(0,0,-1),Point::origin,viewerEyeOffset);
		}
	else
		{
...
void MouseCameraTool::initialize(void)
	...
	if(window==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid window index %d",configuration.windowIndex);
	
	/* Check that the window only uses a single screen and a single viewer: */
	if(window->getVRScreen(0)!=window->getVRScreen(1))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Window %d has two different screens attached",configuration.windowIndex);
	if(window->getViewer(0)!=window->getViewer(1))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Window %d has two different viewers attached",configuration.windowIndex);
	...
	/* Calculate the viewer's eye's distance from the screen: */
	eyeScreenDist=physScreenTransform.inverseTransform(physViewerTransform.transform(viewerEyePos))[2];
	...
	switch(buttonSlotIndex)
		...
		case 3: // Attach to main viewer
			if(cbData->newButtonState)
				...
				lockToMainViewer=!lockToMainViewer;
				
				if(!lockToMainViewer)
					{
					/* Reset the viewer and screen to their previous states: */
					applyCameraState();
					}
				}
			break;
...
void MouseCameraTool::frame(void)
	{
	/* Check if the camera is locked to the main viewer: */
	if(lockToMainViewer)
		{
		/* Update the camera state and bail out: */
		applyCameraState();
		return;
		}
...
void MouseCameraTool::display(GLContextData& contextData) const
	...
	bool drawFrustum=showFrustum&&!lockToMainViewer&&windowIndex!=configuration.windowIndex;
