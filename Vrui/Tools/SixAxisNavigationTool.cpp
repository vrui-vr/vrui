...
void SixAxisNavigationToolFactory::Configuration::load(const Misc::ConfigurationFileSection& cfs)
	{
	/* Get parameters: */
	cfs.updateValue("./translateFactor",translateFactor);
	cfs.updateValue("./translationVectors",translations);
	cfs.updateValue("./rotateFactor",rotateFactor);
	cfs.updateValue("./scaledRotationAxes",rotations);
	cfs.updateValue("./zoomFactor",zoomFactor);
	if(cfs.hasTag("./navigationCenter"))
		cfs.updateValue("./navigationCenter",navigationCenter);
	else
		followDisplayCenter=true;
	cfs.updateValue("./invertNavigation",invertNavigation);
	cfs.updateValue("./showNavigationCenter",showNavigationCenter);
	}

void SixAxisNavigationToolFactory::Configuration::save(Misc::ConfigurationFileSection& cfs) const
	{
	/* Save parameters: */
	cfs.storeValue("./translateFactor",translateFactor);
	cfs.storeValue("./translationVectors",translations);
	cfs.storeValue("./rotateFactor",rotateFactor);
	cfs.storeValue("./scaledRotationAxes",rotations);
	cfs.storeValue("./zoomFactor",zoomFactor);
	if(!followDisplayCenter)
		cfs.storeValue("./navigationCenter",navigationCenter);
	cfs.storeValue("./invertNavigation",invertNavigation);
	cfs.storeValue("./showNavigationCenter",showNavigationCenter);
	}
