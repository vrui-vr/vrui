...
bool GUIInteractor::startDragging(GLMotif::Event& event,const NavTrackerState& initialTransform)
	{
	GLMotif::Draggable* draggable=dynamic_cast<GLMotif::Draggable*>(event.getTargetWidget());
	if(draggedWidget==0&&draggable!=0&&draggable->canDrag(event))
		{
		/* Drag the entire top-level widget: */
		draggedWidget=event.getTargetWidget();
		
		/* Calculate the dragging transformation: */
		draggingTransform=initialTransform;
...
void GUIInteractor::drag(const NavTrackerState& transform,bool alignWidgets)
	{
	if(draggedWidget!=0)
		{
		/* Calculate the new dragging transformation: */
		NavTrackerState newTransform;
		if(alignWidgets)
			{
			newTransform=getUiManager()->calcUITransform(transform.getOrigin());
			newTransform*=NavTrackerState::rotate(Geometry::invert(draggingTransform.getRotation()));
			}
		else
		...
		/* Set the dragged widget's widget transformation: */
		getWidgetManager()->setPrimaryWidgetTransformation(draggedWidget,newTransform);
		}
	}

void GUIInteractor::stopDragging(void)
	{
	/* Stop dragging: */
	draggedWidget=0;
	}
...
bool GUIInteractor::buttonDown(bool force)
	...
	if(getUiManager()->activateGuiInteractor(this))
		...
		if(getWidgetManager()->pointerButtonDown(event)||force)
			{
			/* Try dragging the selected widget: */
			startDragging(event,calcInteractionTransform());
			}
		else
			{
...
void GUIInteractor::move(void)
	...
	if(interacting||getUiManager()->canActivateGuiInteractor(this))
		...
		if(interacting&&draggedWidget!=0)
			{
			/* Drag the dragged widget: */
			drag(calcInteractionTransform());
			}
...
void GUIInteractor::glRenderAction(GLfloat rayWidth,const Color& rayColor,GLContextData& contextData) const
	{
