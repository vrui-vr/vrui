...
void Viewer::setHeadDeviceCallback(const char* argumentsBegin,const char* argumentsEnd,void* userData)
	...
	if(newHeadDeviceName.empty())
		...
		if(newHeadDevice==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Head device \"%s\" not found",newHeadDeviceName.c_str());
...
void Viewer::setHeadTransformCallback(const char* argumentsBegin,const char* argumentsEnd,void* userData)
	...
	if(thisPtr->headTracked)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Viewer is head-tracked");
...
void Viewer::initialize(const Misc::ConfigurationFileSection& configFileSection)
	...
	configFileSection.updateValue("./headTracked",headTracked);
	if(headTracked)
		{
	...
	configFileSection.updateValue("./viewDirection",deviceViewDirection);
	configFileSection.updateValue("./upDirection",deviceUpDirection);
	configFileSection.updateValue("./monoEyePosition",deviceMonoEyePosition);
	configFileSection.updateValue("./leftEyePosition",deviceLeftEyePosition);
	configFileSection.updateValue("./rightEyePosition",deviceRightEyePosition);
	
	/* Get head light enable flag: */
	if(configFileSection.retrieveValue("./headLightEnabled",true))
		lightsource->enable();
	
	/* Get head light position and direction in head device coordinates: */
	headLightDevicePosition=configFileSection.retrieveValue("./headLightPosition",deviceMonoEyePosition);
	headLightDeviceDirection=configFileSection.retrieveValue("./headLightDirection",deviceViewDirection);
	
	/* Retrieve head light's color: */
	GLLight::Color headLightColor=configFileSection.retrieveValue("./headLightColor",GLLight::Color(1.0f,1.0f,1.0f));
	lightsource->getLight().diffuse=headLightColor;
	lightsource->getLight().specular=headLightColor;
	
	/* Retrieve head light's attenuation coefficients: */
	Misc::FixedArray<GLfloat,3> attenuation;
	attenuation[0]=1.0f;
	attenuation[1]=attenuation[2]=0.0f;
	configFileSection.updateValue("./headLightAttenuation",attenuation);
	lightsource->getLight().constantAttenuation=attenuation[0];
	lightsource->getLight().linearAttenuation=attenuation[1];
	lightsource->getLight().quadraticAttenuation=attenuation[2];
	
	/* Retrieve head light's spotlight settings: */
	lightsource->getLight().spotCutoff=configFileSection.retrieveValue("./headLightSpotCutoff",180.0f);
	lightsource->getLight().spotExponent=configFileSection.retrieveValue("./headLightSpotExponent",0.0f);
