...
class TrackingTest:public Vrui::Application
	...
	struct TrackerSample // Structure representing a tracker state sample
		{
		/* Elements: */
		public:
		Point pos; // Tracker position
		LV lv; // Tracker linear velocity
		};
	...
	struct TrackerState // Structure representing the state of a tracker and its noise history
		...
		bool valid; // Flag if the tracker currently has valid tracking data
		std::vector<TrackerSample> samples; // List of tracker samples received in the last second
	...
	struct TimedTrackerSample:public TrackerSample // Structure representing a time-stamped tracker position sample
		{
		/* Elements: */
		public:
	...
	struct TrackerSampleBuffer // Structure for ring buffers of time-stamped tracker position samples
		...
		TimedTrackerSample* buffer; // Array of buffer elements
		...
		TrackerSampleBuffer(void) // Creates a default-sized buffer
			:bufferSize(65536),
			 buffer(new TimedTrackerSample[bufferSize]),
			 tail(0),size(0),
			 lastTimeStamp(0)
			{
			}
		...
		void addSample(TimeStamp timeStamp,const TS& trackerState) // Enters a new tracking sample into the buffer, if it is new
			...
			if(lastTimeStamp!=timeStamp)
				...
				TimedTrackerSample& back=buffer[(tail+size)%bufferSize];
				back.pos=trackerState.positionOrientation.getOrigin();
				back.lv=trackerState.linearVelocity;
				back.time=timeStamp;
		...
		void copySamples(std::vector<TrackerSample>& samples) const // Copies all tracking samples into the given list
			{
			/* Clear the samples list and make room: */
			samples.clear();
			samples.reserve(size);
			
			/* Copy all samples: */
			for(unsigned int i=0;i<size;++i)
				samples.push_back(buffer[(tail+i)%bufferSize]);
			}
	...
	Vrui::TimePoint nextUpdateTime; // Next time at which to update the main thread with new tracking data
	Threads::TripleBuffer<TrackerState*> trackerStates; // Triple buffer of arrays of tracking states
	Threads::MutexCond trackerStateCond; // Condition variable triggered when a new tracking state arrives
...
void TrackingTest::trackingCallback(Vrui::VRDeviceClient* client)
	...
	if(now>=nextUpdateTime)
		...
		trackerStateCond.signal();
		Vrui::requestUpdate();
		
		/* Schedule the next update: */
		nextUpdateTime+=Vrui::TimeVector(1.0/60.0);
		}
...
void TrackingTest::snapRequest(Vrui::ObjectSnapperToolFactory::SnapRequest& snapRequest)
	{
	/* Snap against all valid tracker positions: */
	...
	for(int trackerIndex=0;trackerIndex<numTrackers;++trackerIndex)
		if(tss[trackerIndex].valid)
			snapRequest.snapPoint(Vrui::Point(tss[trackerIndex].state.positionOrientation.getOrigin()));
	}

void TrackingTest::goToPhysicalCallback(Misc::CallbackData* cbData)
	{
...
GLMotif::PopupMenu* TrackingTest::createMainMenu(void)
	...
	goToPhysicalButton->getSelectCallbacks().add(this,&TrackingTest::goToPhysicalCallback);
...
TrackingTest::TrackingTest(int& argc,char**& argv)
	...
	nextUpdateTime=Vrui::TimePoint()+Vrui::TimeVector(1,0); // Wait one second to build up tracking history
	...
	if(drawSampleHistory)
		...
		for(int trackerIndex=0;trackerIndex<numTrackers;++trackerIndex)
			if(tss[trackerIndex].valid)
				...
				glVertexPointer(3,GL_FLOAT,sizeof(TrackerSample),&tss[trackerIndex].samples.front().pos);
				...
				for(std::vector<TrackerSample>::const_iterator sIt=tss[trackerIndex].samples.begin();sIt!=tss[trackerIndex].samples.end();++sIt)
					{
					glVertex(sIt->pos);
					glVertex(sIt->pos+sIt->lv*Scalar(1.0/360.0)*Scalar(2));
					}
				glEnd();
				}
		glDisableClientState(GL_VERTEX_ARRAY);
...
void TrackingTest::resetNavigation(void)
	{
	/* Access the most recent tracker states: */
	const TrackerState* tss=trackerStates.getLockedValue();
	...
	for(int trackerIndex=0;trackerIndex<numTrackers;++trackerIndex)
		if(tss[trackerIndex].valid)
			{
			const TS& ts=tss[trackerIndex].state;
			bbox.addPoint(ts.positionOrientation.getOrigin());
			}
	
	/* Bring all devices into view: */
	Vrui::Point center=Geometry::mid(bbox.min,bbox.max);
	Vrui::Scalar size=Geometry::dist(center,bbox.max)*Vrui::Scalar(1.25);
