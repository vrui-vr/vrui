				...
				while(tokBegin!=argEnd)
					...
					if(tokEnd-tokBegin==11&&strncasecmp(tokBegin,"translation",11)==0)
						...
						if(sc[0]==sc[1]&&sc[1]==sc[2])
							...
							throw Misc::makeStdErr(0,"Non-uniform scaling in VRML transformation");
						}
						...
						throw Misc::makeStdErr(0,"Unrecognized VRML transform token %s",std::string(tokBegin,tokEnd).c_str());
