...
struct OutputConfiguration // Structure describing the physical size and panning domain of an output
	...
	/* Elements: */
	public:
	int screen; // X screen controlling the output
	int sizeMm[2]; // Width and height of the connected output device in mm, if known
	IRect domain; // The output device's pixel rectangle in the virtual screen
	unsigned int frameInterval; // Frame interval in nanoseconds
	};

OutputConfiguration getOutputConfiguration(Display* display,int screen,const char* outputName); // Returns the configuration of an output of the given name, or the root window of the given screen if no match is found
