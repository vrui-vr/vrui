...
void IKAvatarDriver::configure(const char* configName)
	...
	if(configName!=0&&configName[0]!='\0')
		...
		if(!cfg.isValid())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Configuration %s not found",configName);
		}
	else
		{
		...
		if(!cfg.isValid())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No default configuration found");
		}
	
	/* Configure inverse kinematics calculation parameters: */
	cfg.updateValue("./maxArmExtension",maxArmExtension);
	cfg.updateValue("./minWristDist",minWristDist);
	neckPitchOffset=Math::rad(cfg.retrieveValue("./neckPitchOffset",Math::deg(neckPitchOffset)));
	cfg.updateValue("./neckPitchScale",neckPitchScale);
	maxNeckYaw=Math::rad(cfg.retrieveValue("./maxNeckYaw",Math::deg(maxNeckYaw)));
	clavicleYawScale=Math::rad(cfg.retrieveValue("./clavicleYawScale",Math::deg(clavicleYawScale)));
	maxClavicleYaw=Math::rad(cfg.retrieveValue("./maxClavicleYaw",Math::deg(maxClavicleYaw)));
	clavicleRollScale=Math::rad(cfg.retrieveValue("./clavicleRollScale",Math::deg(clavicleRollScale)));
	maxClavicleRoll=Math::rad(cfg.retrieveValue("./maxClavicleRoll",Math::deg(maxClavicleRoll)));
	cfg.updateValue("./wristRelaxFactor",wristRelaxFactor);
	shoulderRotationAngle=Math::rad(cfg.retrieveValue("./shoulderRotationAngle",Math::deg(shoulderRotationAngle)));
	cfg.updateValue("./pelvisPitchFactor",pelvisPitchFactor);
	cfg.updateValue("./pelvisSway",pelvisSway);
	pelvisFlex=Math::rad(cfg.retrieveValue("./pelvisFlex",Math::deg(pelvisFlex)));
	maxFootWrenchCos=Math::cos(Math::rad(cfg.retrieveValue("./maxFootWrench",Math::deg(Math::acos(maxFootWrenchCos)))));
	footSplay=Math::rad(cfg.retrieveValue("./footSplay",Math::deg(footSplay)));
	cfg.updateValue("./stepTime",stepTime);
	cfg.updateValue("./stepHeight",stepHeight);
	...
	headToViewer=cfg.retrieveValue("./headToViewer",Geometry::invert(ikToViewer)*headToViewer);
	headToViewer.leftMultiply(ikToViewer);
	headToViewer.renormalize();
	
	/* Configure the height of the neck joint above the floor when standing erect: */
	cfg.updateValue("./uprightNeckHeight",uprightNeckHeight);
	...
	for(int armIndex=0;armIndex<2;++armIndex)
		...
		armCfg.updateValue("./handToDevice",arm.handToDevice);
		armCfg.updateValue("./claviclePos",arm.claviclePos);
		armCfg.updateValue("./shoulderPos",arm.shoulderPos);
		arm.neckShoulderPos=arm.claviclePos+(arm.shoulderPos-Point::origin);
		armCfg.updateValue("./upperLength",arm.upperLength);
		arm.upperLength2=Math::sqr(arm.upperLength);
		armCfg.updateValue("./lowerLength",arm.lowerLength);
		arm.lowerLength2=Math::sqr(arm.lowerLength);
		arm.length=arm.upperLength+arm.lowerLength;
		arm.length2=Math::sqr(arm.length);
	...
	cfg.updateValue("./pelvisPos",pelvisPos);
	...
	for(int legIndex=0;legIndex<2;++legIndex)
		...
		/* Initialize leg configuration: */
		legCfg.updateValue("./hipPos",leg.hipPos);
		legCfg.updateValue("./upperLength",leg.upperLength);
		leg.upperLength2=Math::sqr(leg.upperLength);
		legCfg.updateValue("./lowerLength",leg.lowerLength);
		leg.lowerLength2=Math::sqr(leg.lowerLength);
		leg.length=leg.upperLength+leg.lowerLength;
		leg.length2=Math::sqr(leg.length);
		legCfg.updateValue("./toePos",leg.toePos);
		legCfg.updateValue("./heelPos",leg.heelPos);
		legCfg.updateValue("./solePos",leg.solePos);
	...
	setLockFeetToNavSpace(cfg.retrieveValue("./lockFeetToNavSpace",lockFeetToNavSpace));
...
void IKAvatarDriver::configureFromTPose(void)
	...
	if(!getInputGraphManager()->isEnabled(arms[0].handDevice)||!getInputGraphManager()->isEnabled(arms[1].handDevice))
		{
		Misc::sourcedUserError(__PRETTY_FUNCTION__,"Both hand tracking devices require valid tracking data for T-pose calibration");
		return;
		}
...
void IKAvatarDriver::setHandDevice(int armIndex,InputDevice* newHandDevice)
	...
	if(arm.handDevice!=0)
		...
		/* Strip the current hand device's handle transformation from the arm's hand transform: */
		ONTransform ht=getInputDeviceManager()->getHandleTransform(getInputGraphManager()->getRootDevice(arm.handDevice));
		arm.handToDevice.leftMultiply(Geometry::invert(ht));
		}
	...
	if(arm.handDevice!=0)
		{
		/* Add the new hand device's handle transformation to the arm's hand transform: */
		ONTransform ht=getInputDeviceManager()->getHandleTransform(getInputGraphManager()->getRootDevice(arm.handDevice));
		arm.handToDevice.leftMultiply(ht);
