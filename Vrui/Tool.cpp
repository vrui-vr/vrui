...
Tool* ToolFactory::createTool(const ToolInputAssignment&) const
	{
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create tool of abstract class %s",getClassName());
...
void ToolFactory::destroyTool(Tool*) const
	{
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot destroy tool of abstract class %s",getClassName());
	}
...
const ToolFactory* Tool::getFactory(void) const
	{
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Tool of abstract class does not have factory object");
...
void Tool::sound(ALContextData&) const
	{
	/* No default behavior */
	}
