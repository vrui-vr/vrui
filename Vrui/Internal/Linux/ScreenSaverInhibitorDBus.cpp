...
ScreenSaverInhibitorDBus::ScreenSaverInhibitorDBus(void)
	...
	if(dbus_error_is_set(&error))
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to session message bus due to error %s",error.message);
		}
	else if(connection==0)
		{
		dbus_error_free(&error);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to session message bus due to unknown error");
		}
	...
	if(screenSaverService.empty())
		...
		dbus_error_free(&error);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No screen saver found on session DBus");
		}
	...
	if(!inhibitScreenSaver(connection,screenSaverService,screenSaverPath,screenSaverInterface,inhibitCookie))
		...
		dbus_connection_unref(connection);
		dbus_error_free(&error);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot inhibit screen saver");
		}
	}
