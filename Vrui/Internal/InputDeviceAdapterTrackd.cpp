...
InputDeviceAdapterTrackd::InputDeviceAdapterTrackd(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	...
	if(sensorMemoryID<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot access shared sensor memory segment using key %x",int(sensorMemoryKey));
	sensorHeader=static_cast<SensorHeader*>(shmat(sensorMemoryID,0,SHM_RDONLY));
	if(sensorHeader==(SensorHeader*)-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot attach to shared sensor memory segment using key %x",int(sensorMemoryKey));
	...
	if(controllerMemoryID<0)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot access shared controller memory segment using key %x",int(controllerMemoryKey));
		}
	...
	if(controllerHeader==(ControllerHeader*)-1)
		...
		shmdt(sensorHeader);
		sensorHeader=(SensorHeader*)-1;
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot attach to shared controller memory segment using key %x",int(controllerMemoryKey));
		}
	...
	configFileSection.updateValue("./calibrationTransformation",calibrationTransformation);
	
	/* Read the update interval: */
	double updateRate=configFileSection.retrieveValue("./updateRate",0.0);
	if(updateRate==0.0)
		updateInterval=0.0;
	else
	...
	if(configFileSection.retrieveValue("./spinPoll",false))
		{
