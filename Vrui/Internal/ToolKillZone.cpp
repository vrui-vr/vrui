...
ToolKillZone::ToolKillZone(const Misc::ConfigurationFileSection& configFileSection)
	...
	if(baseDeviceName!="")
		...
		if(baseDevice==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown base input device \"%s\"",baseDeviceName.c_str());
		}
	
	/* Update settings from configuration file: */
	configFileSection.updateValue("./killZoneActive",active);
	configFileSection.updateValue("./killZoneRender",render);
	configFileSection.updateValue("./killZoneMaterial",material);
	}
