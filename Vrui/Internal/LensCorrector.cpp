...
LensCorrector::DistortionEquation* parseDistortionEquation(const Misc::ConfigurationFileSection& configFileSection)
	...
	if(strcasecmp(eqType.c_str(),"Polynomial")==0)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown distortion function type %s",eqType.c_str());
...
void LensCorrector::uploadWarpMeshes(void) const
	...
	for(int eye=0;eye<2;++eye)
		...
		glBufferDataARB(GL_ARRAY_BUFFER_ARB,warpMeshSize.volume()*sizeof(WarpMeshVertex),0,GL_STATIC_DRAW_ARB);
...
bool LensCorrector::frameCallback(void* userData)
	...
	if(thisPtr->eyeRotVersion!=thisPtr->hmdConfiguration->getEyeRotVersion())
		{
		/* Mark the screen positions as invalid: */
		moveScreens=true;
		
		/* Mark the eye rotation as up-to-date: */
		thisPtr->eyeRotVersion=thisPtr->hmdConfiguration->getEyeRotVersion();
		}
	...
	if(moveScreens)
		...
		for(int eye=0;eye<2;++eye)
			{
			#if 1
			/* Shift, rotate, and scale the screen so that its calculated FoV matches the HMD's configured FoV: */
			LensConfig& lc=thisPtr->lensConfigs[eye];
			VRScreen* screen=thisPtr->window->getVRScreen(eye);
			Point eyePos=thisPtr->viewer->getDeviceEyePosition(eye==0?Viewer::LEFT:Viewer::RIGHT);
			Point sEyePos=screen->getTransform().inverseTransform(eyePos);
			ONTransform screenT=ONTransform::translateFromOriginTo(eyePos);
			screenT*=ONTransform::rotate(thisPtr->hmdConfiguration->getEyeRotation(eye));
			screenT*=ONTransform::translate(Vector(lc.renderedFovs[0]*sEyePos[2],lc.renderedFovs[2]*sEyePos[2],-sEyePos[2]));
			screenT.renormalize();
			screen->setTransform(screenT);
			screen->setSize((lc.renderedFovs[1]-lc.renderedFovs[0])*sEyePos[2],(lc.renderedFovs[3]-lc.renderedFovs[2])*sEyePos[2]);
			#else
			VRScreen* screen=thisPtr->window->getVRScreen(eye);
...
LensCorrector::LensCorrector(VRWindow& sWindow,const WindowProperties& windowProperties,int multisamplingLevel,const GLWindow::WindowPos viewportPos[2],const Misc::ConfigurationFileSection& configFileSection)
	:window(&sWindow),
	 ...
	 ipdDisplayDialog(0),ipdDisplayDialogTimeout(configFileSection.retrieveValue("./ipdDisplayTimeout",2.0)),
	...
	if(viewportPos[0].size[0]!=viewportPos[1].size[0]||viewportPos[0].size[1]!=viewportPos[1].size[1])
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Left and right viewports have different sizes, %dx%d vs %dx%d",
		                       viewportPos[0].size[0],viewportPos[0].size[1],
											     viewportPos[1].size[0],viewportPos[1].size[1]);
		}
	
	/* Query the display's rotation: */
	double displayRotationAngle=configFileSection.retrieveValue("./displayRotation",0.0);
	...
	if(precomputed)
		...
		for(int eye=0;eye<2;++eye)
			...
			Point eyePos=viewer->getDeviceEyePosition(eye==0?Viewer::LEFT:Viewer::RIGHT);
			Point sEyePos=screen->getTransform().inverseTransform(eyePos);
			ONTransform screenT=ONTransform::translateFromOriginTo(eyePos);
			...
			lc.warpMesh=new WarpMeshVertex[warpMeshSize.volume()];
		...
		eyeRotVersion=hmdConfiguration->getEyeRotVersion();
		...
		double lensCenterDist=configFileSection.retrieveValue("./lensCenterDist",Vrui::getInchFactor()*2.5);
		double lensFocalLength=configFileSection.retrieveValue("./lensFocalLength",Vrui::getInchFactor()*2.5);
		double lensScreenDist=configFileSection.retrieveValue("./lensScreenDist",Math::mid(sWindow.getVRScreen(0)->getWidth(),sWindow.getVRScreen(1)->getWidth()));
		
		double lensProjectionDists[2];
		lensProjectionDists[0]=lensProjectionDists[1]=configFileSection.retrieveValue("./lensProjectionDist",0.0);
		...
		configFileSection.updateValue("./leftLensCenter",lensConfigs[0].lensCenter);
		lensConfigs[0].focalLength=configFileSection.retrieveValue("./leftLensFocalLength",lensFocalLength);
		
		configFileSection.updateValue("./leftLensProjectionDist",lensProjectionDists[0]);
		...
		configFileSection.updateValue("./leftOverscan",leftOverscan);
		for(int i=0;i<4;++i)
		...
		configFileSection.updateValue("./rightLensCenter",lensConfigs[1].lensCenter);
		lensConfigs[1].focalLength=configFileSection.retrieveValue("./rightLensFocalLength",lensFocalLength);
		
		configFileSection.updateValue("./rightLensProjectionDist",lensProjectionDists[1]);
		...
		configFileSection.updateValue("./rightOverscan",rightOverscan);
		for(int i=0;i<4;++i)
			lensConfigs[1].overscan[i]=rightOverscan[i];
		
		/* Calculate derived lens configuration: */
		bool projectScreens=configFileSection.retrieveValue("./projectScreens",false);
		for(int eye=0;eye<2;++eye)
			{
			VRScreen* screen=sWindow.getVRScreen(eye);
			LensConfig& lc=lensConfigs[eye];
		...
		configFileSection.updateValue("./warpMeshSize",wms);
		for(int i=0;i<2;++i)
	...
	double superSampling=configFileSection.retrieveValue("./superSampling",1.0);
	for(int i=0;i<2;++i)
	...
	warpReproject=viewer!=0&&configFileSection.retrieveValue("./warpReproject",warpReproject);
	if(vruiVerbose)
		std::cout<<"\tReprojection "<<(warpReproject?"enabled":"disabled")<<std::endl;
	
	/* Retrieve cubic look-up flag: */
	configFileSection.updateValue("./warpCubicLookup",warpCubicLookup);
	...
	for(int eye=0;eye<2;++eye)
		...
		glTexImage2D(GL_TEXTURE_RECTANGLE_ARB,0,GL_RGB8,predistortionFrameSize,0,GL_RGB,GL_UNSIGNED_BYTE,0);
		}
	...
	if(predistortionStencilBufferSize>0)
		...
		if(predistortionStencilBufferSize>8)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Lens distortion correction not supported with stencil depth %d>8",int(predistortionStencilBufferSize));
	...
	configFileSection.updateValue("./oledCorrectionFactors",ocf);
	...
	if(correctOledResponse)
		...
		configFileSection.updateValue("./fixContrast",fixContrast);
		if(fixContrast)
			{
...
void LensCorrector::prepare(int eye,DisplayState& displayState) const
	...
	displayState.viewport.offset=IOffset(0);
	displayState.viewport.size=predistortionFrameSize;
	displayState.frameSize=predistortionFrameSize;
	glViewport(displayState.viewport);
	}
