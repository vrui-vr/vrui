...
static const ControlKeyMapItem rawControlKeyMap[]=
	...
	{QK(XK_KP_Enter,0x0),TCE(TCE::CONFIRM)}
	};
...
InputDeviceAdapterMouse::InputDeviceAdapterMouse(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	...
	configFileSection.updateValue("./buttonKeys",buttonKeyNames);
	...
	StringList modifierKeyNames;
	configFileSection.updateValue("./modifierKeys",modifierKeyNames);
	...
	configFileSection.updateValue("./grabPointer",grabPointer);
	
	/* Check if this adapter is supposed to draw a fake mouse cursor: */
	configFileSection.updateValue("./fakeMouseCursor",fakeMouseCursor);
	if(fakeMouseCursor)
		{
	...
	configFileSection.updateValue("./mouseIdleTimeout",mouseIdleTimeout);
	...
	if(window!=0)
		...
		if(numMousePosChanges>0)
			...
			if(mouseLocked)
				...
				GLWindow::Offset windowCenter=window->getWindowCenterPos();
				if(mousePos[0]!=Scalar(windowCenter[0])+Scalar(0.5)||mousePos[1]!=Scalar(windowCenter[1])+Scalar(0.5))
					...
					window->setCursorPos(windowCenter);
...
void InputDeviceAdapterMouse::setMousePosition(VRWindow* newWindow,const GLWindow::Offset& newMouse)
	...
	for(int i=0;i<2;++i)
		newMousePos[i]=Scalar(newMouse[i])+Scalar(0.5);
...
void InputDeviceAdapterMouse::lockMouse(void)
	...
	GLWindow::Offset windowCenterPos=window->getWindowCenterPos();
	for(int i=0;i<2;++i)
		mousePos[i]=Scalar(windowCenterPos[i])+Scalar(0.5);
	window->updateScreenDevice(mousePos,inputDevices[0]);
	inputDevices[0]->setLinearVelocity(Vector::zero);
	window->setCursorPos(windowCenterPos);
...
void InputDeviceAdapterMouse::unlockMouse(void)
	...
	window->setCursorPos(lockedMousePos);
	window->updateScreenDevice(mousePos,inputDevices[0]);
	inputDevices[0]->setLinearVelocity(Vector::zero);
