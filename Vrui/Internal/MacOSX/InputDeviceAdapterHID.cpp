...
void InputDeviceAdapterHID::createInputDevice(int deviceIndex,const Misc::ConfigurationFileSection& configFileSection)
	...
	if(*colonPtr!=':'||*endPtr!='\0'||newDevice.vendorId<0||newDevice.productId<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed vendorId:productId string \"%s\" for device %s",deviceVendorProductId.c_str(),newDevice.name.c_str());
	...
	configFileSection.updateValue("./buttonNames",newDevice.buttonNames);
	configFileSection.updateValue("./valuatorNames",newDevice.valuatorNames);
...
InputDeviceAdapterHID::InputDeviceAdapterHID(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	...
	if(hidManager==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot access HID manager");
	...
	if(dictionarySet==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create dictionary set");
	for(std::vector<Device>::iterator dIt=devices.begin();dIt!=devices.end();++dIt)
		...
		if(dictionary==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create dictionary");
		...
		if(!setDictionaryValue(dictionary,kIOHIDVendorIDKey,dIt->vendorId)||!setDictionaryValue(dictionary,kIOHIDProductIDKey,dIt->productId))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set product / vendor IDs in dictionary");
	...
	if(IOHIDManagerOpen(hidManager,kIOHIDOptionsTypeNone)!=kIOReturnSuccess)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open HID manager");
	...
	if(deviceSet==0||CFSetGetCount(deviceSet)==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No devices found");
