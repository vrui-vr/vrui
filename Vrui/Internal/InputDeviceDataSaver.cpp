...
InputDeviceDataSaver::InputDeviceDataSaver(const Misc::ConfigurationFileSection& configFileSection,InputDeviceManager& inputDeviceManager,TextEventDispatcher* sTextEventDispatcher,unsigned int randomSeed)
	...
	for(int i=0;i<numInputDevices;++i)
		...
		const ONTransform& handleTransform=inputDeviceManager.getHandleTransform(inputDevices[i]);
		inputDeviceDataFile->write(handleTransform.getTranslation().getComponents(),3);
		inputDeviceDataFile->write(handleTransform.getRotation().getQuaternion(),4);
