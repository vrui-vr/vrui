...
#include <Vrui/Internal/VRCompositorClient.h>
...
void VRWindow::moveWindow(const NavTransform& transform)
	...
	if(movePrimaryWidgets)
		...
		for(GLMotif::WidgetManager::PoppedWidgetIterator wIt=wm->beginPrimaryWidgets();wIt!=wm->endPrimaryWidgets();++wIt)
			...
			wm->setWidgetTransformation(wIt,GLMotif::WidgetManager::Transformation::translate(translation)*wIt.getWidgetToWorld());
			}
		}
	}
...
int VRWindow::getVisualFlags(const Misc::ConfigurationFileSection& configFileSection)
	...
	bool frontBufferRendering=vsync&&lensCorrection&&!configFileSection.retrieveValue("./useBackBuffer",false);
...
void VRWindow::initContext(GLContext* context,int screen,const WindowProperties& properties,int visualFlags)
	{
	/* Convert the window properties and visual flags into OpenGL context properties: */
	GLContext::Properties contextProperties;
	for(int i=0;i<4;++i)
		contextProperties.colorBufferSize[i]=properties.colorBufferSize[i];
	contextProperties.depthBufferSize=properties.depthBufferSize;
	contextProperties.stencilBufferSize=properties.stencilBufferSize;
	contextProperties.numAuxBuffers=properties.numAuxBuffers;
	for(int i=0;i<4;++i)
		contextProperties.accumBufferSize[i]=properties.accumBufferSize[i];
	contextProperties.numSamples=visualFlags&MULTISAMPLEMASK;
	contextProperties.direct=(visualFlags&DIRECT)!=0x0;
	contextProperties.backbuffer=(visualFlags&BACKBUFFER)!=0x0;
	contextProperties.stereo=(visualFlags&STEREO)!=0x0;
...
GLWindow::WindowPos getInitialWindowPos(const OutputConfiguration& outputConfiguration,const Misc::ConfigurationFileSection& configFileSection)
	...
	Geometry::ComponentArray<int,2> windowSize(outputConfiguration.domainSize[0]/2,outputConfiguration.domainSize[1]/2);
	configFileSection.updateValue("./windowSize",windowSize);
	...
	configFileSection.updateValue("./windowPos",windowPos);
...
VRWindow::VRWindow(GLContext* sContext,const OutputConfiguration& sOutputConfiguration,const char* windowName,const Misc::ConfigurationFileSection& configFileSection,VruiState* sVruiState,InputDeviceAdapterMouse* sMouseAdapter)
	:GLWindow(sContext,sOutputConfiguration.screen,windowName,
	          ...
	          configFileSection.retrieveValue("./decorate",true)),
	 ...
	 mouseScreen(0),
	 showFpsFont(0),
	 showFps(configFileSection.retrieveValue("./showFps",false)),burnMode(false),
	 trackToolKillZone(false),
	 dirty(true),resizeViewport(true),enabled(true),
	 saveScreenshot(false),
	 movieSaver(0),movieSaverRecording(configFileSection.retrieveValue("./saveMovieAutostart",false))
	{
	...
	bool lensCorrection=configFileSection.hasTag("./lensCorrectorName");
	frontBufferRendering=vsync&&lensCorrection&&!configFileSection.retrieveValue("./useBackBuffer",false);
	...
	if(screens[0]==0||screens[1]==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No screen(s) provided");
	...
	if(viewers[0]==0||viewers[1]==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No viewer(s) provided");
	
	/* Override the panning domain from the configuration file: */
	WindowPos panningDomain(outputConfiguration.domainOrigin,outputConfiguration.domainSize);
	configFileSection.updateValue("./panningDomain",panningDomain);
	for(int i=0;i<2;++i)
		{
	...
	if(configFileSection.retrieveValue("./autoScreenSize",false))
		{
	...
	*********************************************************************/
	...
	if(multitouchAdapter!=0&&!XQueryExtension(getContext().getDisplay(),"XInputExtension",&xinput2Opcode,&xiEvent,&xiError))
		{
		Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"XInput extension not supported");
		multitouchAdapter=0;
		}
	...
	if(multitouchAdapter!=0&&(XIQueryVersion(getContext().getDisplay(),&xiVersion[0],&xiVersion[1])!=Success||xiVersion[0]<2||(xiVersion[0]==2&&xiVersion[1]<2)))
		{
		Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"XInput extension does not support multitouch");
		multitouchAdapter=0;
		}
	...
	if(multitouchAdapter!=0)
		...
		if(touchDeviceId==-1)
			{
			Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"No direct-mode multitouch-capable devices found");
			multitouchAdapter=0;
			}
		}
	...
	if(multitouchAdapter!=0)
		...
		if(XISelectEvents(getContext().getDisplay(),getWindow(),&eventMask,1)!=Success)
			{
			Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"Cannot listen for multitouch events");
			multitouchAdapter=0;
			}
	...
	if(windowType==SPLITVIEWPORT_STEREO)
		...
		if(!patternValid)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid interleave pattern %s ",pattern.c_str());
		...
		try
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set mode AutoStereoscopicStereo due to exception %s",err.what());
			}
		...
		if(int(viewMap.getSize(0))!=rootPos.size[0]||int(viewMap.getSize(1))!=rootPos.size[1])
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"View map image size does not match display size");
		...
		if(asQuadSizeUniformIndex<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Interzigging shader does not define quadSize variable");
		}
	...
	if(vsync)
		...
		if(canVsync(frontBufferRendering))
			...
			if(lensCorrector!=0&&lensCorrector->doesReproject())
				...
				preSwapDelay=configFileSection.retrieveValue("./preSwapDelay",0.5f)/1000.0f;
				if(preSwapDelay>0.0f)
					...
					if(canPreVsync())
						...
						Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"Pre-swap delay not supported by local OpenGL");
						preSwapDelay=0.0f;
						}
					}
				}
			...
			Misc::sourcedConsoleError(__PRETTY_FUNCTION__,"Vertical retrace synchronization requested but not supported");
		}
	...
	if(configFileSection.hasTag("./mouseScreenName"))
		...
		if(mouseScreen==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Screen %s does not exist",mouseScreenName.c_str());
		}
	...
	if(configFileSection.retrieveValue("./saveMovie",false))
		...
		getCommandDispatcher().addCommandCallback("VRWindow.toggleMovieSaver",&VRWindow::toggleMovieSaverCallback,this,0,"Toggles the window's movie saver between paused and active");
		}
		...
		case KeyRelease:
			...
			if(event.type==KeyPress)
				...
				if(exitKey.matches(keySym,keyEvent.state))
					...
					Misc::formattedLogNote("VRWindow: Saving window contents as %s",screenshotImageFileName.c_str());
					}
					...
					if(burnMode)
						...
						if(burnModeNumFrames>0)
							...
							Misc::formattedLogNote("VRWindow: Leaving burn mode: %u frames in %f ms,averaging %f fps, frame time range [%f ms, %f ms]",burnModeNumFrames,burnModeTime*1000.0,double(burnModeNumFrames)/burnModeTime,burnModeMin*1000.0,burnModeMax*1000.0);
							}
						else
							Misc::logNote("VRWindow: Leaving burn mode during spin-up phase");
						burnMode=false;
						}
					else
						{
						Misc::logNote("VRWindow: Entering burn mode");
						burnMode=true;
...
void VRWindow::draw(void)
	...
	switch(windowType)
		...
		case SPLITVIEWPORT_STEREO:
			...
			if(lensCorrector!=0)
				...
				lensCorrector->prepare(*displayState);
	...
	if(saveScreenshot)
		...
		try
			...
			Misc::sourcedUserError(__PRETTY_FUNCTION__,"Cannot save screenshot to file %s due to exception %s",screenshotImageFileName.c_str(),err.what());
			}
...
void VRWindow::swapBuffers(void)
	{
	// LOGGING
	static Realtime::TimePointMonotonic timeBase;
	...
	if(preSwapDelay>0.0f)
		...
		// std::cout<<std::setw(8)<<double(Realtime::TimePointMonotonic()-timeBase)*1000.0;
		
		mustWait=!waitForPreVsync(preSwapDelay);
		
		// LOGGING
		// std::cout<<(mustWait?" W ":"   ")<<std::setw(8)<<double(Realtime::TimePointMonotonic()-timeBase)*1000.0;
		...
		// glXWaitX();
		glDrawBuffer(frontBufferRendering?GL_FRONT:GL_BACK);
		lensCorrector->warp();
		glFlush();
	...
	if(preSwapDelay>0.0f)
		{
		// Realtime::TimePointMonotonic newTimeBase;
		// std::cout<<" "<<std::setw(8)<<double(newTimeBase-timeBase)*1000.0<<std::endl;
