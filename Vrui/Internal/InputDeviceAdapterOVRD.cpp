...
inline void writeString(const std::string& string,IO::File& pipe)
	...
	pipe.write(string.data(),string.size());
	}
...
struct ProtocolPacket // Base class for protocol packets
	...
	void write(IO::File& file) const
		...
		file.write(packetType.data(),ptLen);
		}
...
class SharedMemory // Class representing a block of shared memory mapped into the process' address space
	...
	SharedMemory(const char* sharedMemoryName)
		...
		if(fd<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot access shared memory segment %s",sharedMemoryName);
		...
		if(fstat(fd,&sharedMemoryStats)<0)
			{
			close(fd);
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query size of shared memory segment %s",sharedMemoryName);
			}
		...
		if(address==(void*)-1)
			{
			close(fd);
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot map shared memory segment %s",sharedMemoryName);
			}
...
InputDeviceAdapterOVRD::InputDeviceAdapterOVRD(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	:InputDeviceAdapter(sInputDeviceManager),
	 hmdId(~0x0U),
	 cameraTransform(configFileSection.retrieveValue("./cameraTransform",TrackerState::identity)),
	 postTransform(configFileSection.retrieveValue("./postTransform",TrackerState::identity))
	{
	...
	if(connectRep.packetType!="OculusVR_Authorized")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"OVR tracking daemon rejected connection");
	...
	*********************************************************************/
	...
	if(hmdDetectResult.numHmds==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No Oculus Rift HMDs connected");
	...
	if(configFileSection.retrieveValue("./dynamicPrediction",false))
	...
	******************************************/
	...
	std::string name=configFileSection.retrieveString("./name","OculusRift");
