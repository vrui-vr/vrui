...
InputDeviceAdapterVisBox::InputDeviceAdapterVisBox(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	...
	if(sharedMemoryID<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot attach to shared memory segment using key %x",int(sharedMemoryKey));
	...
	if(xyzhpr==(const float*)-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot map shared memory segment using key %x",int(sharedMemoryKey));
