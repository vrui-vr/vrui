...
void InputDeviceAdapterDeviceDaemon::initializeInputDevice(int deviceIndex,const Misc::ConfigurationFileSection& configFileSection)
	{
	...
	for(int vdIndex=0;vdIndex<deviceClient.getNumVirtualDevices();++vdIndex)
		...
		if(vd.name==configFileSection.getName())
			...
			Vector rayDirection=configFileSection.retrieveValue("./deviceRayDirection",vd.rayDirection);
			Scalar rayStart=configFileSection.retrieveValue("./deviceRayStart",vd.rayStart);
			...
			/* Check if there is a non-identity handle transformation: */
			if(vd.handleTransform!=VRDeviceDescriptor::ONTransform::identity)
				{
				/* Register the handle transformation with the input device manager: */
				inputDeviceManager->addHandleTransform(newDevice,vd.handleTransform);
				}
	...
	/* Check if there is a handle transformation: */
	ONTransform handleTransform;
	configFileSection.updateValue("./handleTransform",handleTransform);
	if(handleTransform!=ONTransform::identity)
		{
		/* Register the handle transformation with the input device manager: */
		inputDeviceManager->addHandleTransform(inputDevices[deviceIndex],handleTransform);
		}
	}
...
InputDeviceAdapterDeviceDaemon::InputDeviceAdapterDeviceDaemon(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	...
	/* Update Vrui's environment definition with the one stored on the device daemon: */
	deviceClient.getEnvironmentDefinition(modifyEnvironmentDefinition());
	...
	if(deviceClient.hasSharedMemory())
		{
		/* Retrieve the initial device state: */
...
void InputDeviceAdapterDeviceDaemon::updateInputDevices(void)
	...
	if((predictMotion||inputDeviceManager->isPredictionEnabled())&&deviceDaemonPredictOnUpdate)
		...
		if(inputDeviceManager->isPredictionEnabled())
			...
			const TimePoint& pt=inputDeviceManager->getPredictionTime();
