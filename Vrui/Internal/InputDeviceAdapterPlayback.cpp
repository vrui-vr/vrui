...
InputDeviceAdapterPlayback::InputDeviceAdapterPlayback(InputDeviceManager* sInputDeviceManager,const Misc::ConfigurationFileSection& configFileSection)
	:InputDeviceAdapter(sInputDeviceManager),
	 ...
	 mouseCursorFaker(0),
	 synchronizePlayback(configFileSection.retrieveValue("./synchronizePlayback",false)),
	 quitWhenDone(configFileSection.retrieveValue("./quitWhenDone",false)),
	 soundPlayer(0),
	...
	if(strncmp(header,fileHeader,29)!=0)
		...
		{
		header[32]='\0';
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported input device data file version %s",header+29);
		}
	...
	if(fakeMouseCursorDevice>=0)
		...
		mouseCursorFaker->setCursorSize(configFileSection.retrieveValue("./mouseCursorSize",mouseCursorFaker->getCursorSize()));
		mouseCursorFaker->setCursorHotspot(configFileSection.retrieveValue("./mouseCursorHotspot",mouseCursorFaker->getCursorHotspot()));
		}
	...
	if(saveMovie)
		...
		if(!Misc::isValidTemplate(movieFileNameTemplate,'d',1024))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Movie file name template \"%s\" does not have exactly one %%d conversion",movieFileNameTemplate.c_str());
		
		/* Get the index of the window from which to save the frames: */
		configFileSection.updateValue("./movieWindowIndex",movieWindowIndex);
		
		/* Get the intended frame rate for the movie: */
		movieFrameTimeInterval=1.0/configFileSection.retrieveValue("./movieFrameRate",1.0/movieFrameTimeInterval);
		
		/* Get the number of movie frames to skip: */
		configFileSection.updateValue("./movieSkipFrames",movieFrameStart);
		
		/* Get the index of the first movie frame: */
		configFileSection.updateValue("./movieFirstFrameIndex",movieFrameOffset);
		}
	}
