...
/* Divide signed integers with rounding (tricky because C++ rounds towards zero): */
inline int divideRound(int dividend,int divisor)
	{
	/* Ensure that the divisor is non-negative: */
	if(divisor<0)
		{
		dividend=-dividend;
		divisor=-divisor;
	...
	/* Divide and round depending on whether the dividend is non-negative or negative: */
	if(dividend>=0)
...
inline long divideRound(long dividend,long divisor)
	{
	/* Ensure that the divisor is non-negative: */
	if(divisor<0L)
		{
	...
	/* Divide and round depending on whether the dividend is non-negative or negative: */
	if(dividend>=0L)
		return (dividend+divisor/2L)/divisor;
	else
		return -((-dividend+(divisor-1L)/2L)/divisor);
	}

/* Calculate the greatest common divisor of two integers: */
template <class IntegerParam>
IntegerParam gcd(IntegerParam a,IntegerParam b);

/* Calculate the least common multiplier (common denominator) of two integers: */
template <class IntegerParam>
IntegerParam lcm(IntegerParam a,IntegerParam b);
...
inline long int abs(long int value)
	{
	return labs(value);
	}
