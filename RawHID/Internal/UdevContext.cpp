...
UdevContext::UdevContext(void)
	...
	context=udev_new();
	if(context==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create udev context");
	}
...
void UdevContext::setUserData(void* userData)
	{
	udev_set_userdata(context,userData);
	}

const void* UdevContext::getUserData(void) const
	{
...
UdevDevice UdevContext::getDeviceFromSyspath(const char* syspath)
	...
	if(result==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot find device for syspath %s",syspath);
