...
UdevMonitor::UdevMonitor(void)
	...
	/* Create a monitor for the new context: */
	monitor=udev_monitor_new_from_netlink(context.getContext(),"udev");
	if(monitor==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create udev monitor");
...
UdevMonitor::UdevMonitor(UdevContext& context)
	:monitor(0),fd(-1),listening(false)
	{
	/* Create a monitor for the given context: */
	monitor=udev_monitor_new_from_netlink(context.getContext(),"udev");
	if(monitor==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create udev monitor");
...
UdevMonitor::~UdevMonitor(void)
	{
	/* Release the monitor's udev context: */
...
void UdevMonitor::addSubsystemFilter(const char* subsystem,const char* deviceType)
	...
	if(udev_monitor_filter_add_match_subsystem_devtype(monitor,subsystem,deviceType)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Internal libudev error");
