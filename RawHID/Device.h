...
class Device
	...
	/* Elements: */
	private:
	std::string deviceNode; // Device node's name in the file system
	...
	/* Constructors and destructors: */
	public:
	Device(const char* devnode,int sBusType,unsigned short sVendorId,unsigned short sProductId,const char* sSerialNumber); // Opens a device with exactly specified information, probably from a DeviceMonitor's AddEvent callback
	...
	/* Methods: */
	const std::string& getDeviceNode(void) const // Returns the device's node file name in the file system
		{
		return deviceNode;
		}
