...
void Device::initDevice(const char* devnode,int sBusType,unsigned short sVendorId,unsigned short sProductId,const std::string& sSerialNumber)
	{
	/* Remember the device node name: */
	deviceNode=devnode;
	
	/* Try opening the device node file: */
	fd=open(devnode,O_RDWR);
	if(fd<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot open device node %s",devnode);
...
Device::Device(const char* devnode,int sBusType,unsigned short sVendorId,unsigned short sProductId,const char* sSerialNumber)
	:fd(-1)
	{
	/* Just call the initDevice method already: */
	initDevice(devnode,sBusType,sVendorId,sProductId,sSerialNumber);
	}
...
Device::Device(int busTypeMask,unsigned short sVendorId,unsigned short sProductId,unsigned int index)
	...
	if(fd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot find device %04x:%04x, index %u",sVendorId,sProductId,index);
	}
...
Device::Device(const Device::DeviceMatcher& deviceMatcher,unsigned int index)
	...
	/* Check if device was found: */
	if(fd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot find device");
	}
...
Device::Device(int busTypeMask,unsigned short sVendorId,unsigned short sProductId,const std::string& sSerialNumber)
	...
	/* Check if device was found: */
	if(fd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot find device %04x:%04x, serial number %s",sVendorId,sProductId,sSerialNumber.c_str());
	}
...
size_t Device::readReport(Device::Byte* report,size_t reportSize)
	...
	if(readResult<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Unable to read report");
...
void Device::readSizedReport(Device::Byte* report,size_t reportSize)
	...
	ssize_t readResult=read(fd,report,reportSize);
	if(readResult<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot read");
	else if(size_t(readResult)<reportSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Read truncated by %u bytes",(unsigned int)(reportSize-sizeof(readResult)));
	}
...
void Device::writeReport(const Device::Byte* report,size_t reportSize)
	...
	if(writeResult<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot write");
	else if(size_t(writeResult)<reportSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Write truncated by %u bytes",(unsigned int)(reportSize-sizeof(writeResult)));
	}
...
void Device::readSizedFeatureReport(Device::Byte* report,size_t reportSize)
	...
	if(ioctlResult<0)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Read truncated by %u bytes",(unsigned int)(reportSize-sizeof(ioctlResult)));
	}
...
void Device::writeFeatureReport(const Device::Byte* report,size_t reportSize)
	...
	if(ioctlResult<0)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Write truncated by %u bytes",(unsigned int)(reportSize-sizeof(ioctlResult)));
	}
