...
class PoolAllocator
	...
	PoolAllocator(PoolAllocator&& source) // Move constructor
		:firstChunk(source.firstChunk),slotSize(source.slotSize),
		 numSlotsPerChunk(source.numSlotsPerChunk),firstSlot(source.firstSlot)
		{
		/* Invalidate the source object: */
		source.firstChunk=0;
		}
	PoolAllocator& operator=(PoolAllocator&& source) // Move assignment operator
		{
		if(this!=&source)
			{
			/* Destroy this object: */
			...
			firstChunk=source.firstChunk;
			slotSize=source.slotSize;
			numSlotsPerChunk=source.numSlotsPerChunk;
	...
	private:
	PoolAllocator(const PoolAllocator& source); // Prohibit copy constructor
	PoolAllocator& operator=(const PoolAllocator& source); // Prohibit assignment operator
	public:
