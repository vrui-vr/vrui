...
class MessageLogger:public RefCounted
	...
	enum Target // Enumerated type for message targets
		{
		Log=0, // Message can go to some log to be viewed off-line
	...
	protected:
	int targetFds[3]; // Array of file descriptors to send messages for the three defined targets
	int minMessageLevel; // Minimum message severity level that gets logged
	bool printTimeStamps[3]; // Flags to print time stamps for the three defined targets
	...
	virtual void setLogFile(const char* logFileName); // Starts routing log messages to a file of the given name; stops routing messages if file name is null
	virtual void setMinMessageLevel(int newMinMessageLevel); // Sets the minimum logged message severity level
	virtual void setPrintTimeStamps(Target target,bool newPrintTimeStamps); // Enables or disables printing of time stamps for the given target
	...
	static void logFormattedMessage(const char* prettyFunction,Target target,int messageLevel,const char* formatString,...); // Logs a printf-style formatted message from the given source, assumed to be an evaluation of the __PRETTY_FUNCTION__ macro, of the given level
	static void logFormattedMessage(const char* prettyFunction,Target target,int messageLevel,const char* formatString,va_list args); // Ditto, using a va_list variable argument structure
	};
...
inline void sourcedLogNote(const char* prettyFunction,const char* formatString,...) // Logs a formatted note to the message log
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Log,MessageLogger::Note,formatString,args);
	va_end(args);
	}

inline void sourcedLogWarning(const char* prettyFunction,const char* formatString,...) // Logs a formatted warning to the message log
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Log,MessageLogger::Warning,formatString,args);
	va_end(args);
	}

inline void sourcedLogError(const char* prettyFunction,const char* formatString,...) // Logs a formatted error to the message log
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Log,MessageLogger::Error,formatString,args);
	va_end(args);
	}
...
inline void sourcedConsoleNote(const char* prettyFunction,const char* formatString,...) // Logs a formatted note to the console
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Console,MessageLogger::Note,formatString,args);
	va_end(args);
	}

inline void sourcedConsoleWarning(const char* prettyFunction,const char* formatString,...) // Logs a formatted warning to the console
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Console,MessageLogger::Warning,formatString,args);
	va_end(args);
	}

inline void sourcedConsoleError(const char* prettyFunction,const char* formatString,...) // Logs a formatted error to the console
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::Console,MessageLogger::Error,formatString,args);
	va_end(args);
	}
...
inline void sourcedUserNote(const char* prettyFunction,const char* formatString,...) // Logs a formatted note to the user
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::User,MessageLogger::Note,formatString,args);
	va_end(args);
	}

inline void sourcedUserWarning(const char* prettyFunction,const char* formatString,...) // Logs a formatted warning to the user
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::User,MessageLogger::Warning,formatString,args);
	va_end(args);
	}

inline void sourcedUserError(const char* prettyFunction,const char* formatString,...) // Logs a formatted error to the user
	{
	va_list args;
	va_start(args,formatString);
	MessageLogger::logFormattedMessage(prettyFunction,MessageLogger::User,MessageLogger::Error,formatString,args);
	va_end(args);
	}
