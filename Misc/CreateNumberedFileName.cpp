...
char* createNumberedFileName(const char* baseFileName,int numDigits,char* fileNameBuffer)
	...
	if(directory==0)
		throw makeStdErr(__PRETTY_FUNCTION__,"Cannot search directory for file %s",baseFileName);
...
std::string createNumberedFileName(const std::string& baseFileName,int numDigits)
	...
	if(directory==0)
		throw makeStdErr(__PRETTY_FUNCTION__,"Cannot search directory for file %s",baseFileName.c_str());
