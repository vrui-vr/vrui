...
class ReadBuffer
	...
	class ReadError:public std::runtime_error // Exception class to report buffer reading errors
		...
		public:
		ReadError(const char* source,size_t numBytes,size_t numBytesRead); // Error where numBytesRead were read instead of numBytes
		};
	...
	void readRaw(void* data,size_t dataSize) // Reads a chunk of data from the buffer
		...
		if(unread>=dataSize)
			...
			throw ReadError(__PRETTY_FUNCTION__,dataSize,unread);
		}
