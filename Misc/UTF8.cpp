...
unsigned int UTF8::decodeFirst(unsigned char code[4])
	...
	if(code[0]<0x80U)
		...
		{
		throw makeStdErr(__PRETTY_FUNCTION__,"Synchronization lost");
		...
		{
		throw makeStdErr(__PRETTY_FUNCTION__,"Invalid code byte");
		return 0; // Never reached; just to make compiler happy
		}
	}
...
unsigned int UTF8::decode(std::string::const_iterator& begin,std::string::const_iterator end)
	...
	if(result>=0x80U)
		...
		unsigned int numContinuationBytes=0;
		...
		while(numContinuationBytes>0)
			...
			if(begin==end)
				throw makeStdErr(__PRETTY_FUNCTION__,"Truncated character");
...
unsigned int UTF8::encode(unsigned int c,unsigned char code[4])
	...
	if(c<0x80U) // 7 significant bits
		...
		if(c<0x800U) // 11 significant bits
			...
			throw makeStdErr(__PRETTY_FUNCTION__,"Invalid character code");
		
		/* Encode the character: */
		for(unsigned int byte=numBytes-1;byte>0;--byte,c>>=6)
