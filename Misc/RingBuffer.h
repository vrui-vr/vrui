...
class RingBuffer
	...
	/* Private methods: */
	void increaseBufferSize(void) // Increases the buffer size if there is overrun
		{
	...
	RingBuffer& clear(size_t resetBufferSize =0) // Clears the ring buffer; resets buffer to given size if !=0
		{
		/* Destroy all entries in place: */
		if(tail>=head)
			{
			/* Destroy from head to tail: */
			for(;head!=tail;++head)
		...
		if(resetBufferSize!=0&&resetBufferSize+1!=size_t(bufferEnd-buffer))
			{
			/* Re-allocate the buffer: */
			free(buffer);
			buffer=static_cast<Entry*>(malloc((resetBufferSize+1)*sizeof(Entry)));
			bufferEnd=buffer+(resetBufferSize+1);
			}
	...
	RingBuffer& push_back(const Entry& newEntry) // Places the given entry at the end of the buffer
		...
		/* Increase the buffer size if the buffer became full: */
		if(tail==head)
			increaseBufferSize();
