...
class MemMappedFile
	...
	class WriteError:public std::runtime_error // Exception class to report file writing errors
		...
		WriteError(const char* source,size_t numBytes,size_t numBytesWritten)
			:std::runtime_error(makeStdErrMsg(source,"Error writing %u bytes to file, wrote %u bytes instead",numBytes,numBytesWritten))
			{
			}
		};
	...
	int puts(const char* string)
		...
		if(!writeProtected)
			throw WriteError(__PRETTY_FUNCTION__,1,0);
	...
	void readRaw(void* buffer,size_t size)
		...
		if(ioPtr+size>blockEnd)
			throw ReadError(__PRETTY_FUNCTION__,size,blockEnd-ioPtr);
	...
	void writeRaw(const void* buffer,size_t size)
		...
		/* Check if writing is allowed: */
		if(!writeProtected)
			throw WriteError(__PRETTY_FUNCTION__,size,0);
		
		/* Check if the memory block can hold enough data: */
		if(ioPtr+size>blockEnd)
			throw WriteError(__PRETTY_FUNCTION__,size,blockEnd-ioPtr);
