...
PathType getPathTypeFollowLinks(const char* path); // Ditto, but resolves symbolic links and returns the type of the final linked filesystem object
...
bool isFileWriteable(const char* path); // Returns true if the given path points to a regular file that can be written by the current user
