...
class ConfigurationFileBase
	...
	class MalformedConfigFileError:public std::runtime_error // Exception class to report malformed configuration files
		...
		public:
		MalformedConfigFileError(const char* source,const char* error,int lineNumber,const char* configFileName);
		};
	...
	class SectionNotFoundError:public std::runtime_error // Exception class to report missing sections
		...
		/* Constructors and destructors: */
		public:
		SectionNotFoundError(const char* source,const char* sectionPath,const char* subsectionName);
		};
	...
	class TagNotFoundError:public std::runtime_error // Exception class to report missing tags
		...
		/* Constructors and destructors: */
		public:
		TagNotFoundError(const char* source,const char* sectionPath,const char* tagName);
		};
	...
	class SectionValueCoder // Base class giving type-level access to tag values of a specified section
		...
		std::string& updateString(const char* tag,std::string& currentValue) const // Updates the given string value; does nothing if tag does not exist
			{
			currentValue=baseSection->retrieveTagValue(tag,currentValue);
			return currentValue;
			}
		...
		std::string& updateString(const char* tag,std::string& currentValue) // Updates the given string value; adds tag if tag does not exist
			{
			currentValue=baseSection->retrieveTagValue(tag,currentValue);
			return currentValue;
			}
		...
		template <class ValueParam,class ValueCoderParam>
		ValueParam& updateValueWC(const char* tag,ValueParam& currentValue,ValueCoderParam& coder) const // Updates the given value; does nothing if tag does not exist
			{
			const std::string* value=baseSection->findTagValue(tag);
			if(value!=0)
				currentValue=coder.decode(value->data(),value->data()+value->size());
			return currentValue;
			}
		template <class ValueParam,class ValueCoderParam>
		...
		template <class ValueParam,class ValueCoderParam>
		ValueParam& updateValueWC(const char* tag,ValueParam& currentValue,ValueCoderParam& coder) // Updates the given value; adds tag if tag does not exist
			{
			const std::string* value=baseSection->findTagValue(tag);
			if(value!=0)
				currentValue=coder.decode(value->data(),value->data()+value->size());
			else
				baseSection->storeTagValue(tag,coder.encode(currentValue));
			return currentValue;
			}
		template <class ValueParam,class ValueCoderParam>
		...
		*******************************************************************/
		...
		template <class ValueParam>
		ValueParam& updateValue(const char* tag,ValueParam& currentValue) const // Ditto
			{
			const std::string* value=baseSection->findTagValue(tag);
			if(value!=0)
				currentValue=ValueCoder<ValueParam>::decode(value->data(),value->data()+value->size());
			return currentValue;
			}
		template <class ValueParam>
		...
		template <class ValueParam>
		ValueParam& updateValue(const char* tag,ValueParam& currentValue) // Ditto
			{
			const std::string* value=baseSection->findTagValue(tag);
			if(value!=0)
				currentValue=ValueCoder<ValueParam>::decode(value->data(),value->data()+value->size());
			else
				baseSection->storeTagValue(tag,ValueCoder<ValueParam>::encode(currentValue));
			return currentValue;
			}
		template <class ValueParam>
...
class ConfigurationFile:public ConfigurationFileBase,public ConfigurationFileBase::SectionValueCoder
	...
	ConfigurationFileSection getSection(const char* relativePath) const; // Returns pointer to the given relative path
