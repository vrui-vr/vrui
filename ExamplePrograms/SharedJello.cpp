...
SharedJello::SharedJello(int& argc,char**& argv)
	...
	if(serverHostName==0||serverPortID<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No server host name or port ID provided");
	...
	if(readMessage(*pipe)!=CONNECT_REPLY)
		...
		/* Bail out: */
		pipe=0;
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Connection refused by shared Jell-O server");
		}
