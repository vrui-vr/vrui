...
void* ClusterJello::simulationThreadMethodMaster(void)
	...
	while(true)
		...
		if(lastFrameTime>=nextUpdateTime)
			...
			if(clusterPipe!=0)
				{
				/* Broadcast the crystal state to the cluster: */
...
ClusterJello::ClusterJello(int& argc,char**& argv)
	...
	if(Vrui::isHeadNode())
		...
		/* Send the initial simulation parameters to the cluster: */
		...
		{
		/* Receive the initial simulation parameters from the head node: */
