...
class ShowEarthModel:public Vrui::Application,public SceneGraph::GraphNode,public GLObject
	{
	/* Embedded classes: */
	private:
	...
	class RotatedGeodeticCoordinateTransform:public Vrui::GeodeticCoordinateTransform
		...
		/* New methods: */
		Vrui::Scalar getRotationAngle(void) const
			{
	...
	/* Elements: */
	SceneGraph::ONTransformNodePointer rotationNode; // Scene graph node implementing the rotation animation
	...
	std::vector<SceneGraph::GraphNodePointer> sceneGraphs; // Vector of scene graphs to render
	std::vector<bool> sceneGraphAddeds; // Flag whether each scene graph is currently a child of the rotation node
	...
	virtual SceneGraph::Box calcBoundingBox(void) const;
	virtual void glRenderAction(SceneGraph::GLRenderState& renderState) const;
	
	/* Methods from class GLContext: */
