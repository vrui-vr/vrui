...
class OptotypeTexture:public GLMotif::Texture
	...
	protected:
	virtual void uploadTexture(GLuint textureObjectId,bool npotdtSupported,const Size& textureSize,GLContextData& contextData) const;
...
void OptotypeTexture::uploadTexture(GLuint textureObjectId,bool npotdtSupported,const GLMotif::Texture::Size& textureSize,GLContextData& contextData) const
	{
	/* Create a frame buffer to render an optotype into the widget's texture: */
	GLFrameBuffer fb(textureSize,!npotdtSupported);
	
	#if 0
	/* Create the texture image: */
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGB8,textureSize,0,GL_RGB,GL_UNSIGNED_BYTE,0);
	...
	Misc::Rect<2> previousViewport=glGetViewport();
	glViewport(getSize());
	glPushMatrix();
	glLoadIdentity();
	...
	glCopyTexImage2D(GL_TEXTURE_2D,0,GL_RGB8,textureSize,0);
	}
...
OptotypeTexture::OptotypeTexture(const char* sName,GLMotif::Container* sParent,Vrui::Scalar sAngle,bool sManageChild)
	...
	for(int i=0;i<2;++i)
		resolution[i]=GLfloat(getSize()[i])/(GLfloat(Vrui::getUiSize())*4.0f);
