...
class LDKernel // Minimization kernel to calculate lens distortion parameters
	...
	LDKernel(const Point* sQuad,const Point* sEdge,const Images::Size& imageSize)
...
class ImageLoader:public Threads::WorkerPool::JobFunction // Class to load an image file from a background thread
	{
	/* Elements: */
	private:
	...
	virtual void operator()(int parameter) const
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot call const method");
		}
	virtual void operator()(int parameter)
		{
		/* Load the requested image file: */
		IO::FilePtr imageFile=directory->openFile(fileName.c_str());
		image=Images::readGenericImageFile(*imageFile,Images::getImageFileFormat(fileName.c_str()));
		}
	
	/* New methods: */
	const Images::BaseImage& getImage(void) const // Returns the loaded image when the job is complete
		{
		return image;
		}
	};
...
GLMotif::PopupMenu* ImageViewer::createMainMenu(void)
	...
	mainMenu->setTitle("ImageViewer");
	
	/* Create a button to load image files: */
...
/* Helper function to create a filename filter for supported image file formats: */

std::string createImageExtensionFilter(void)
	{
	/* Get the list of supported image file name extensions from the Image library: */
	std::vector<std::string> supportedExtensions=Images::getSupportedImageFileExtensions();
	
	/* Create a file selection dialog extension filter: */
	std::vector<std::string>::iterator seIt=supportedExtensions.begin();
	std::string result(*seIt);
	for(++seIt;seIt!=supportedExtensions.end();++seIt)
		{
		result.push_back(';');
		result.append(*seIt);
		}
