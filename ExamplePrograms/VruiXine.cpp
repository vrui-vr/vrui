...
class VruiXine:public Vrui::Application,public GLObject
	...
	std::string icpFileName; // File name of the current intrinsic camera parameters
	bool haveIcp; // Flag if there are valid intrinsic camera parameters
	Video::IntrinsicParameters icp; // Current intrinsic camera parameters
	...
	void loadVideoCommandCallback(const char* argumentBegin,const char* argumentEnd); // Callback called from the "VruiXine::loadVideo" console command
...
void VruiXine::setStereoMode(int newStereoMode)
	...
	lockScreenToEyes=((stereoMode==1||stereoMode==2)&&(screenMode==2||screenMode==3))||screenMode==4;
	}
...
void VruiXine::loadVideo(const char* newVideoFileName)
	...
	try
		...
		/* Read intrinsic camera parameters if present: */
		if(cfg.hasTag("./icp"))
			{
			/* Read the Base64-encoded intrinsic camera parameters into a file: */
			std::string icpString=cfg.retrieveValue<std::string>("./icp");
			IO::VariableMemoryFilePtr encoded=new IO::VariableMemoryFile;
			for(std::string::iterator isIt=icpString.begin();isIt!=icpString.end();++isIt)
				encoded->write(char(*isIt));
			encoded->flush();
			
			/* Decode and read the intrinsic camera parameters: */
			IO::Base64Filter decoder(encoded);
			icp.read(decoder);
			haveIcp=true;
			}
	...
	/* Enable or disable "true view" mode: */
	screenModes->getChild(4)->setEnabled(haveIcp);
...
GLMotif::PopupWindow* VruiXine::createStreamControlDialog(void)
	...
	for(int i=0;i<4;++i)
		...
		cropSliders[i]->setValueRange(0.0,10000.0,1.0);
...
void VruiXine::saveConfigurationCallback(Misc::CallbackData* cbData)
	...
	try
		...
		/* Write intrinsic camera parameters if there are valid ones: */
		if(haveIcp)
			{
			/* Write the intrinsic camera parameters into a Base64-encoded binary buffer: */
			IO::VariableMemoryFilePtr encoded=new IO::VariableMemoryFile;
			{
			IO::Base64Filter encoder(encoded);
			icp.write(encoder);
			}
			encoded->flush();
			
			/* Read the encoded parameters into a string: */
			encoded->rewind();
			std::string icpString;
			while(!encoded->eof())
				icpString.push_back(encoded->getChar());
			
			/* Write the encoded parameters: */
			cfg.storeValue<std::string>("./icp",icpString);
			}
...
GLMotif::PopupWindow* VruiXine::createScreenControlDialog(void)
	...
	screenModes->addToggle("True View");
	screenModes->getChild(4)->setEnabled(false);
...
void VruiXine::loadVideoCommandCallback(const char* argumentBegin,const char* argumentEnd)
	{
	/* Extract the media resource locator: */
	while(argumentEnd!=argumentBegin&&isspace(argumentEnd[-1]))
		--argumentEnd;
	std::string mrl(argumentBegin,argumentEnd);
...
VruiXine::VruiXine(int& argc,char**& argv)
	...
	for(int i=1;i<argc;++i)
		...
		if(argv[i][0]=='-')
			...
			if(strcasecmp(argv[i]+1,"vo")==0)
			...
			else if(strcasecmp(argv[i]+1,"icp")==0)
				{
				++i;
				if(i<argc)
					icpFileName=argv[i];
				else
					std::cerr<<"Dangling option -icp ignored"<<std::endl;
				}
				...
				screenMode=3;
			else if(strcasecmp(argv[i]+1,"calibrated")==0)
				screenMode=4;
	...
	if(mrl==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No stream MRL provided");
	
	/* Point the video file selection helper to the given stream's directory: */
	videoFileSelectionHelper.setCurrentDirectory(IO::openFileDirectory(mrl));
	
	/* Load intrinsic camera parameters if requested: */
	if(!icpFileName.empty())
		{
		try
			{
			/* Read the intrinsic camera parameters file: */
			IO::FilePtr icpFile=IO::openFile(icpFileName.c_str());
			icpFile->setEndianness(Misc::LittleEndian);
			icp.read(*icpFile);
			
			/* Mark intrinsic camera parameters as valid: */
			haveIcp=true;
			}
		catch(const std::runtime_error& err)
			{
			/* Show an error message: */
			Misc::formattedUserError("Load ICP: Unable to load ICP file %s due to exception %s",icpFileName.c_str(),err.what());
			icpFileName.clear();
			}
		}
	
	/* Disable calibrated display if there are no intrinsic camera parameters: */
	if(!haveIcp&&screenMode==4)
		screenMode=0;
	...
	if((xine=xine_new())==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create xine engine instance");
	...
	if((videoOutPort=xine_open_video_driver(xine,videoOutDriver,XINE_VISUAL_TYPE_RAW,&rawVisual))==0)
		...
		shutdownXine();
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open video output port for driver %s",videoOutDriver!=0?videoOutDriver:"auto");
		}
	...
	if((audioOutPort=xine_open_audio_driver(xine,audioOutDriver,0))==0)
		...
		/* Shut down and signal an error: */
		shutdownXine();
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open audio output port for driver %s",audioOutDriver!=0?audioOutDriver:"auto");
		}
	...
	if((stream=xine_stream_new(xine,audioOutPort,videoOutPort))==0)
		...
		/* Shut down and signal an error: */
		shutdownXine();
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create multimedia stream object");
		}
	...
	if((eventQueue=xine_event_new_queue(stream))==0)
		...
		/* Shut down and signal an error: */
		shutdownXine();
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create event queue");
		}
	...
	/* Register console and pipe commands: */
	Misc::CommandDispatcher& dispatcher=Vrui::getCommandDispatcher();
	dispatcher.addCommandCallback("VruiXine::loadVideo",
	                              Misc::CommandDispatcher::wrapMethod<VruiXine,&VruiXine::loadVideoCommandCallback>,
	                              this,
	                              "<MRL>",
	                              "Immediately plays the video identified by the given MRL");
	}
...
void VruiXine::display(GLContextData& contextData) const
	...
	if(frame.format==XINE_VORAW_YV12)
		...
		if(dataItem->frameTextureVersion!=videoFrameVersion)
			{
			/* Query the appropriate texture pixel format: */
			GLenum pixelFormat=Images::BaseImage::getUseGammaCorrection()?GL_SRGB8_EXT:GL_RGB8;
			
			glPixelStorei(GL_UNPACK_SKIP_ROWS,crop[2]);
			glPixelStorei(GL_UNPACK_ROW_LENGTH,frame.size[0]);
			glPixelStorei(GL_UNPACK_SKIP_PIXELS,crop[0]);
			glTexImage2D(GL_TEXTURE_2D,0,pixelFormat,frameSize[0],frameSize[1],0,GL_RGB,GL_UNSIGNED_BYTE,frame.planes[0]);
			}
		}
	...
	if(dataItem->bufferDataVersion!=screenParametersVersion)
		...
		if(screenMode==4)
			{
			typedef Video::IntrinsicParameters::Scalar IScalar;
			typedef Video::IntrinsicParameters::ImagePoint IPoint;
			typedef Video::IntrinsicParameters::Vector IVector;
			
			/* Create a calibrated screen: */
			for(unsigned int y=0;y<newNvs[1];++y)
				for(unsigned int x=0;x<newNvs[0];++x,++vPtr)
					...
					IPoint ip;
					ip[0]=IScalar(x)*IScalar(frameSize[0]-1)/IScalar(newNvs[0]-1)+IScalar(0.5);
					ip[1]=IScalar(y)*IScalar(frameSize[1]-1)/IScalar(newNvs[1]-1)+IScalar(0.5);
					IVector v=icp.unproject(ip);
					v.normalize();
					vPtr->position[0]=GLfloat(v[0]*fullSphereRadius);
					vPtr->position[1]=GLfloat(-v[2]*fullSphereRadius);
					vPtr->position[2]=GLfloat(v[1]*fullSphereRadius);
					}
			}
			...
			if(screenCurvature>Vrui::Scalar(0.005))
				...
				if(screenMode!=0&&haveIcp)
					{
					const Video::LensDistortion& ld=icp.getLensDistortion();
					Video::LensDistortion::Scalar usx=Video::LensDistortion::Scalar(icp.getImageSize()[0])/Video::LensDistortion::Scalar(newNvs[0]-1);
					Video::LensDistortion::Scalar usy=Video::LensDistortion::Scalar(icp.getImageSize()[1])/Video::LensDistortion::Scalar(newNvs[1]-1);
					Video::LensDistortion::Point up;
					for(unsigned int y=0;y<newNvs[1];++y)
						{
						up[1]=Video::LensDistortion::Scalar(y)*usy;
						for(unsigned int x=0;x<newNvs[0];++x,++vPtr)
							{
							vPtr->texCoord[0]=GLfloat(x)*ts[0]+to[0];
							vPtr->texCoord[1]=1.0f-(GLfloat(y)*ts[1]+to[1]);
							
							/* Calculate the undistorted point position in image space, distort, and transform back to quad space: */
							up[0]=Video::LensDistortion::Scalar(x)*usx;
							Video::LensDistortion::Point p=ld.distort(up);
							vPtr->position[0]=GLfloat(x0+Vrui::Scalar(p[0]/usx)*sx);
							vPtr->position[1]=GLfloat(screenCenter[1]);
							vPtr->position[2]=GLfloat(z0+Vrui::Scalar(p[1]/usy)*sz);
							}
						}
					}
				else
...
void VruiXine::initContext(GLContextData& contextData) const
	...
	/* Fragment shader to render video in gamma-corrected Y'CbCr 4:2:0 format: */
	static const char* fragmentShaderSourceSYV12="\
		uniform sampler2D ypTextureSampler; // Sampler for input Y' texture \n\
		uniform sampler2D cbTextureSampler; // Sampler for input Cb texture \n\
		uniform sampler2D crTextureSampler; // Sampler for input Cr texture \n\
		 \n\
		...
		void main() \n\
			...
			rgb.b=grey+(ypcbcr.g-128.0/255.0)*2.01723; \n\
			 \n\
			/* Store the gamma-corrected RGB color: */ \n\
			gl_FragColor=vec4(sRGBToLinear(rgb.r),sRGBToLinear(rgb.g),sRGBToLinear(rgb.b),1.0); \n\
			} \n\
		";
	...
	if(Images::BaseImage::getUseGammaCorrection())
		dataItem->videoShaders[0].compileFragmentShaderFromString(fragmentShaderSourceSYV12);
	else
