...
PointSet::PointSet(const char* pointFileName,double scaleFactor,const float colorMask[3])
	...
	if(nextChar==EOF)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Early end of file in input file \"%s\"",pointFileName);
	...
	if(!cartesianValid&&!topographicValid)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No point coordinates in input file \"%s\"",pointFileName);
