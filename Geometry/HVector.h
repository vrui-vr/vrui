...
class HVector:public ComponentArray<ScalarParam,dimensionParam+1>
	...
	HVector(ScalarParam sX,ScalarParam sY,ScalarParam sW) // Constructor for 2D (Euclidean) vector
		:ComponentArray<ScalarParam,dimensionParam+1>(sX,sY,sW)
		{
		}
	HVector(ScalarParam sX,ScalarParam sY,ScalarParam sZ,ScalarParam sW) // Constructor for 3D (Euclidean) vector
	...
	template <class SourceScalarParam>
	explicit HVector(const Geometry::Vector<SourceScalarParam,dimensionParam>& v) // Ditto with type conversion
		{
		for(int i=0;i<affineDimension;++i)
			components[i]=ScalarParam(v[i]);
		components[affineDimension]=ScalarParam(0);
		}
	...
	template <class SourceScalarParam>
	explicit HVector(const Geometry::Point<SourceScalarParam,dimensionParam>& p) // Ditto with type conversion
		{
		for(int i=0;i<affineDimension;++i)
			components[i]=ScalarParam(p[i]);
		components[affineDimension]=ScalarParam(1);
		}
	...
	Vector toVector(void) const // Converts homogeneous vector to Euclidean vector
		{
		Vector result;
		for(int i=0;i<affineDimension;++i)
