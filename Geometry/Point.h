...
class Point:public Geometry::ComponentArray<ScalarParam,dimensionParam>
	...
	Point& addScaled(const Geometry::Vector<ScalarParam,dimensionParam>& v,ScalarParam s) // Adds a scaled vector
		{
		for(int i=0;i<dimension;++i)
			components[i]+=v[i]*s;
		return *this;
		}
	Point& subtractScaled(const Geometry::Vector<ScalarParam,dimensionParam>& v,ScalarParam s) // Subtracts a scaled vector
		{
		for(int i=0;i<dimension;++i)
			components[i]-=v[i]*s;
		return *this;
...
template <class ScalarParam,int dimensionParam>
inline Point<ScalarParam,dimensionParam> addScaled(const Point<ScalarParam,dimensionParam>& p,const Vector<ScalarParam,dimensionParam>& v,ScalarParam s) // Adds a scaled vector to a point
	{
	Point<ScalarParam,dimensionParam> result;
	for(int i=0;i<dimensionParam;++i)
		result[i]=p[i]+v[i]*s;
...
template <class ScalarParam>
inline Point<ScalarParam,2> addScaled(const Point<ScalarParam,2>& p,const Vector<ScalarParam,2>& v,ScalarParam s) // Adds a scaled vector to a point
	{
	return Point<ScalarParam,2>(p[0]+v[0]*s,p[1]+v[1]*s);
	}

template <class ScalarParam>
inline Point<ScalarParam,3> addScaled(const Point<ScalarParam,3>& p,const Vector<ScalarParam,3>& v,ScalarParam s) // Adds a scaled vector to a point
	{
	return Point<ScalarParam,3>(p[0]+v[0]*s,p[1]+v[1]*s,p[2]+v[2]*s);
	}

template <class ScalarParam,int dimensionParam>
inline Point<ScalarParam,dimensionParam> subtractScaled(const Point<ScalarParam,dimensionParam>& p,const Vector<ScalarParam,dimensionParam>& v,ScalarParam s) // Subtracts a scaled vector from a point
	{
	Point<ScalarParam,dimensionParam> result;
	for(int i=0;i<dimensionParam;++i)
		result[i]=p[i]-v[i]*s;
...
template <class ScalarParam>
inline Point<ScalarParam,2> subtractScaled(const Point<ScalarParam,2>& p,const Vector<ScalarParam,2>& v,ScalarParam s) // Adds a scaled vector to a point
	{
	return Point<ScalarParam,2>(p[0]-v[0]*s,p[1]-v[1]*s);
	}

template <class ScalarParam>
inline Point<ScalarParam,3> subtractScaled(const Point<ScalarParam,3>& p,const Vector<ScalarParam,3>& v,ScalarParam s) // Adds a scaled vector to a point
	{
	return Point<ScalarParam,3>(p[0]-v[0]*s,p[1]-v[1]*s,p[2]-v[2]*s);
	}

template <class ScalarParam,int dimensionParam>
inline ScalarParam sqrDist(const Point<ScalarParam,dimensionParam>& p1,const Point<ScalarParam,dimensionParam>& p2) // Squared Euclidean distance between two points
	{
...
template <class ScalarParam,int dimensionParam>
inline double dist(const Point<ScalarParam,dimensionParam>& p1,const Point<ScalarParam,dimensionParam>& p2) // Euclidean distance between two points
	{
	double result=0.0;
	for(int i=0;i<dimensionParam;++i)
