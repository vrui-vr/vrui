...
class YpCbCr420Texture:public GLObject
	...
	struct ImagePlane // Structure for image planes
		...
		/* Elements: */
		public:
		Size size; // Plane's width and height
	...
	struct DataItem:public GLObject::DataItem // Structure to hold per-context state
		...
		Size textureSizes[3]; // Current size of the three image plane textures
	...
	const Size& getFrameSize(void) const // Returns the size of the video frame
		{
	...
	void setFrameSize(const Size& newFrameSize); // Sets the video frame size for the next and all following frames
