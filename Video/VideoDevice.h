...
class VideoDevice
	...
	static VideoDevice* openVideoDevice(const std::string& videoDeviceName,unsigned int videoDeviceIndex =0); // Opens the video device of the given name and index among devices of the same name; throws exception if no matching device found
...
/* Type shortcuts: */
typedef std::vector<VideoDevice::DeviceIdPtr> VideoDeviceList;
typedef std::vector<VideoDataFormat> VideoFormatList;
