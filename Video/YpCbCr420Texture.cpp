...
void YpCbCr420Texture::setFrameSize(const Size& newFrameSize)
	{
	/* Initialize the image plane's sizes: */
	for(int i=0;i<3;++i)
		{
		planes[i].size=newFrameSize;
		if(i>0)
			{
			planes[i].size[0]>>=1;
			planes[i].size[1]>>=1;
...
void YpCbCr420Texture::install(GLContextData& contextData,GLfloat texCoord[2]) const
	...
	if(dataItem->shaderSupported)
		...
		for(int i=0;i<3;++i)
			...
			if(dataItem->frameNumber!=frameNumber)
				...
				if(dataItem->npotdtSupported)
					dataItem->textureSizes[i]=planes[i].size;
				else
					{
					...
					if(mustResize)
						...
						glTexImage2D(GL_TEXTURE_2D,0,GL_LUMINANCE8,dataItem->textureSizes[i],0,GL_LUMINANCE,GL_UNSIGNED_BYTE,0);
						}
					}
				...
				if(dataItem->npotdtSupported)
					glTexImage2D(GL_TEXTURE_2D,0,GL_LUMINANCE8,planes[i].size,0,GL_LUMINANCE,GL_UNSIGNED_BYTE,planes[i].base);
				else
					glTexSubImage2D(GL_TEXTURE_2D,0,planes[i].size,GL_LUMINANCE,GL_UNSIGNED_BYTE,planes[i].base);
				}
			}
		...
		if(dataItem->frameNumber!=frameNumber)
			...
			if(dataItem->npotdtSupported)
				dataItem->textureSizes[0]=planes[0].size;
			else
				{
				/* Calculate the next power-of-two texture size: */
				bool mustResize=false;
				...
				if(mustResize)
					{
					/* Re-upload the texture: */
					glTexImage2D(GL_TEXTURE_2D,0,GL_RGB8,dataItem->textureSizes[0],0,GL_RGB,GL_UNSIGNED_BYTE,0);
					}
				}
			
			/* Convert the Y'CbCr 4:2:0 image to a temporary RGB image: */
			unsigned char* rgb=new unsigned char[planes[0].size.volume()*3];
			...
			if(dataItem->npotdtSupported)
				glTexImage2D(GL_TEXTURE_2D,0,GL_RGB8,planes[0].size,0,GL_RGB,GL_UNSIGNED_BYTE,rgb);
			else
				glTexSubImage2D(GL_TEXTURE_2D,0,planes[0].size,GL_RGB,GL_UNSIGNED_BYTE,rgb);
