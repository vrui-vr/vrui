...
class ViewerComponent:public GLObject
	...
	struct DataItem:public GLObject::DataItem // Structure to store per-context OpenGL state
		...
		Size size; // Size of the video frame in the locked video frame buffer slot
		...
		const Size& getSize(void) const // Returns the size of the video frame
			{
			return size;
			}
	...
	std::vector<VideoDevice::DeviceIdPtr> videoDeviceList; // The list of all video devices currently connected to the host
	unsigned int videoDeviceIndex; // The index of the currently used video device in the device list
	VideoDevice* videoDevice; // Pointer to the video recording device
	std::vector<VideoDataFormat> videoFormats; // List of the video device's advertised video formats
	VideoDataFormat videoFormat; // Configured video format of the video device
	ImageExtractor* videoExtractor; // Helper object to convert video frames to RGB
	...
	/* Private methods: */
	void frameCallback(const FrameBuffer* frameBuffer); // Callback receiving incoming video frames
	...
	const Images::BaseImage& getCurrentFrame(void) const // Returns the currently locked video frame as an image
		{
		return videoFrames.getLockedValue();
		}
