...
ImageExtractorBA81::ImageExtractorBA81(const Size& sSize,BayerPattern sBayerPattern)
...
void ImageExtractorBA81::extractYpCbCr(const FrameBuffer* frame,void* image)
	{
	/* Convert the raw image to RGB first: */
	switch(bayerPattern)
		{
		case BAYER_RGGB:
			extractRGBFromRGGB(frame,image);
	...
	unsigned char* cEnd=cPtr+size.volume()*3;
	for(;cPtr!=cEnd;cPtr+=3)
		{
