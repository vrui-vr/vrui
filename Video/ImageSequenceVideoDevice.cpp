...
void ImageSequenceVideoDevice::loadFrame(int frameIndex)
	{
	/* Lock the current frame for updates: */
	...
	currentBuffer.start=static_cast<unsigned char*>(currentFrame.modifyPixels());
	currentBuffer.size=size_t(frameSize[1])*size_t(frameSize[0])*3*sizeof(unsigned char);
...
void* ImageSequenceVideoDevice::refreshThreadMethod(void)
	{
	Misc::Time nextUpdate=Misc::Time::now();
	while(true)
		{
		/* Sleep until the next update interval: */
		Misc::Time now=Misc::Time::now();
		while(now<nextUpdate)
			{
			Misc::sleep(nextUpdate-now);
			now=Misc::Time::now();
			}
		...
		/* Advance the update time: */
		nextUpdate+=Misc::Time(1.0/15.0);
		}
...
ImageSequenceVideoDevice::ImageSequenceVideoDevice(const char* sFrameNameTemplate)
	...
	while(i<length)
		...
		if(frameNameTemplate[i]=='d')
			...
			if(indexBegin==-1)
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"More than one %%d conversion in frame name template %s",frameNameTemplate.c_str());
			}
		else
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid %% conversion in frame name template %s",frameNameTemplate.c_str());
		}
	if(indexBegin==-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No %%d conversion in frame name template %s",frameNameTemplate.c_str());
	...
	if(firstIndex>=lastIndex)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No frame images found");
