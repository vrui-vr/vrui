...
Math::Rational getFrameInterval(dc1394framerate_t framerate)
	{
	switch(framerate)
		{
		case DC1394_FRAMERATE_1_875:
			return Math::Rational(8,15);
			break;
		
		case DC1394_FRAMERATE_3_75:
			return Math::Rational(4,15);
			break;
		
		case DC1394_FRAMERATE_7_5:
			return Math::Rational(2,15);
			break;
		
		case DC1394_FRAMERATE_15:
			return Math::Rational(1,15);
			break;
		
		case DC1394_FRAMERATE_30:
			return Math::Rational(1,30);
			break;
		
		case DC1394_FRAMERATE_60:
			return Math::Rational(1,60);
			break;
		
		case DC1394_FRAMERATE_120:
			return Math::Rational(1,120);
			break;
		
		case DC1394_FRAMERATE_240:
			return Math::Rational(1,240);
			break;
		
		default:
			return Math::Rational::nan;
		}
	}
...
DC1394VideoDevice::DC1394VideoDevice(uint64_t guid,unsigned int unitIndex)
	...
	try
		...
		if((context=dc1394_new())==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open DC1394 device context");
		...
		if(dc1394_camera_enumerate(context,&cameraList)!=DC1394_SUCCESS||cameraList==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query list of cameras in DC1394 device context");
		if(cameraList->num==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"DC1394 device context contains no cameras");
		...
		if(guid!=0)
			...
			if(cameraIndex==-1)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Device context contains no camera with GUID %lx and unit index %u",guid,unitIndex);
			}
		else
			cameraIndex=0;
		...
		if((camera=dc1394_camera_new_unit(context,cameraList->ids[cameraIndex].guid,unitIndex))==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open camera with GUID %lx and unit index %u",cameraList->ids[cameraIndex].guid,unitIndex);
...
std::vector<VideoDataFormat> DC1394VideoDevice::getVideoFormatList(void) const
	...
	if(dc1394_video_get_supported_modes(camera,&videoModes)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's video format list");
	...
	for(unsigned int modeIndex=0;modeIndex<videoModes.num;++modeIndex)
		...
		if(parseVideoMode(videoModes.modes[modeIndex],format))
			...
			if(dc1394_video_get_supported_framerates(camera,videoModes.modes[modeIndex],&frameRates)!=DC1394_SUCCESS)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query list of supported frame rates");
			
			/* Process all frame rates: */
			for(unsigned int frameRateIndex=0;frameRateIndex<frameRates.num;++frameRateIndex)
				{
				/* Convert the frame rate enumerant to a frame interval and store it if it is valid: */
				format.frameInterval=getFrameInterval(frameRates.framerates[frameRateIndex]);
...
VideoDataFormat DC1394VideoDevice::getVideoFormat(void) const
	...
	if(dc1394_video_get_mode(camera,&videoMode)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's current video mode");
	
	/* Parse the video mode: */
	VideoDataFormat result;
	if(!parseVideoMode(videoMode,result))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot parse camera's current video mode");
	
	/* Query the camera's current frame rate: */
	dc1394framerate_t framerate;
	if(dc1394_video_get_framerate(camera,&framerate)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's current frame rate");
	result.frameInterval=getFrameInterval(framerate);
	if(!result.frameInterval.isFinite())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot parse camera's current frame rate");
...
VideoDataFormat& DC1394VideoDevice::setVideoFormat(VideoDataFormat& newFormat)
	...
	switch(newFormat.pixelFormat)
		...
		case 0x20363159: // 'Y16 '
			if(bayerPattern!=BAYER_INVALID)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Camera does not support greyscale pixel formats");
		...
		case 0x32384142: // 'BA82'
			if(bayerPattern==BAYER_INVALID)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Camera does not support Bayer pixel formats");
		...
		default:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown pixel format");
		}
	
	/* Get the list of video modes supported by the camera device: */
	dc1394video_modes_t videoModes;
	if(dc1394_video_get_supported_modes(camera,&videoModes)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's supported video formats");
	...
	if(bestModeIndex==-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Camera does not support the requested video format");
	...
	if(dc1394_video_get_supported_framerates(camera,videoModes.modes[bestModeIndex],&framerates)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's supported frame rates");
	...
	for(unsigned int framerateIndex=0;framerateIndex<framerates.num;++framerateIndex)
		...
		Math::Rational frameInterval=getFrameInterval(framerates.framerates[framerateIndex]);
		
		/* Calculate the smaller ratio of the requested and reported frame rates: */
		Math::Rational ratio=frameInterval/newFormat.frameInterval;
		if(ratio>Math::Rational(1))
			ratio.invert();
		double match(ratio);
	...
	if(bestFramerateIndex==-1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Camera does not support the requested video frame rate");
	newFormat.frameInterval=getFrameInterval(framerates.framerates[bestFramerateIndex]);
	
	/* Set the camera's video mode: */
	if(dc1394_video_set_mode(camera,videoModes.modes[bestModeIndex])!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set camera's video mode");
	if(dc1394_video_set_framerate(camera,framerates.framerates[bestFramerateIndex])!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set camera's frame rate");
...
ImageExtractor* DC1394VideoDevice::createImageExtractor(void) const
	...
	if(format.isPixelFormat("BA81"))
		...
		{
		char fcc[5];
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown pixel format %s",format.getFourCC(fcc));
		}
...
GLMotif::Widget* DC1394VideoDevice::createControlPanel(GLMotif::WidgetManager* widgetManager)
	...
	if(dc1394_feature_get_all(camera,features)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's features");
	...
	for(int featureIndex=0;featureIndex<DC1394_FEATURE_NUM;++featureIndex)
		...
		if(feature->available)
			...
			if(dc1394_feature_get(camera,feature)!=DC1394_SUCCESS)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query camera's feature \"%s\"",featureNames[feature->id-DC1394_FEATURE_MIN]);
...
unsigned int DC1394VideoDevice::allocateFrameBuffers(unsigned int requestedNumFrameBuffers)
	...
	if(dc1394_capture_setup(camera,requestedNumFrameBuffers,DC1394_CAPTURE_FLAGS_DEFAULT)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot allocate frame buffers");
...
FrameBuffer* DC1394VideoDevice::dequeueFrame(void)
	...
	dc1394video_frame_t* frame=0;
	if(dc1394_capture_dequeue(camera,DC1394_CAPTURE_POLICY_WAIT,&frame)!=DC1394_SUCCESS||frame==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot dequeue the next frame buffer");
...
void DC1394VideoDevice::enqueueFrame(FrameBuffer* frame)
	...
	if(myFrame==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching frame buffer type");
	...
	if(dc1394_capture_enqueue(camera,myFrame->frame)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot enqueue the frame buffer");
...
void DC1394VideoDevice::stopStreaming(void)
	...
	if(dc1394_video_set_transmission(camera,DC1394_OFF)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot stop image transfers");
...
void DC1394VideoDevice::releaseFrameBuffers(void)
	...
	if(dc1394_capture_stop(camera)!=DC1394_SUCCESS)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot release frame buffers");
	}
	...
	for(unsigned int i=0;i<cameraList->num;++i)
		...
		if(camera!=0)
			...
			if(cIt.isFinished())
				...
				name.push_back(':');
				char index[10];
				name.append(Misc::print(++cIt->getDest(),index+sizeof(index)-1));
				}
