...
#include <Video/Internal/BayerPattern.h>
...
class MT9V034 // Helper class to communicate with the Oculus Rift DK2 camera's imaging sensor
	...
	void uvcSetGetCurrent(Misc::UInt8 selector,Misc::UInt8* packet,size_t packetSize)
		...
		if(result<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Error during UVC_SET_CUR request");
		
		/* Send a get current request: */
		query.query=UVC_GET_CUR;
		result=ioctl(fd,UVCIOC_CTRL_QUERY,&query);
		if(result<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Error during UVC_GET_CUR request");
		}
	...
	Misc::UInt16 read(Misc::UInt8 registerIndex) // Reads a value from the given register
		...
		if(packet[0]!=0x84U||packet[4]!=0x00U||packet[5]!=0x00U)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"I2C communication error");
	...
	void readEeprom(Misc::UInt16 address,Misc::UInt8 size,Misc::UInt8* buffer) // Reads a block from the controller's EEPROM
		{
		if(size>32)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Block size larger than 32");
	...
	void setupSequence(void) // Performs a setup sequence of unknown meaning
		...
		if(packet[0]!=0x80U||packet[1]!=0xdcU||packet[2]!=0x00U)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"I2C communication error during first exchange");
		...
		if(packet[0]!=0xa0U||packet[1]!=0x98U||packet[2]!=0x00U)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"I2C communication error during second exchange");
		}
	};
...
void OculusRiftDK2VideoDevice::getControlValues(void)
	...
	if(chipVersion!=0x1324U)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported imaging sensor version");
...
OculusRiftDK2VideoDevice::OculusRiftDK2VideoDevice(const char* videoDeviceName)
	...
	/* Read the camera version and serial number strings: */
	sensor.readEeprom(0x0ff0U,16U,reinterpret_cast<Misc::UInt8*>(versionNumber));
	versionNumber[16]='\0';
	
	sensor.readEeprom(0x2800U,32U,reinterpret_cast<Misc::UInt8*>(serialNumber));
	serialNumber[32]='\0';
	
	/* Check if the kernel driver corrects the camera's firmware bug: */
	VideoDataFormat fmt=V4L2VideoDevice::getVideoFormat();
	buggy=fmt.size[0]==376;
	}
...
std::vector<VideoDataFormat> OculusRiftDK2VideoDevice::getVideoFormatList(void) const
	...
	format.frameInterval=Math::Rational(1,30);
	...
	format.frameInterval=Math::Rational(1,60);
	result.push_back(format);
...
void OculusRiftDK2VideoDevice::saveConfiguration(Misc::ConfigurationFileSection& cfg) const
	{
	/* Call the base class method to save frame size, frame rate, and pixel format: */
	VideoDevice::saveConfiguration(cfg);
	}

void OculusRiftDK2VideoDevice::configure(const Misc::ConfigurationFileSection& cfg)
	{
		...
		if(ioctl(videoFd,VIDIOC_QUERYCAP,&videoCap)==0)
			if((videoCap.capabilities&V4L2_CAP_VIDEO_CAPTURE)!=0&&(videoCap.capabilities&V4L2_CAP_STREAMING)!=0)
				...
				if(name=="Camera DK2")
					{
					/* Create a device ID: */
					DeviceId* newDeviceId=new DeviceId("Oculus Rift DK2 Tracking Camera");
