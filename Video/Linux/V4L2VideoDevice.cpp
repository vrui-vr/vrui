...
void V4L2VideoDevice::enumFrameIntervals(VideoDataFormat& format,std::vector<VideoDataFormat>& formatList) const
	...
	while(ioctl(videoFd,VIDIOC_ENUM_FRAMEINTERVALS,&frameIntervalEnum)==0)
		...
		if(frameIntervalEnum.type==V4L2_FRMIVAL_TYPE_DISCRETE)
			...
			format.frameInterval=Math::Rational(frameIntervalEnum.discrete.numerator,frameIntervalEnum.discrete.denominator);
			...
			format.frameInterval=Math::Rational(frameIntervalEnum.stepwise.min.numerator,frameIntervalEnum.stepwise.min.denominator);
			Math::Rational max(frameIntervalEnum.stepwise.max.numerator,frameIntervalEnum.stepwise.max.denominator);
			Math::Rational step(frameIntervalEnum.stepwise.step.numerator,frameIntervalEnum.stepwise.step.denominator);
			while(format.frameInterval<=max)
				{
				/* Add the video data format to the list: */
				formatList.push_back(format);
				
				/* Go to the next frame interval step: */
				format.frameInterval+=step;
				}
			}
...
void V4L2VideoDevice::setControl(unsigned int controlId,const char* controlTag,const Misc::ConfigurationFileSection& cfg)
	...
	if(getVideoDeviceControl(videoFd,controlId,oldControlValue)!=0)
		...
		// throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query control %s",controlTag);
		return;
		}
	...
	if(queryControl.type==V4L2_CTRL_TYPE_INTEGER)
		controlValue=cfg.retrieveValue(controlTag,oldControlValue);
	else if(queryControl.type==V4L2_CTRL_TYPE_BOOLEAN)
		controlValue=cfg.retrieveValue(controlTag,oldControlValue!=0)?1:0;
		...
		if(selectedMenuChoice<0)
			{
			/* Let's agree to fail silently for now: */
			// throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set menu control %s to choice %s",controlTag,menuChoice.c_str());
			return;
			}
		...
		{
		/* Let's agree to fail silently for now: */
		// throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set control %s",controlTag);
		return;
		}
	...
	if(controlValue!=oldControlValue&&setVideoDeviceControl(videoFd,controlId,controlValue)!=0)
		{
		/* Let's agree to fail silently for now: */
		// throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set value for control %s",controlTag);
		return;
		}
	}
...
void V4L2VideoDevice::integerControlChangedCallback(Misc::CallbackData* cbData)
	...
	if(setVideoDeviceControl(videoFd,controlId,controlValue)!=0)
		Misc::userError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,errno,"Cannot set control").c_str());
	}
		...
		if(ioctl(videoFd,VIDIOC_DQBUF,&buffer)<0)
			...
			if(errno==EINTR) // If the ioctl was interrupted, try again
				...
				if(runStreamingThread)
					...
					Misc::userError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,errno,"Shutting down streaming thread").c_str());
					}
...
V4L2VideoDevice::V4L2VideoDevice(const char* videoDeviceName)
	...
	if(videoFd<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot open V4L2 video device %s",videoDeviceName);
	...
	if(ioctl(videoFd,VIDIOC_QUERYCAP,&videoCap)!=0)
		...
		int error=errno;
		close(videoFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,error,"Cannot query capabilities of V4L2 video device %s",videoDeviceName);
		}
	...
	if((videoCap.capabilities&V4L2_CAP_VIDEO_CAPTURE)==0)
		...
		int error=errno;
		close(videoFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,error,"Cannot capture video from V4L2 video device %s",videoDeviceName);
		}
...
VideoDataFormat V4L2VideoDevice::getVideoFormat(void) const
	...
	/* Query the current video format: */
	...
	if(ioctl(videoFd,VIDIOC_G_FMT,&videoFormat)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot query video format");
	...
	if(ioctl(videoFd,VIDIOC_G_PARM,&streamParameters)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot query video frame interval");
	result.frameInterval=Math::Rational(streamParameters.parm.capture.timeperframe.numerator,streamParameters.parm.capture.timeperframe.denominator);
...
VideoDataFormat& V4L2VideoDevice::setVideoFormat(VideoDataFormat& newFormat)
	...
	if(ioctl(videoFd,VIDIOC_S_FMT,&format)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot set video format");
	
	v4l2_streamparm streamParameters;
	memset(&streamParameters,0,sizeof(v4l2_streamparm));
	streamParameters.type=V4L2_BUF_TYPE_VIDEO_CAPTURE;
	streamParameters.parm.capture.timeperframe.numerator=newFormat.frameInterval.getNumerator();
	streamParameters.parm.capture.timeperframe.denominator=newFormat.frameInterval.getDenominator();
	if(ioctl(videoFd,VIDIOC_S_PARM,&streamParameters)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot set video frame interval");
	...
	newFormat.frameInterval=Math::Rational(streamParameters.parm.capture.timeperframe.numerator,streamParameters.parm.capture.timeperframe.denominator);
		...
		if(getVideoDeviceControl(videoFd,queryControl.id,controlValue)==0)
			...
			switch(queryControl.type)
				...
				case V4L2_CTRL_TYPE_INTEGER:
					...
					cfg.storeValue(controlNameToTag(queryControl).c_str(),controlValue);
					break;
				
				case V4L2_CTRL_TYPE_BOOLEAN:
					/* Store a boolean control value: */
					cfg.storeValue(controlNameToTag(queryControl).c_str(),controlValue!=0);
					break;
...
ImageExtractor* V4L2VideoDevice::createImageExtractor(void) const
	...
	else if(format.isPixelFormat("YU12"))
		{
	...
	else if(format.isPixelFormat("YV12"))
		{
		ptrdiff_t yStride=ptrdiff_t(format.size[0])*sizeof(unsigned char);
		ptrdiff_t ySize=ptrdiff_t(format.size[1])*yStride;
		ptrdiff_t cbcrStride=ptrdiff_t((format.size[0]+1)/2)*sizeof(unsigned char);
...
unsigned int V4L2VideoDevice::allocateFrameBuffers(unsigned int requestedNumFrameBuffers)
	...
	if(!canStream)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Video device does not support streaming I/O");
	...
	if(ioctl(videoFd,VIDIOC_REQBUFS,&requestBuffers)==0)
		...
		if(!frameBuffersOk)
			...
			/* Signal an error: */
			throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot allocate memory-mapped streaming buffers");
			}
		}
	else
		{
		...
		if(ioctl(videoFd,VIDIOC_REQBUFS,&requestBuffers)!=0)
			throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot allocate user-space streaming buffers");
...
void V4L2VideoDevice::startStreaming(void)
	...
	if(ioctl(videoFd,VIDIOC_STREAMON,&streamType)!=0)
		...
		/* Throw an exception: */
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,error,"Cannot start streaming video capture");
		}
	}
...
FrameBuffer* V4L2VideoDevice::dequeueFrame(void)
	...
	if(frameBuffersMemoryMapped)
		...
		buffer.memory=V4L2_MEMORY_USERPTR;
	if(ioctl(videoFd,VIDIOC_DQBUF,&buffer)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot dequeue frame buffer");
...
void V4L2VideoDevice::enqueueFrame(FrameBuffer* frame)
	...
	if(ioctl(videoFd,VIDIOC_QBUF,&buffer)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot enqueue frame buffer");
	}
...
void V4L2VideoDevice::stopStreaming(void)
	...
	if(ioctl(videoFd,VIDIOC_STREAMOFF,&streamType)!=0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot stop streaming video capture");
	}
