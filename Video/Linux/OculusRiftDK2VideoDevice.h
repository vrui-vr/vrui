...
class OculusRiftDK2VideoDevice:public V4L2VideoDevice
	...
	/* Elements: */
	private:
	char versionNumber[17]; // Camera's version number
	char serialNumber[33]; // Camera's serial number
	bool buggy; // Flag whether the camera's firmware bug is exposed by the kernel video driver
	...
	const char* getVersionNumber(void) const // Returns the camera's version number string
		{
		return versionNumber;
		}
	const char* getSerialNumber(void) const // Returns the camera's serial number string
		{
		return serialNumber;
		}
