...
class Time:public timespec // Base class for absolute time points or relative time vectors
	...
	#endif
	static void sleepUntil(clockid_t clockId,const Time& wakeupTime); // Sleeps until the clock of the given ID reaches the given absolute wake-up time
	...
	/* Methods: */
	explicit operator timeval(void) const // Converts time object to struct timeval with microsecond resolution
		{
		timeval result;
	...
	explicit operator double(void) const // Converts time object to number of whole and fractional seconds
		{
...
class TimeVector:public Time // Class for relative time vectors
	...
	/* Methods: */
	explicit operator long(void) const // Converts time vector to signed number of nanoseconds; can overflow for very large time intervals
		{
		return long(tv_sec)*1000000000L+long(tv_nsec);
		}
...
class TimePointRealtime:public Time // Class for absolute time points in seconds since the epoch (January 1st, 1970, 12:00 midnight)
	...
	static void sleep(const TimePointRealtime& wakeupTime) // Suspends calling thread until realtime clock reaches given time point
		...
		sleepUntil(CLOCK_REALTIME,wakeupTime);
		}
...
class TimePointMonotonic:public Time // Class for absolute time points in seconds since an unspecified time in the past
	...
	static void sleep(const TimePointMonotonic& wakeupTime) // Suspends calling thread until monotonic clock reaches given time point
		{
		/* Sleep until the wake-up time: */
		sleepUntil(CLOCK_MONOTONIC,wakeupTime);
		}
	static void sleep(const TimeVector& sleepTime) // Suspends calling thread until the monotonic clock has advanced by the given amount
		{
		/* Calculate the wake-up time: */
...
class TimePointRaw:public Time // Class for absolute time points in seconds since an unspecified time in the past; does not experience discontinuities or drift
	...
	explicit TimePointRaw(const timespec& source) // Creates time point from timespec structure
		:Time(source)
		{
		}
	TimePointRaw(time_t sSec,long sNsec) // Creates time point from given number of seconds and nanoseconds
		:Time(sSec,sNsec)
		{
		}
	TimePointRaw(double sSec) // Creates time point from number of whole and fractional seconds
		:Time(sSec)
		{
		}
	
	/* Methods: */
	TimePointRaw& set(void) // Sets time point by sampling the raw monotonic clock
		{
		#if REALTIME_CONFIG_HAVE_POSIX_CLOCKS
		clock_gettime(CLOCK_MONOTONIC_RAW,this);
		#else
	...
	TimeVector setAndDiff(void) // Sets time point by sampling the raw monotonic clock and returns time difference from last value
		{
		/* Get the current time: */
		timespec current;
		#if REALTIME_CONFIG_HAVE_POSIX_CLOCKS
	...
	bool operator<(const TimePointRaw& other) const
		{
		return tv_sec<other.tv_sec||(tv_sec==other.tv_sec&&tv_nsec<other.tv_nsec);
		}
	bool operator<=(const TimePointRaw& other) const
	...
	TimePointRaw& operator+=(const TimeVector& other) // Adds a time vector to a time point
		{
		add(other.tv_sec,other.tv_nsec);
		return *this;
		}
	TimePointRaw& operator-=(const TimeVector& other) // Subtracts a time vector from a time point
		{
		subtract(other.tv_sec,other.tv_nsec);
		return *this;
		}
	static void sleep(const TimePointRaw& wakeupTime) // Suspends calling thread until raw monotonic clock reaches given time point
		{
		/* Sleep until the wake-up time: */
		sleepUntil(CLOCK_MONOTONIC_RAW,wakeupTime);
		}
	static void sleep(const TimeVector& sleepTime) // Suspends calling thread until the raw monotonic clock has advanced by the given amount
		{
		/* Calculate the wake-up time: */
		TimePointRaw wakeupTime;
		wakeupTime+=sleepTime;
		
		/* Sleep until the wake-up time: */
		sleepUntil(CLOCK_MONOTONIC_RAW,wakeupTime);
		}
	void sleepFromCurrent(const TimeVector& sleepTime) const // Suspends calling thread until the raw monotonic clock has advanced by the given amount from the time point
		{
		/* Calculate the wake-up time: */
		TimePointRaw wakeupTime=*this;
		wakeupTime+=sleepTime;
...
inline TimePointRaw operator+(const TimePointRaw& t1,const TimeVector& t2)
	{
	return TimePointRaw(t1.tv_sec+t2.tv_sec,t1.tv_nsec+t2.tv_nsec);
	}

inline TimePointRaw operator+(const TimeVector& t1,const TimePointRaw& t2)
	{
	return TimePointRaw(t1.tv_sec+t2.tv_sec,t1.tv_nsec+t2.tv_nsec);
	}
...
inline TimeVector operator-(const TimePointRaw& t1,const TimePointRaw& t2)
	{
	return TimeVector(t1.tv_sec-t2.tv_sec,t1.tv_nsec-t2.tv_nsec);
	}
