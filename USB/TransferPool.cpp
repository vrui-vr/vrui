...
void TransferPool::transferCallback(libusb_transfer* transfer)
	...
	if(!thisPtr->cancelling)
		...
		if(thisPtr->active.empty())
			Misc::consoleError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Buffer underrun, transmission stalled").c_str());
		}
	}
...
void TransferPool::submit(Device& device,unsigned int endpoint,unsigned int numActiveTransfers,TransferPool::UserTransferCallback* newUserTransferCallback)
	...
	for(unsigned int i=0;i<numActiveTransfers;++i)
		...
		if(submitResult==0)
			...
			libusb_error error=libusb_error(submitResult);
			Misc::consoleError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot submit transfer due to error %d (%s)",error,libusb_strerror(error)).c_str());
			#else
			Misc::consoleError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot submit transfer due to error %d (%s)",submitResult,libusb_error_name(submitResult)).c_str());
			#endif
			}
		}
	}
	if(numSubmittedTransfers<numActiveTransfers)
		Misc::consoleError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Failed submitting %u out of %u requested transfers",numActiveTransfers-numSubmittedTransfers,numActiveTransfers).c_str());
