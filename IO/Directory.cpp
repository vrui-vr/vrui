...
Directory::OpenError::OpenError(const char* source,const char* directoryName)
	:std::runtime_error(Misc::makeStdErrMsg(source,"Cannot open directory %s",directoryName))
	{
	}

Directory::OpenError::OpenError(const char* source,int libcError,const char* directoryName)
	:std::runtime_error(Misc::makeLibcErrMsg(source,libcError,"Cannot open directory %s",directoryName))
	{
	}
	...
	while(compBegin!=length)
		...
		if(compLen==0||(compLen==1&&path[compBegin]=='.'))
			...
			if(compBegin==prefixLength)
				throw OpenError(__PRETTY_FUNCTION__,path.c_str());
...
std::string Directory::createNumberedFileName(const char* fileNameTemplate,int numDigits)
	...
	if(nextFreeNumber>0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No more available names");
