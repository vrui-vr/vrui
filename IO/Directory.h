...
class Directory:public Threads::RefCounted
	...
	class OpenError:public std::runtime_error // Exception class to report directory opening errors
		...
		public:
		OpenError(const char* source,const char* directoryName); // Creates exception for internal errors
		OpenError(const char* source,int libcError,const char* directoryName); // Creates exception for libc errors
		};
