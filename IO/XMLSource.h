...
class XMLSource
	...
	class Processor // Base class for objects that process the structure of an XML file during parsing
		{
		friend class XMLSource;
		
		/* Embedded classes: */
		public:
		enum NodeTypeFlags // Enumerated type for XML document node types and node type components a processor is interested in
			{
			CharacterData=0x1,
			Comment=0x2,
			ProcessingInstruction=0x4,
			Element=0x8,
			Attribute=0x10,
			AllNodeTypes=0x1f
			};
		
		enum DocumentSection // Enumerated type for XML document sections
			{
			Prolog,RootElement,Epilog
			};
		
		/* Elements: */
		private:
		const XMLSource& source; // Reference to the XML source on which the processor operates
		unsigned int nodeTypeMask; // Bit mask of XML document node types  the processor is interested in; disabled node types will be skipped for improved performance
		DocumentSection currentSection; // Currently parsed XML document section
		std::vector<std::string> activeElements; // Stack of names of currenly active XML elements; exactly non-empty while parsing the RootElement document section
		
		/* Constructors and destructors: */
		public:
		Processor(const XMLSource& sSource); // Creates a processor for the given XML source
		virtual ~Processor(void); // Destroys the processor
		
		/* Methods: */
		void setNodeTypeMask(unsigned int newNodeTypeMask); // Sets the bit mask of node types the processor is interested in
		DocumentSection getCurrentSection(void) const // Returns the currently-parsed XML document section
			{
			return currentSection;
			}
		bool inRootElement(void) const // Returns true if currently parsing the XML document's root element
			{
			return currentSection==RootElement;
			}
		const std::string& getCurrentElement(void) const // Returns the name of the currently active XML element; must only be called while parsing the RootElement document section
			{
			return activeElements.back();
			}
		const std::vector<std::string>& getActiveElements(void) const // Returns the stack of currently active XML elements
			{
			return activeElements;
			}
		virtual void characterData(const std::string& characterData); // Processes an uninterrupted segment of XML character data
		virtual void comment(const std::string& comment); // Processes an XML comment
		virtual void processingInstruction(const std::string& target,const std::string& instruction); // Processes an XML processing instruction
		virtual bool startElement(const std::string& name); // Starts processing an XML element of the given name; skips the entire element and all its sub-content including the closing tag if the method returns false
		virtual void attribute(const std::string& name,const std::string& value); // Processes an element attribute
		virtual void enterElement(void); // Enters an XML element's content after all attribute/value pairs have been processed; is not called for elements with self-closing start tags
		virtual void endElement(bool selfClosing); // Ends processing the current XML element; flag is true if the element's start tag was self-closing
		};
	...
	void processCharacterData(Processor& processor); // Reads a segment of XML character data and hands it to the given processor
	void processComment(Processor& processor); // Reads an XML comment and hands it to the given processor
	void processPI(Processor& processor); // Reads an XML processing instruction and hands it to the given processor
	void processElement(Processor& processor); // Reads an XML element and hands it to the given processor
	...
	void process(Processor& processor); // Processes the XML document with the given processor; must be called immediately after construction
	};
