...
XBaseTable::XBaseTable(const char* fileName,SeekableFilePtr sFile)
	...
	if(file->read<char>()!=0x0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Table file %s is encrypted",fileName);
	...
	while(true)
		...
		if(field.dataType=='N'&&field.numDecimals>field.size)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Field \"%s\" in table file %s has size %u, but %u decimals",field.name,fileName,(unsigned int)field.size,(unsigned int)field.numDecimals);
		if(field.dataType!='N'&&field.numDecimals>0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Field \"%s\" in table file %s is not of type NUMBER, but has %u decimals",field.name,fileName,(unsigned int)field.numDecimals);
		if(field.dataType=='L'&&field.size!=1)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Field \"%s\" in table file %s is of type BOOLEAN, but has wrong size %u",field.name,fileName,(unsigned int)field.size);
		if(field.dataType=='D'&&field.size!=8)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Field \"%s\" in table file %s is of type DATE, but has wrong size %u",field.name,fileName,(unsigned int)field.size);
	...
	if(fields.empty())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Table file %s does not define fields",fileName);
	if(recordSize!=size_t(fieldRecordOffset))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Table file %s reports record size %u, but has record size %u",fileName,(unsigned int)recordSize,(unsigned int)fieldRecordOffset);
	Offset currentPos=file->getReadPos();
	if(currentPos>headerSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Table file %s has %u bytes of header data after field definitions",fileName,(unsigned int)(currentPos-headerSize));
	if(currentPos<headerSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Table file %s has %u bytes of missing header data",fileName,(unsigned int)(headerSize-currentPos));
	}
	...
	switch(f.dataType)
		...
		case 'L':
			if(s[0]=='Y'||s[0]=='y'||s[0]=='T'||s[0]=='t')
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unrecognized value \"%c\" in boolean field",s[0]);
			break;
		
		default:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching field data type \"%c\"",f.dataType);
		}
