	...
	do
		...
		if(result==Z_STREAM_END)
			...
			if(inflateEnd(&stream)!=Z_OK)
				{
				if(stream.msg!=0)
					throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error \"%s\" after decompression",stream.msg));
				else
					throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Data corruption detected after decompression"));
				}
			
			/* Put the data that wasn't used by the decompressor back into the source file's buffer: */
			gzippedFile->putBackInBuffer(stream.avail_in);
			...
			{
			if(stream.msg!=0)
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error \"%s\" while decompressing",stream.msg));
			else
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Internal zlib error while decompressing"));
			}
		}
...
void GzipFilter::writeData(const File::Byte* buffer,size_t bufferSize)
	...
	while(stream.avail_in>0)
		...
		if(deflate(&stream,Z_NO_FLUSH)!=Z_OK)
			{
			if(stream.msg!=0)
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error \"%s\" while compressing",stream.msg));
			else
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Internal zlib error while compressing"));
			}
...
void GzipFilter::init(void)
	...
	if(canRead&&canWrite)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot read and write from/to gzipped file simultaneously"));
	else if(canRead)
		{
		...
		if(inflateInit2(&stream,15+32)!=Z_OK) // Detect zlib and gzip headers
			{
			if(stream.msg!=0)
				throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error \"%s\" during initialization",stream.msg));
			else
				throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Internal zlib error during initialization"));
			}
		...
		while(stream.avail_in==0)
			...
			if(result==Z_STREAM_END)
				...
				throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"File is not gzip-compressed"));
			}
		}
	else if(canWrite)
		{
	...
	if(getWriteBufferSize()!=0)
		...
		while(result!=Z_STREAM_END)
			...
			if(result!=Z_STREAM_END&&result!=Z_OK)
				...
				if(stream.msg!=0)
					Misc::userError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error \"%s\" while compressing",stream.msg).c_str());
				else
					Misc::userError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Internal zlib error while compressing").c_str());
