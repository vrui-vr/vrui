		...
		do
			...
			if(inflateResult==Z_STREAM_END)
				...
				if(inflateEndResult!=Z_OK)
					throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Caught zlib error %d after decompression",inflateEndResult));
				eof=true;
				break;
				}
			else if(inflateResult!=Z_OK)
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot decompress due to zlib error %d",inflateResult));
			}
...
void ZipArchiveStreamingFile::writeData(const File::Byte* buffer,size_t bufferSize)
	...
	throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot write to ZIP archives"));
	}

size_t ZipArchiveStreamingFile::writeDataUpTo(const File::Byte* buffer,size_t bufferSize)
	{
	/* Writing is not supported: */
	throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot write to ZIP archives"));
	}
...
ZipArchiveStreamingFile::ZipArchiveStreamingFile(SeekableFilePtr sArchive,unsigned int sCompressionMethod,SeekableFile::Offset sNextReadPos,size_t sCompressedSize)
	...
	if(sCompressionMethod!=0)
		...
		stream->zfree=0;
		stream->opaque=0;
		int inflateInitResult=inflateInit2(stream,-MAX_WBITS);
		if(inflateInitResult!=Z_OK)
			{
			delete[] compressedBuffer;
			delete stream;
			throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot initialize decompressor due to zlib error %d",inflateInitResult));
			}
		}
	}
...
FilePtr ZipArchiveDirectory::openFile(const char* fileName,File::AccessMode accessMode) const
	...
	if(accessMode==IO::File::WriteOnly||accessMode==IO::File::ReadWrite)
		throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot write to file %s in zip archive",fileName));
	...
	if(entry.first!=0)
		...
		if(entry.first->entries[entry.second].filePos!=~Offset(0))
			...
			throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"%s is not a file",fileName));
		}
	else if(entry.second==1)
		throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"%s is a directory",fileName));
	else
		throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"%s not found",fileName));
	}
...
DirectoryPtr ZipArchiveDirectory::openDirectory(const char* directoryName) const
	...
	if(entry.first!=0&&entry.first->entries[entry.second].filePos==~Offset(0))
		...
		throw OpenError(__PRETTY_FUNCTION__,directoryName);
	}
...
bool ZipArchive::Directory::addPath(const char* path,const ZipArchive::FileID& fileId)
	...
	if(eIt==entries.end())
		...
		if(path[nameLen]=='\0')
			...
			return newEntry.child->addPath(path+nameLen+1,fileId);
			}
		}
...
std::pair<ZipArchive::Directory*,unsigned int> ZipArchive::Directory::findPath(const char* path)
	...
	while(*prefixStart!='\0')
		...
		if(prefixLen==2&&prefixStart[0]=='.'&&prefixStart[1]=='.')
		...
		else if(!currentDir->entries.empty())
			{
			...
			if(strncmp(l->name,prefixStart,prefixLen)!=0||l->name[prefixLen]!='\0')
				return std::pair<Directory*,unsigned int>(0,0);
...
ZipArchive::FileNotFoundError::FileNotFoundError(const char* source,const char* sFileName)
	:std::runtime_error(Misc::makeStdErrMsg(source,"%s not found",sFileName)),
	 fileName(sFileName)
	{
	}
...
int ZipArchive::initArchive(void)
	...
	if(directoryPos+Offset(directorySize)>endOfCentralDirPos||endOfCentralDirPos+Offset(sizeof(Misc::UInt32)*3+sizeof(Misc::UInt16)*5+eocdCommentLength)!=archiveSize)
		return -3;
	if(directoryPos+Offset(directorySize)!=endOfCentralDirPos)
		{
		/* Print a warning: */
		Misc::consoleWarning(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"%u extra bytes at end of central directory",(unsigned int)(endOfCentralDirPos-(directoryPos+Offset(directorySize)))).c_str());
		}
	...
	*************************************************/
	...
	do
		...
		if(entryHeader==0x05054b50U||entryHeader==0x06054b50U) // Digital signature entry or end-of-central-directory entry
			...
			char badEntryId[9];
			badEntryId[8]='\0';
			for(int i=7;i>=0;--i,entryHeader>>=4)
				{
				int hexDigit=entryHeader&0xf;
				badEntryId[i]=hexDigit<10?hexDigit+'0':hexDigit-10+'a';
				}
			Misc::consoleWarning(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unrecognized entry ID 0x%s in central directory",badEntryId).c_str());
...
ZipArchive::ZipArchive(const char* archiveFileName)
	...
	switch(initArchive())
		{
		case -1:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"%s is not a valid ZIP archive",archiveFileName);
		
		case -2:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot locate central directory in ZIP archive %s",archiveFileName);
		
		case -3:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid central directory in ZIP archive %s",archiveFileName);
		}
	}
...
ZipArchive::ZipArchive(SeekableFilePtr sArchive)
	...
	switch(initArchive())
		{
		case -1:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Source file is not a valid ZIP archive");
		
		case -2:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to locate central directory in ZIP archive");
		
		case -3:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid central directory in ZIP archive");
		}
	}
...
ZipArchive::FileID ZipArchive::findFile(const char* fileName) const
	...
	if(entry.first!=0&&entry.first->entries[entry.second].filePos!=~Offset(0))
		...
		throw FileNotFoundError(__PRETTY_FUNCTION__,fileName);
	}
...
FilePtr ZipArchive::openFile(const ZipArchive::FileID& fileId)
	...
	if(archive->read<Misc::UInt32>()!=0x04034b50U)
		throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Invalid file header signature"));
...
SeekableFilePtr ZipArchive::openSeekableFile(const ZipArchive::FileID& fileId)
	...
	if(compressionMethod==0)
		...
		archive->read(static_cast<char*>(result->getMemory()),compressedSize);
		}
	else
		{
		/* Read the compressed data: */
		Bytef* compressed=new Bytef[compressedSize];
		archive->read(compressed,compressedSize);
		...
		if(inflateResult!=Z_STREAM_END)
			...
			delete[] compressed;
			delete result;
			throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Cannot decompress due to zlib error %d",inflateResult));
			}
		delete[] compressed;
		int inflateEndResult=inflateEnd(&stream);
		if(inflateEndResult!=Z_OK)
			{
			delete result;
			throw File::OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Caught zlib error %d after decompression",inflateEndResult));
			}
		}
...
DirectoryPtr ZipArchive::openDirectory(const char* directoryName)
	...
	if(entry.first!=0&&entry.first->entries[entry.second].filePos==~Offset(0))
		...
		throw FileNotFoundError(__PRETTY_FUNCTION__,directoryName);
	}
