...
class VariableMemoryFile:public File
	...
	virtual ~VariableMemoryFile(void); // Destroys the buffered file
	...
	/* New methods: */
	void readFile(File& sourceFile); // Replaces this file's contents with the contents of the given file
	...
	FilePtr getReader(void) const; // Returns a File abstraction to read the contents of the file
	};

typedef Misc::Autopointer<VariableMemoryFile> VariableMemoryFilePtr; // Type for pointers to variable-sized in-memory files
