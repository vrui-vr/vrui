...
class TokenSource
	...
	bool isToken(char token) const // Returns true if the most recently read token matches the given single-character token
		{
		return tokenSize==1&&tokenBuffer[0]==token;
		}
	bool isToken(const char* token) const; // Returns true if the most recently read token matches the given string
	bool isCaseToken(char token) const // Returns true if the most recently read token matches the given single-character token up to case
		{
		return tokenSize==1&&tolower(tokenBuffer[0])==tolower(token);
		}
