...
SeekableFile::SeekError::SeekError(const char* source,SeekableFile::Offset sFailedOffset)
	:Error(Misc::makeStdErrMsg(source,"Failed seek to position %ld",(long int)sFailedOffset)),
	 failedOffset(sFailedOffset)
	{
	}

SeekableFile::SeekError::SeekError(const char* source,int libcError,SeekableFile::Offset sFailedOffset)
	:Error(Misc::makeLibcErrMsg(source,libcError,"Failed seek to position %ld",(long int)sFailedOffset)),
	 failedOffset(sFailedOffset)
	{
	}
...
void SeekableFile::setReadPosAbs(SeekableFile::Offset newOffset)
	{
	if(newOffset<0)
		throw SeekError(__PRETTY_FUNCTION__,newOffset);
