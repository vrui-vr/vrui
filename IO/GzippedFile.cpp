...
size_t GzippedFile::readData(File::Byte* buffer,size_t bufferSize)
	...
	if(readSize<0)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Fatal error while reading from file"));
...
GzippedFile::GzippedFile(const char* inputFileName)
	...
	if((inputFile=gzopen(inputFileName,"r"))==0)
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Error while opening gzipped input file %s",inputFileName));
	}
