...
class File:public Threads::RefCounted
	...
	class ReadError:public Error // Exception class to report file reading errors
		...
		/* Constructors and destructors: */
		public:
		ReadError(const char* source,size_t sNumMissingBytes);
		};
	...
	class UngetCharError:public Error // Exception class to report errors while putting characters back into the source
		...
		/* Constructors and destructors: */
		public:
		UngetCharError(const char* source);
		};
	...
	class WriteError:public Error // Exception class to report file writing errors
		...
		/* Constructors and destructors: */
		public:
		WriteError(const char* source,size_t sNumMissingBytes);
		};
	...
	void ungetChar(int character) // Puts the given character back into the read buffer; throws exception if there is no room
		...
		if(readPtr==readBuffer)
			throw UngetCharError(__PRETTY_FUNCTION__);
	...
	size_t readUpTo(void* buffer,size_t bufferSize) // Reads up to the given amount of data into the provided buffer; returns amount of data read; returns zero when called at end-of-file
		...
		if(readPtr==readDataEnd)
			{
			/* Check if the read buffer can be bypassed: */
			if(canReadThrough)
				{
				/* Read directly into the provided buffer: */
				return readData(static_cast<Byte*>(buffer),bufferSize);
				}
	...
	void putBackInBuffer(size_t putbackSize) // Puts the tail end of data previously returned by readInBuffer back into the read buffer
		{
		/* Check if there is enough data in the buffer to rewind: */
		if(size_t(readPtr-readBuffer)<putbackSize)
			throw UngetCharError(__PRETTY_FUNCTION__);
	...
	void discard(void) // Discards the current contents of the write buffer
		{
		/* Reset the write buffer: */
		writePtr=writeBuffer;
		}
	...
	template <class FileDataParam,class DataParam>
	void read(DataParam* data,size_t numItems) // Reads array of values with simple type conversion
		{
		/* Read the data elements individually and convert their types: */
		for(size_t i=0;i<numItems;++i)
			data[i]=read<FileDataParam>();
		}
	template <class DataParam>
	...
	template <class FileDataParam,class DataParam>
	void write(const DataParam* data,size_t numItems) // Writes array of values with simple type conversion
		{
		/* Convert the data elements' types and write them individually: */
		for(size_t i=0;i<numItems;++i)
			write(FileDataParam(data[i]));
		}
	};
