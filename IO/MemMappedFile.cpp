...
void MemMappedFile::openFile(const char* fileName,File::AccessMode accessMode,int flags,int mode)
	...
	if(fd<0)
		throw OpenError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,errno,"Unable to open file %s for %s",fileName,getAccessModeName(accessMode)));
	...
	if(fstat(fd,&statBuffer)<0)
		...
		int error=errno;
		close(fd);
		throw OpenError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,error,"Unable to determine size of file %s",fileName));
		}
	...
	if(memBase==MAP_FAILED)
		...
		int error=errno;
		close(fd);
		throw OpenError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,error,"Unable to memory-map file %s",fileName));
		}
...
MemMappedFile::~MemMappedFile(void)
	...
	if(memBase!=0)
		...
		if(munmap(memBase,memSize)<0)
			{
			/* Can't throw an exception; show an error message to the user instead: */
			Misc::userError(Misc::makeLibcErrMsg(__PRETTY_FUNCTION__,errno,"Unable to unmap memory").c_str());
			}
		}
	}
