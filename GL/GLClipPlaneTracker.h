...
class GLClipPlaneTracker
	...
	GLint* clipPlaneEnableds; // Array of enabled flags for OpenGL clipping planes, in a format ready to be uploaded into a shader program's uniform variables
	...
	const GLint* getClipPlaneEnableds(void) const // Returns the array of clipping plane enabled flags
		{
		return clipPlaneEnableds;
		}
	
	/* Methods to update OpenGL clipping plane state; return true if current clipping plane state changed due to provided values: */
	bool enableClipPlane(int clipPlaneIndex,const Plane& plane,bool enable =true); // Sets the given clipping plane in the current OpenGL context using a homogeneous plane equation, enables clipping plane if flag is true
	bool enableClipPlane(int clipPlaneIndex,const Vector& planeNormal,const Vector& planePoint,bool enable =true); // Sets the given clipping plane in the current OpenGL context using an affine plane equation, enables clipping plane if flag is true
	bool disableClipPlane(int clipPlaneIndex); // Disables the given clipping plane in the current OpenGL context
	
	/* Methods to enable additional clipping planes; return the index of the added clipping plane, or -1 if there were too many clipping planes: */
	int addClipPlane(const Plane& plane);
	int addClipPlane(const Vector& planeNormal,const Vector& planePoint);
	
	/* Helper methods for GLSL shaders: */
	void uploadClipPlaneEnableds(GLint uniformLocation) const; // Uploads the clipping plane enabled flag array into the uniform variable array at the given location
