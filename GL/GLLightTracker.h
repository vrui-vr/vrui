...
class GLLightTracker
	...
	GLint* lightEnableds; // Array of enabled flags for OpenGL light sources, in a format ready to be uploaded into a shader program's uniform variables
	...
	const GLint* getLightEnableds(void) const // Returns the array of light source enabled flags
		{
		return lightEnableds;
		}
	...
	int addLight(const GLLight& light); // Adds a light source to the current lighting state; returns the index of the newly enabled OpenGL light source, or -1 if there were no available light sources
	...
	void uploadLightEnableds(GLint uniformLocation) const; // Uploads the light source enabled flag array into the uniform variable array at the given location
