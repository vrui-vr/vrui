...
GLLightTracker::GLLightTracker(void)
	:version(0),
	 ...
	 maxNumLights(0),lightStates(0),lightEnableds(0),
	...
	/* Initialize the light state arrays: */
	lightStates=new LightState[maxNumLights];
	lightEnableds=new GLint[maxNumLights];
	for(int i=0;i<maxNumLights;++i)
		...
		lightEnableds[i]=i==0?GLint(1):GLint(0);
		}
...
GLLightTracker::~GLLightTracker(void)
	{
	/* Destroy the light state arrays: */
	delete[] lightStates;
	delete[] lightEnableds;
	}
...
bool GLLightTracker::update(void)
	...
	if(lightingEnabled)
		...
		for(int lightIndex=0;lightIndex<maxNumLights;++lightIndex)
			...
			lightEnableds[lightIndex]=enabled?GLint(1):GLint(0);
			}
...
bool GLLightTracker::enableLight(int lightIndex,const GLLight& light)
	...
	lightEnableds[lightIndex]=GLint(1);
	
	return changed;
	}

int GLLightTracker::addLight(const GLLight& light)
	{
	/* Find an unused light source: */
	int lightIndex;
	for(lightIndex=0;lightIndex<maxNumLights&&lightStates[lightIndex].enabled;++lightIndex)
		;
	
	/* Bail out if all light sources are already used: */
	if(lightIndex==maxNumLights)
		return -1;
	
	/* Enable the found light source: */
	lightStates[lightIndex].enabled=true;
	
	/* Check attenuation state: */
	lightStates[lightIndex].attenuated=light.position[3]!=GLLight::Scalar(0)&&(light.constantAttenuation!=GLLight::Scalar(1)||light.linearAttenuation!=GLLight::Scalar(0)||light.quadraticAttenuation!=GLLight::Scalar(0));;
	
	/* Check spotlight state: */
	lightStates[lightIndex].spotLight=light.position[3]!=GLLight::Scalar(0)&&light.spotCutoff<=GLLight::Scalar(90);;
	
	/* Update the lighting state version number: */
	++version;
...
bool GLLightTracker::disableLight(int lightIndex)
	...
	lightEnableds[lightIndex]=GLint(0);
...
void GLLightTracker::uploadLightEnableds(GLint uniformLocation) const
	{
	/* Upload the enabled array: */
	glUniform1ivARB(uniformLocation,maxNumLights,lightEnableds);
	}
