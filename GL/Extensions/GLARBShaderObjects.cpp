...
void glCompileShaderFromString(GLhandleARB shaderObject,const char* shaderSource)
	...
	if(!compileStatus)
		...
		/* Signal an error: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot compile shader due to error %s",compileLogBuffer);
		}
	}
...
void glCompileShaderFromFile(GLhandleARB shaderObject,const char* shaderSourceFileName,IO::File& shaderSourceFile)
	...
	if(!compileStatus)
		...
		/* Signal an error: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot compile shader file %s due to error %s",shaderSourceFileName,compileLogBuffer);
		}
	}
