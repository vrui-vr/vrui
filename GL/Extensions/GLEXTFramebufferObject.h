...
inline void glRenderbufferStorageEXT(GLenum target,GLenum internalformat,const Misc::Size<2>& size)
	{
	glRenderbufferStorageEXT(target,internalformat,size[0],size[1]);
	}
...
void glPrintFramebufferStatusEXT(std::ostream& stream,const char* prettyFunction,const char* tag); // Ditto, with source prefix from evaluating __PRETTY_FUNCTION__
...
void glThrowFramebufferStatusExceptionEXT(const char* prettyFunction,const char* tag); // Ditto, with source prefix from evaluating __PRETTY_FUNCTION__
