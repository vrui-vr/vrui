...
void GLShader::compileVertexShader(const char* shaderSourceFileName)
	{
	if(programObject!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot compile after linking");
...
void GLShader::bindAttribLocation(GLuint index,const char* attributeName)
	{
	if(programObject!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot bind attribute location after linking");
...
void GLShader::linkShader(void)
	{
	if(programObject!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot link shader program multiple times");
	...
	if(!linkStatus)
		...
		/* Signal an error: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot link shader program due to error %s",linkLogBuffer);
		}
	}
...
int GLShader::getAttribLocation(const char* attributeName) const
	{
	if(programObject==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot use shader program before linking");
