...
class GLFrameBuffer
	...
	/* Embedded classes: */
	public:
	typedef Misc::Size<2> Size; // Type for framebuffer sizes
	...
	/* Elements: */
	private:
	Size size; // The width and height of the frame buffer in pixels
	bool haveTextureRectangles; // Flag whether the local OpenGL support texture rectangles
	Size paddedSize; // Actual size of the frame buffer in pixels, after padding if requested or rectangle textures are not supported
	...
	/* Constructors and destructors: */
	public:
	GLFrameBuffer(const Size& sSize,bool pad =false); // Creates a frame buffer of the given size with no attachments; pads size to power of two if flag is true
	private:
	...
	const Size& getSize(void) const // Returns the width and height of the frame buffer
		{
		return size;
		}
	unsigned int getSize(int dimension) const // Ditto
		{
