...
void NaturalPointClient::readRigidBody(PacketBuffer& packet,NaturalPointClient::RigidBody& rigidBody,bool readValidFlag) const
	...
	for(int j=0;j<numBodyMarkers;++j,++mIt)
		...
		if(mIt==rigidBody.markers.end())
			...
			rigidBody.markers.push_back(Point::origin);
		...
		case NAT_FRAMEOFDATA:
			...
			for(int markerSet=0;markerSet<numMarkerSets;++markerSet,++msIt)
				...
				for(int i=0;i<numMarkers;++i,++mIt)
					...
					if(mIt==msIt->markers.end())
						{
						/* Extend the marker array: */
						msIt->markers.push_back(Point::origin);
			...
			for(int i=0;i<numOtherMarkers;++i,++omIt)
				...
				if(omIt==frame.otherMarkers.end())
					{
					/* Extend the marker array: */
					frame.otherMarkers.push_back(Point::origin);
...
NaturalPointClient::NaturalPointClient(const char* serverHostName,int commandPort,const char* dataMulticastGroup,int dataPort)
	...
	if(dataSocketFd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create data socket");
	...
	if(bind(dataSocketFd,(struct sockaddr*)&dataSocketAddress,sizeof(struct sockaddr_in))==-1)
		{
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot bind data socket to port %d",dataPort);
		}
	...
	if(dataEntry==0)
		{
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot resolve data multicast group %s",dataMulticastGroup);
		}
	...
	if(serverEntry==0)
		{
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot resolve server %s",serverHostName);
		}
	...
	if(connect(dataSocketFd,(const struct sockaddr*)&dataServerAddress,sizeof(struct sockaddr_in))<0)
		{
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect data socket to multicast address %s",dataMulticastGroup);
		}
	...
	if(getsockname(dataSocketFd,(struct sockaddr*)&dataLocalAddress,&dataLocalAddressLen)<0)
		{
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query data socket's local address");
		}
	...
	if(setsockopt(dataSocketFd,SOL_SOCKET,SO_BROADCAST,&broadcastFlag,sizeof(int))<0)
		...
		int myerrno=errno;
		close(dataSocketFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Cannot enable broadcast");
		}
	...
	if(setsockopt(dataSocketFd,IPPROTO_IP,IP_ADD_MEMBERSHIP,&addGroupRequest,sizeof(struct ip_mreq))<0)
		...
		int myerrno=errno;
		close(dataSocketFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Cannot join multicast group");
		}
	...
	if(numPingTries>=5)
		...
		close(dataSocketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to server %s",serverHostName);
		}
	}
