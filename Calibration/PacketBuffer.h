...
class PacketBuffer
	...
	class ReadError:public std::runtime_error // Exception class to report file reading errors
		...
		ReadError(const char* source,size_t numBytes,size_t numBytesRead)
			:std::runtime_error(Misc::makeStdErrMsg(source,"Error reading %u bytes from packet, read %u bytes instead",numBytes,numBytesRead))
			{
			}
		};
	...
	DataParam read(void) // Reads single value
		...
		if(readPtr+sizeof(DataParam)>dataEnd)
			throw ReadError(__PRETTY_FUNCTION__,sizeof(DataParam),dataEnd-readPtr);
	...
	size_t read(DataParam* data,size_t numItems) // Reads array of values
		...
		if(readPtr+sizeof(DataParam)*numItems>dataEnd)
			throw ReadError(__PRETTY_FUNCTION__,sizeof(DataParam)*numItems,dataEnd-readPtr);
