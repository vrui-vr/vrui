...
AlignTrackingMarkers::AlignTrackingMarkers(int& argc,char**& argv)
	...
	if((fileName==0||bodyName==0)&&(naturalPointServerName==0||naturalPointRigidBodyId==-1))
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No file name and rigid body name or NaturalPoint server name and rigid body ID provided");
		}
	...
	if(fileName!=0&&bodyName!=0)
		...
		if(extPtr!=0&&strcasecmp(extPtr,".rdef")==0)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Marker file %s has unrecognized extension",fileName);
		}
	else
		{
