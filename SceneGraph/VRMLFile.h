...
class VRMLFile:public IO::TokenSource
	...
	class ParseError:public std::runtime_error // Exception class to signal errors while parsing a VRML file
		...
		ParseError(const VRMLFile& vrmlFile,const char* error);
		};
	...
	std::vector<NodeMap> nodeMapStack; // Stack of maps of named nodes
	...
	void parseProto(void); // Parses a prototype definition
	void parseExternProto(void); // Parses an external prototype definition
	...
	~VRMLFile(void); // Closes the VRML file
	...
	/* Main method: */
	void parse(GroupNode& root); // Adds top-level nodes from the VRML file to the given group node
	
	/* Post-parsing query methods: */
	NodePointer getNode(const std::string& nodeName); // Returns a pointer to a node that was named in the VRML file; returns 0 if the name is not defined
