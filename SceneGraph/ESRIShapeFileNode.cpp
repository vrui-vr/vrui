...
void readPointArray(IO::File& shapeFile,int numPoints,bool readZ,bool readM,const MapProjection* projection,CoordinateNode& coord)
	...
	for(int i=0;i<numPoints;++i)
		{
		/* Read a single point: */
		shapeFile.read(ps[i].getComponents(),2);
		ps[i][2]=0.0;
		}
...
/******************************************
Static elements of class ESRIShapeFileNode:
******************************************/

const char* ESRIShapeFileNode::className="ESRIShapeFile";
...
void ESRIShapeFileNode::update(void)
	...
	if(!fromBinary)
		...
		if(!url.getValues().empty())
			...
			Misc::SelfDestructPointer<MapProjection> projection;
			...
			if(applyProjection)
				...
				try
					...
					projection.setTarget(parseProjectionFile(prjFile));
					}
				catch(const std::runtime_error& err)
					{
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot read projection file %s due to exception %s",prjFileName.c_str(),err.what());
					}
				catch(int errorCode)
					{
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed projection file %s",prjFileName.c_str());
					}
				}
			...
			****************************/
			...
			if(shapeFile->read<int>()!=9994)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid magic number in file %s",shapeFileName.c_str());
			...
			if(shapeFile->read<int>()!=1000)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported version number in file %s",shapeFileName.c_str());
			...
			shapeFile->read(shapeBBox.min.getComponents(),2);
			shapeFile->read(shapeBBox.max.getComponents(),2);
			...
			while(filePos<fileSize)
				...
				switch(recordShapeType)
					...
					case MULTIPOINTM:
						...
						isPolyline=false;
						readPointArray(*shapeFile,recordNumPoints,recordShapeType==MULTIPOINTZ,readM,projection.getTarget(),*pointsCoord);
					...
					case POLYLINEM:
						...
						isPolyline=true;
						readPointArray(*shapeFile,recordNumPoints,recordShapeType==POLYLINEZ,readM,projection.getTarget(),*polylinesCoord);
					...
					case POLYGONM:
						...
						/* Read the points and add them to the polyline set: */
						isPolyline=true;
						readPointArray(*shapeFile,recordNumPoints,recordShapeType==POLYGONZ,readM,projection.getTarget(),*polylinesCoord);
					...
					case MULTIPATCH:
						...
						/* Read the points and add them to the polyline set: */
						isPolyline=true;
						readPointArray(*shapeFile,recordNumPoints,true,readM,projection.getTarget(),*polylinesCoord);
				...
				if(filePos!=shapeFile->getReadPos())
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Record with invalid size %u in file %s",(unsigned int)recordSize,shapeFileName.c_str());
...
void ESRIShapeFileNode::read(SceneGraphReader& reader)
	{
	/* Call the base class method: */
	GroupNode::read(reader);
	
	/* No need to read anything because once loaded it's just a group node */
...
void ESRIShapeFileNode::write(SceneGraphWriter& writer) const
	{
	/* Call the base class method: */
	GroupNode::write(writer);
	
	/* Nothing to do; it's just a group node at this point */
	}
