...
/**************************************************************
Static elements of class GeodeticToCartesianPointTransformNode:
**************************************************************/

const char* GeodeticToCartesianPointTransformNode::className="GeodeticToCartesianPointTransform";
...
void GeodeticToCartesianPointTransformNode::read(SceneGraphReader& reader)
	{
	/* Read all fields: */
	reader.readSFNode(referenceEllipsoid);
	reader.readField(longitude);
	reader.readField(latitude);
	reader.readField(elevation);
	reader.readField(degrees);
	reader.readField(colatitude);
	reader.readField(elevationScale);
	reader.readField(moveToOrigin);
	reader.readField(originPoint);
	}

void GeodeticToCartesianPointTransformNode::write(SceneGraphWriter& writer) const
	{
	/* Write all fields: */
	writer.writeSFNode(referenceEllipsoid);
	writer.writeField(longitude);
	writer.writeField(latitude);
	writer.writeField(elevation);
	writer.writeField(degrees);
	writer.writeField(colatitude);
	writer.writeField(elevationScale);
	writer.writeField(moveToOrigin);
	writer.writeField(originPoint);
	}
...
PointTransformNode::TBox GeodeticToCartesianPointTransformNode::calcBoundingBox(const std::vector<Point>& points,const std::vector<int>& pointIndices) const
	{
	...
	for(std::vector<int>::const_iterator piIt=pointIndices.begin();piIt!=pointIndices.end();++piIt)
		if(*piIt>=0)
			result.addPoint(transformPoint(TPoint(points[*piIt])));
