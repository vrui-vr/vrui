...
Doom3MD5Anim::Doom3MD5Anim(Doom3FileManager& fileManager,const char* animFileName)
	...
	if(!source.isString("MD5Version"))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s is not a valid MD5 animation file",animFileName);
	int md5Version=source.readInteger();
	if(md5Version!=10)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot parse MD5 animation files of version %d",md5Version);
	...
	if(!source.isString("hierarchy")||source.readChar()!='{')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s does not contain a joint hierarchy",animFileName);
	for(int jointIndex=0;jointIndex<numJoints;++jointIndex)
		{
		if(source.peekc()=='}')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Short joint hierarchy at %s",source.where().c_str());
	...
	if(source.readChar()!='}') // Oops, additional joints in list
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Long joint hierarchy at %s",source.where().c_str());
	
	/* Skip the bounding box information for now: */
	if(!source.isString("bounds")||source.readChar()!='{')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s does not contain a bounding box list",animFileName);
	for(int frameIndex=0;frameIndex<numFrames;++frameIndex)
		...
		for(int i=0;i<2;++i)
			{
			if(source.readChar()!='(')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed bounding box corner at %s",source.where().c_str());
			for(int j=0;j<3;++j)
	...
	if(source.readChar()!='}') // Oops, additional bounding boxes in list
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Long bounding box list at %s",source.where().c_str());
	
	/* Read the animation's base transformations: */
	if(!source.isString("baseframe")||source.readChar()!='{')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s does not contain a base frame",animFileName);
	for(int jointIndex=0;jointIndex<numJoints;++jointIndex)
		{
		if(source.peekc()=='}')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Short base frame at %s",source.where().c_str());
		Joint& j=joints[jointIndex];
		...
		if(source.readChar()!='(')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed joint orientation for joint index %d at %s",jointIndex,source.where().c_str());
		for(int i=0;i<3;++i)
	...
	if(source.readChar()!='}') // Oops, additional joint in list
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Long base frame at %s",source.where().c_str());
	...
	for(int frameIndex=0;frameIndex<numFrames;++frameIndex)
		...
		if(!source.isString("frame"))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed animation frame at %s",source.where().c_str());
		if(source.readInteger()!=frameIndex)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching frame index at %s",source.where().c_str());
		if(source.readChar()!='{')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed animation frame at %s",source.where().c_str());
