...
inline void checkImageMapSyntax(Doom3ValueSource& source,char expectedCharacter)
	...
	if(source.readChar()!=expectedCharacter)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed image map at %s",source.where().c_str());
	}
...
Doom3MaterialManager::Expression* Doom3MaterialManager::parseTerm(Doom3ValueSource& source)
	...
	if(source.peekc()=='(')
		...
		if(source.readChar()!=')')
			{
			delete result;
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing closing parenthesis at %s",source.where().c_str());
			}
		...
		if(equal(term,"time"))
			...
			if(parmIndex<0||parmIndex>=12)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown variable %s at %s",term.c_str(),source.where().c_str());
			...
			#if 0
			if(table==0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown table %s at %s",term.c_str(),source.where().c_str());
			#else
			// DEBUGGING
			...
			if(source.readChar()!='[')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed table expression at %s",source.where().c_str());
	...
	if(dataItem->haveShaders)
		...
		for(int i=0;i<DataItem::numShaders;++i)
			...
			if(!linkStatus)
				...
				/* Signal an error: */
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Error \"%s\" while linking shader",linkLogBuffer);
				}
			...
			if(i==0)
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching tangent attribute indices in shader programs");
...
void Doom3MaterialManager::parseMaterialFile(Doom3FileManager& fileManager,const char* fileName)
	...
	while(!source.eof())
		...
		if(materialName=="particle"||materialName=="skin")
			...
			if(source.readChar()!='{')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed particle/skin/etc. definition at %s",source.where().c_str());
			...
			if(source.readChar()!='(')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed guided material definition at %s",source.where().c_str());
			...
			if(source.readChar()!='{')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed table definition at %s",source.where().c_str());
			...
			while(!source.eof()&&!(source.peekc()=='.'||(source.peekc()>='0'&&source.peekc()<='9')))
				...
				if(equal(keyword,"snap"))
					...
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unrecognized table keyword %s at %s",keyword.c_str(),source.where().c_str());
				}
			...
			if(materialID.isValid()&&!materialTree.getLeafValue(materialID).loaded)
				...
				if(source.readChar()!='{')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed material definition at %s",source.where().c_str());
					...
					if(token=="{")
							...
							if(equal(setting,"if"))
								...
								if(equal(blendType,"blend"))
									...
									for(int blendIndex=0;blendIndex<2;++blendIndex)
										...
										if(equal(blendType,"gl_zero"))
											...
											throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unrecognized blending mode %s at %s",blendType.c_str(),source.where().c_str());
										
										if(blendIndex==0)
											{
											stage.blendSrc=blendMode;
											if(source.readChar()!=',')
												throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed blending function at %s",source.where().c_str());
								...
								for(int i=0;i<2;++i)
									...
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed scroll keyword at %s",source.where().c_str());
									delete parseExpression(source);
									}
								}
								...
								for(int i=0;i<2;++i)
									{
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed translate keyword at %s",source.where().c_str());
									delete parseExpression(source);
									}
								}
								...
								for(int i=0;i<2;++i)
									{
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed scale keyword at %s",source.where().c_str());
									delete parseExpression(source);
									}
								}
								...
								for(int i=0;i<2;++i)
									{
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed centerScale keyword at %s",source.where().c_str());
									delete parseExpression(source);
									}
								}
								...
								for(int i=0;i<2;++i)
									{
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed shear keyword at %s",source.where().c_str());
									delete parseExpression(source);
									}
								}
								...
								for(int i=0;i<4;++i)
									{
									if(i>0&&source.readChar()!=',')
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed color keyword at %s",source.where().c_str());
									Expression* exp=parseExpression(source);
						...
						if(equal(token,"qer_editorimage"))
							...
							for(int i=0;i<2&&source.peekc()=='(';++i)
								...
								if(source.readChar()!=')')
									throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed RGBA color at %s",source.where().c_str());
								}
							}
				...
				if(source.readChar()!='{')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed skipped material definition at %s",source.where().c_str());
