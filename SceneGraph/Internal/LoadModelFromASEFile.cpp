...
Doom3Model* loadModelFromASEFile(Doom3FileManager& fileManager,Doom3MaterialManager& materialManager,const char* aseFileName)
	...
	while(!aseTok.eof())
		...
		if(strcasecmp(tag,"}")==0)
			{
			if(groupStack.empty())
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Extra closing brace in file %s",aseFileName);
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing tag in file %s",aseFileName);
			}
			...
			if(!groupStack.empty())
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Non-global MATERIAL_LIST group in file %s",aseFileName);
			
			/* Enter a material list group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MATERIAL_LIST group in file %s",aseFileName);
			...
			if(groupStack.back()!=MATERIAL_LIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MATERIAL_COUNT outside of MATERIAL_LIST in file %s",aseFileName);
			...
			/* Check if inside MATERIAL_LIST group: */
			if(groupStack.back()!=MATERIAL_LIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MATERIAL group outside of MATERIAL_LIST in file %s",aseFileName);
			
			/* Read the the material index: */
			currentMaterialIndex=atoi(aseTok.readNextToken());
			if(currentMaterialIndex>int(materialNames.size()))
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Material index out of bounds in file %s",aseFileName);
			else if(currentMaterialIndex==int(materialNames.size()))
				materialNames.push_back("");
			
			/* Enter a material group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MATERIAL group in file %s",aseFileName);
			...
			if(groupStack.back()!=MATERIAL)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"SUBMATERIAL group outside of MATERIAL in file %s",aseFileName);
			...
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in SUBMATERIAL group in file %s",aseFileName);
			...
			if(groupStack.back()!=MATERIAL&&groupStack.back()!=SUBMATERIAL)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MAP_DIFFUSE group outside of MATERIAL or SUBMATERIAL in file %s",aseFileName);
			
			/* Enter a diffuse map group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MAP_DIFFUSE group in file %s",aseFileName);
			...
			if(groupStack.back()!=MAP_DIFFUSE)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"BITMAP outside of MAP_DIFFUSE in file %s",aseFileName);
			...
			/* Check if at top level: */
			if(!groupStack.empty())
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Non-global GEOMOBJECT group in file %s",aseFileName);
			
			/* Enter a geometry object group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in GEOMOBJECT group in file %s",aseFileName);
			...
			if(groupStack.back()!=GEOMOBJECT)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"NODE_TM group outside of GEOMOBJECT in file %s",aseFileName);
			
			/* Enter a node transformation group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in NODE_TM group in file %s",aseFileName);
			...
			/* Check if inside GEOMOBJECT group: */
			if(groupStack.back()!=GEOMOBJECT)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH group outside of GEOMOBJECT in file %s",aseFileName);
			
			/* Enter a mesh group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_NUMVERTEX group outside of MESH in file %s",aseFileName);
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_VERTEX_LIST group outside of MESH in file %s",aseFileName);
			
			/* Enter a mesh vertex list group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH_VERTEX_LIST group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH_VERTEX_LIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_VERTEX outside of MESH_VERTEX_LIST in file %s",aseFileName);
			
			/* Read the vertex: */
			int vertexIndex=atoi(aseTok.readNextToken());
			if(vertexIndex<0||vertexIndex>=numMeshVertices)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Vertex index out of range in file %s",aseFileName);
			for(int i=0;i<3;++i)
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_NUMFACES group outside of MESH in file %s",aseFileName);
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_FACE_LIST group outside of MESH in file %s",aseFileName);
			
			/* Enter a mesh face list group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH_FACE_LIST group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH_FACE_LIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_FACE outside of MESH_FACE_LIST in file %s",aseFileName);
			
			/* Read the face: */
			int faceIndex=atoi(aseTok.readNextToken());
			if(faceIndex<0||faceIndex>=numMeshFaces)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Face index out of range in file %s",aseFileName);
			...
			while(true)
				...
				if(label[1]==':')
					...
					if(faceVertexIndex!=face.numVertices)
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Face vertex index out of order in file %s",aseFileName);
			...
			for(int i=0;i<face.numVertices;++i)
				...
				if(aseTok.readNextToken()[2]!=':')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing face edge flag in file %s",aseFileName);
				aseTok.readNextToken();
				}
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_NUMTVERTEX group outside of MESH in file %s",aseFileName);
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_TVERTLIST group outside of MESH in file %s",aseFileName);
			
			/* Enter a mesh texture vertex list group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH_TVERTLIST group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH_TVERTLIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_TVERT outside of MESH_TVERTLIST in file %s",aseFileName);
			...
			if(textureVertexIndex<0||textureVertexIndex>=numMeshTextureVertices)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Texture vertex index out of range in file %s",aseFileName);
			for(int i=0;i<2;++i)
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_NUMTVFACES group outside of MESH in file %s",aseFileName);
			
			/* Read the number of texture faces: */
			int numTextureFaces=atoi(aseTok.readNextToken());
			if(numTextureFaces!=numMeshFaces)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching number of faces and texture faces in file %s",aseFileName);
			}
		else if(strcasecmp(tag,"*MESH_TFACELIST")==0)
			{
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_TFACELIST group outside of MESH in file %s",aseFileName);
			
			/* Enter a mesh texture face list group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH_TFACELIST group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH_TFACELIST)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_TFACE outside of MESH_TFACELIST in file %s",aseFileName);
			...
			if(textureFaceIndex<0||textureFaceIndex>=numMeshFaces)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Texture face index out of range in file %s",aseFileName);
			...
			/* Check if inside MESH group: */
			if(groupStack.back()!=MESH)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_NORMALS group outside of MESH in file %s",aseFileName);
			
			/* Enter a mesh normals group: */
			if(strcmp(aseTok.readNextToken(),"{")!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in MESH_NORMALS group in file %s",aseFileName);
			...
			if(groupStack.back()!=MESH_NORMALS)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_FACENORMAL outside of MESH_NORMALS in file %s",aseFileName);
			...
			/* Check if inside MESH_NORMALS group: */
			if(groupStack.back()!=MESH_NORMALS)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_VERTEXNORMAL outside of MESH_NORMALS in file %s",aseFileName);
			
			if(currentNormalFace==-1)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MESH_VERTEXNORMAL without current face in file %s",aseFileName);
			MeshFace& face=meshFaces[currentNormalFace];
			
			/* Read the vertex index: */
			int vertexIndex=atoi(aseTok.readNextToken());
			if(vertexIndex<0||vertexIndex>=numMeshVertices)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Vertex index out of range in MESH_VERTEXNORMAL in file %s",aseFileName);
			...
			for(faceVertexIndex=0;faceVertexIndex<face.numVertices&&meshFaceVertexIndices[face.firstVertex+faceVertexIndex]!=vertexIndex;++faceVertexIndex)
				;
			if(faceVertexIndex>=face.numVertices)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Vertex not found in current face in MESH_VERTEXNORMAL in file %s",aseFileName);
			if(faceVertexIndex!=face.numVertexNormals)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Face vertex index out of order in MESH_VERTEXNORMAL in file %s",aseFileName);
			...
			/* Check if inside GEOMOBJECT group: */
			if(groupStack.back()!=GEOMOBJECT)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"MATERIAL_REF group outside of GEOMOBJECT in file %s",aseFileName);
	...
	if(!groupStack.empty())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing closing brace at end of file in file %s",aseFileName);
