...
class Doom3ValueSource:public IO::ValueSource
	...
	int readInteger(void)
		...
		try
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Number format error at %s:%s",fileName.c_str(),lineNumber);
			}
		return result;
		}
	unsigned int readUnsignedInteger(void)
		...
		try
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Number format error at %s:%u",fileName.c_str(),lineNumber);
			}
		return result;
		}
