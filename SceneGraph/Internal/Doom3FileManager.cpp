...
Doom3FileManagerDirectory::Doom3FileManagerDirectory(Doom3FileManager& sFileManager,const std::string& sPathName)
	...
	if(!directoryIt.isValid())
		throw OpenError(__PRETTY_FUNCTION__,pathName.c_str());
	}
...
Misc::PathType Doom3FileManagerDirectory::getPathType(const char* relativePath) const
	{
	/* Shunt for now: */
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Not implemented");
	}
...
IO::FilePtr Doom3FileManagerDirectory::openFile(const char* fileName,IO::File::AccessMode accessMode) const
	...
	if(accessMode==IO::File::WriteOnly||accessMode==IO::File::ReadWrite)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot write to file %s",fileName);
...
/********************************************
Methods of class Doom3FileManager::ReadError:
********************************************/

Doom3FileManager::ReadError::ReadError(const char* prettyFunction,const char* fileName)
	:std::runtime_error(Misc::makeStdErrMsg(prettyFunction,"File %s not found",fileName))
	{
	}
...
IO::FilePtr Doom3FileManager::getFile(const char* fileName)
	...
	if(!leafId.isValid())
		throw ReadError(__PRETTY_FUNCTION__,fileName);
