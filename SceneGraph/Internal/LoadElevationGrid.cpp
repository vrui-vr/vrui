...
void installImageGrid(ElevationGridNode& node,Images::BaseImage& image)
	...
	switch(image.getScalarType())
		...
		default:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Source image has unsupported pixel type");
		}
...
void loadAGRGrid(ElevationGridNode& node)
	...
	if(!ok)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s is not an ARC/INFO ASCII grid",node.heightUrl.getValue(0).c_str());
...
void loadElevationGrid(ElevationGridNode& node)
	...
	if(node.heightUrlFormat.getNumValues()>=1&&node.heightUrlFormat.getValue(0)=="BIL")
		...
		if(endiannessCode=="LE")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown endianness %s",endiannessCode.c_str());
		...
		if(format=="UINT8")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown raw data type %s",format.c_str());
		}
	else
		{
		...
		if(heightIff==Images::IFF_BIL)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has unknown format",node.heightUrl.getValue(0).c_str());
		}
	}
