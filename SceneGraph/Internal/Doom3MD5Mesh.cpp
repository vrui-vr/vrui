...
Doom3MD5Mesh::Doom3MD5Mesh(Doom3FileManager& fileManager,Doom3MaterialManager& sMaterialManager,const char* meshFileName)
	...
	if(!source.isString("MD5Version"))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s is not a valid MD5 mesh file",meshFileName);
	int md5Version=source.readInteger();
	if(md5Version!=10)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot parse MD5 mesh files of version %d",md5Version);
	...
	if(!source.isString("joints")||source.readChar()!='{')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s does not contain a joint list",meshFileName);
	for(int jointIndex=0;jointIndex<numJoints;++jointIndex)
		{
		if(source.peekc()=='}')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Short joint list at %s",source.where().c_str());
		...
		if(parentIndex==-1)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Parent joint index out of range at %s",source.where().c_str());
		
		/* Read the joint's origin: */
		if(source.readChar()!='(')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed joint position at %s",source.where().c_str());
		Vector translation;
		for(int i=0;i<3;++i)
		...
		if(source.readChar()!='(')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed joint orientation at %s",source.where().c_str());
	...
	if(source.readChar()!='}')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Long joint list at %s",source.where().c_str());
	...
	for(int meshIndex=0;meshIndex<numMeshes;++meshIndex)
		...
		if(!source.isString("mesh")||source.readChar()!='{')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing mesh definition at %s",source.where().c_str());
		...
		if(!source.isString("shader"))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing shader name in mesh definition at %s",source.where().c_str());
		m.shader=materialManager.loadMaterial(source.readString().c_str());
		
		/* Read the mesh's vertices: */
		if(!source.isString("numverts"))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing vertex list in mesh definition at %s",source.where().c_str());
		...
		for(int vertexIndex=0;vertexIndex<m.numVertices;++vertexIndex)
			...
			if(!source.isString("vert"))
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed vertex definition at %s",source.where().c_str());
			if(source.readInteger()!=vertexIndex)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching vertex index at %s",source.where().c_str());
			
			/* Read the vertex' texture coordinates: */
			if(source.readChar()!='(')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed vertex texture coordinates at %s",source.where().c_str());
			for(int i=0;i<2;++i)
		...
		if(!source.isString("numtris"))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing triangle list in mesh definition at %s",source.where().c_str());
		...
		for(int triangleIndex=0;triangleIndex<m.numTriangles;++triangleIndex,tPtr+=3)
			...
			if(!source.isString("tri"))
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed triangle definition at %s",source.where().c_str());
			if(source.readInteger()!=triangleIndex)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching triangle index at %s",source.where().c_str());
		...
		if(!source.isString("numweights"))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing joint weight list in mesh definition at %s",source.where().c_str());
		...
		for(int weightIndex=0;weightIndex<m.numWeights;++weightIndex)
			...
			if(!source.isString("weight"))
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed joint weight definition at %s",source.where().c_str());
			if(source.readInteger()!=weightIndex)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching joint weight index at %s",source.where().c_str());
			...
			if(w.jointIndex<0||w.jointIndex>=numJoints)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Joint index out of range in joint weight definition at %s",source.where().c_str());
			w.weight=Scalar(source.readNumber());
			
			/* Read the joint weight's vertex position: */
			if(source.readChar()!='(')
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed joint weight position at %s",source.where().c_str());
			for(int i=0;i<3;++i)
		...
		for(int vertexIndex=0;vertexIndex<m.numVertices;++vertexIndex,++vPtr)
			...
			if(Math::abs(weightSum-Scalar(1))>Scalar(1.0e-6))
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Vertex weights add up to %f instead of 1.0 at %s",weightSum,source.where().c_str());
			}
		
		if(source.readChar()!='}')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed mesh definition at %s",source.where().c_str());
...
void Doom3MD5Mesh::setJointTransform(const Doom3MD5Mesh::JointID& jointID,const Doom3MD5Mesh::Transform& newTransform,bool cascade)
	...
	if(jointID.mesh!=this)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Given joint ID not part of mesh");
