...
/**************************************
Static elements of class FancyTextNode:
**************************************/

const char* FancyTextNode::className="FancyText";
...
void FancyTextNode::update(void)
	...
	for(MFString::ValueList::const_iterator sIt=string.getValues().begin();sIt!=string.getValues().end();++sIt)
		if(!Misc::UTF8::isValid(sIt->begin(),sIt->end()))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"String is not a valid UTF-8 string");
...
void FancyTextNode::read(SceneGraphReader& reader)
	{
	/* Call the base class method: */
	GeometryNode::read(reader);
	
	/* Read all fields: */
	reader.readField(string);
	reader.readSFNode(fontStyle);
	reader.readField(depth);
	reader.readField(front);
	reader.readField(outline);
	reader.readField(back);
	reader.readField(length);
	reader.readField(maxExtent);
	}

void FancyTextNode::write(SceneGraphWriter& writer) const
	{
	/* Call the base class method: */
	...
	writer.writeSFNode(fontStyle);
	writer.writeField(depth);
	writer.writeField(front);
	writer.writeField(outline);
	writer.writeField(back);
	writer.writeField(length);
	writer.writeField(maxExtent);
	}

bool FancyTextNode::canCollide(void) const
	{
	return false;
	}

int FancyTextNode::getGeometryRequirementMask(void) const
	{
...
void FancyTextNode::testCollision(SphereCollisionQuery& collisionQuery) const
	{
	/* Not implemented yet: */
	}

void FancyTextNode::glRenderAction(int appearanceRequirementsMask,GLRenderState& renderState) const
	{
