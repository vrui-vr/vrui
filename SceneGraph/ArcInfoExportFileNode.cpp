...
/**********************************************
Static elements of class ArcInfoExportFileNode:
**********************************************/

const char* ArcInfoExportFileNode::className="ArcInfoExportFile";
	...
	if(!fromBinary)
		...
		if(!url.getValues().empty())
			...
			if(exportFile.readString()!="EXP"||exportFile.readInteger()!=0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported file format");
...
void ArcInfoExportFileNode::read(SceneGraphReader& reader)
	{
	/* Call the base class method: */
...
void ArcInfoExportFileNode::write(SceneGraphWriter& writer) const
	{
	/* Call the base class method: */
	GroupNode::write(writer);
	
	/* No need to write anything because once loaded it's just a group node */
	}
