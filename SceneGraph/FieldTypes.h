...
class MF:public Field
	...
	unsigned int removeValue(Value value) // Removes all instances of the given value from the list; returns the number of instances removed
		{
	...
	bool removeFirstValue(Value value) // Removes the first instance of the given value from the list; returns true if an instance was removed
		{
		for(typename ValueList::iterator vIt=values.begin();vIt!=values.end();++vIt)
			if(*vIt==value)
				{
				values.erase(vIt);
				return true;
...
/***********************************************************************
Specialized class for multi-value boolean fields, because
std::vector<bool>'s implementation as a bit vector throws a wrench:
***********************************************************************/

template <>
class MF<bool>:public Field
	{
	/* Embedded classes: */
	public:
	typedef bool Value; // Field value type
	typedef std::vector<Value> ValueList; // Type for value lists
	...
	MF(bool sValue) // Creates a single-valued field
		{
		values.push_back(sValue);
		}
	...
	Value getValue(size_t index) const // Returns the given value from the list
		{
		return values[index];
		}
	void setValue(Value newValue) // Sets the field to the single given value
		{
		values.clear();
		values.push_back(newValue);
		}
	void setValue(size_t index,Value newValue) // Sets one value in the list
		{
		values[index]=newValue;
		}
	void clearValues(void) // Removes all values from the list
	...
	void appendValue(Value newValue) // Appends a new value to the end of the list
		{
		values.push_back(newValue);
		}
	void insertValue(size_t index,Value newValue) // Inserts the given value before the current value at the given index
		{
		values.insert(values.begin()+index,newValue);
		}
	};
