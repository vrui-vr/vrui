...
FancyFontStyleNode::Glyph FancyFontStyleNode::GlyphCreator::createGlyph(unsigned int glyphIndex)
	...
	if(ftError!=FT_Err_Ok)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot load glyph %u due to error %d",glyphIndex,ftError);
	
	/* Check if the glyph is an outline glyph: */
	FT_GlyphSlot slot=ftFace->glyph;
	if(slot->format!=FT_GLYPH_FORMAT_OUTLINE)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Glyph %u is not an outline glyph",glyphIndex);
...
const char* FancyFontStyleNode::className="FancyFontStyle";
...
FancyFontStyleNode::FancyFontStyleNode(void)
	...
	if(ftLibraryRefCount==0)
		...
		if(error!=FT_Err_Ok)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot initialize FreeType library due to error code %d",error);
		}
	++ftLibraryRefCount;
	}
	}
...
void FancyFontStyleNode::update(void)
	...
	if(!url.getValues().empty())
		...
		if(family.getValues().empty())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No font family defined");
		else if(family.getValue(0)=="SANS")
			fontFamily=SANS;
		else if(family.getValue(0)=="TYPEWRITER")
			fontFamily=TYPEWRITER;
		else if(family.getValue(0)!="SERIF")
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid font family %s",family.getValue(0).c_str());
		...
		if(style.getValue()=="BOLD")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid font style %s",style.getValue().c_str());
	...
	if(fontFileName.empty()||fontFileName!=newFontFileName)
		...
		if(ftError==FT_Err_Unknown_File_Format)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot load font face from file %s due to unknown file format",newFontFileName.c_str());
		else if(ftError!=FT_Err_Ok)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot load font face from file %s due to error %d",newFontFileName.c_str(),ftError);
	...
	if(ftError!=FT_Err_Ok)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set font size due to error %d",ftError);
	...
	if(justify.getNumValues()>=1)
		...
		if(justify.getValue(0)=="FIRST")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid justification %s",justify.getValue(0).c_str());
		}
	...
	if(justify.getNumValues()>=2)
		...
		if(justify.getValue(1)=="BEGIN")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid justification %s",justify.getValue(1).c_str());
		}
	}

void FancyFontStyleNode::read(SceneGraphReader& reader)
	{
	/* Read all fields: */
	reader.readField(url);
	reader.readField(family);
	reader.readField(style);
	reader.readField(language);
	reader.readField(size);
	reader.readField(spacing);
	reader.readField(justify);
	reader.readField(horizontal);
	reader.readField(leftToRight);
	reader.readField(topToBottom);
	reader.readField(precision);
	}

void FancyFontStyleNode::write(SceneGraphWriter& writer) const
	{
	/* Write all fields: */
	writer.writeField(url);
	writer.writeField(family);
	writer.writeField(style);
	writer.writeField(language);
	writer.writeField(size);
	writer.writeField(spacing);
	writer.writeField(justify);
	writer.writeField(horizontal);
	writer.writeField(leftToRight);
	writer.writeField(topToBottom);
	writer.writeField(precision);
	}
