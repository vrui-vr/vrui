...
/********************************************
Static elements of class ImageProjectionNode:
********************************************/

const char* ImageProjectionNode::className="ImageProjection";
...
void ImageProjectionNode::read(SceneGraphReader& reader)
	{
	/* Read all fields: */
	reader.readSFNode(geometryTransform);
	reader.readSFNode(imageTransform);
	reader.readField(imagePixelTransform);
	reader.readField(imageWidth);
	reader.readField(imageHeight);
	reader.readField(flipVertical);
	}

void ImageProjectionNode::write(SceneGraphWriter& writer) const
	{
	/* Write all fields: */
	writer.writeSFNode(geometryTransform);
	writer.writeSFNode(imageTransform);
	writer.writeField(imagePixelTransform);
	writer.writeField(imageWidth);
	writer.writeField(imageHeight);
	writer.writeField(flipVertical);
	}
