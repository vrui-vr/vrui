...
/************************************
Static elements of class QuadSetNode:
************************************/

const char* QuadSetNode::className="QuadSet";
...
void QuadSetNode::update(void)
	...
	numQuads=coord.getValue()!=0?coord.getValue()->point.getNumValues()/4:0;
	
	/* Update the quad set version number: */
	++version;
	}

void QuadSetNode::read(SceneGraphReader& reader)
	{
	...
	reader.readField(subdivideX);
	reader.readField(subdivideY);
	}

void QuadSetNode::write(SceneGraphWriter& writer) const
	{
	/* Call the base class method: */
	...
	writer.writeField(subdivideX);
	writer.writeField(subdivideY);
	}

bool QuadSetNode::canCollide(void) const
	{
	return false;
	}

int QuadSetNode::getGeometryRequirementMask(void) const
	{
	...
	if(numQuads>0)
		...
		if(pointTransform.getValue()!=0)
			...
			if(subdivideX.getValue()>1||subdivideY.getValue()>1)
				...
				MFPoint::ValueList::const_iterator quadsEnd=points.begin()+(numQuads*4);
				for(MFPoint::ValueList::const_iterator pIt=points.begin();pIt!=quadsEnd;++pIt)
					result.addPoint(pointTransform.getValue()->transformPoint(*pIt));
				}
			}
		else
			{
...
void QuadSetNode::testCollision(SphereCollisionQuery& collisionQuery) const
	{
	/* Not implemented yet: */
	}

void QuadSetNode::glRenderAction(int appearanceRequirementsMask,GLRenderState& renderState) const
	{
