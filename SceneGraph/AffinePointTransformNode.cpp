...
/*************************************************
Static elements of class AffinePointTransformNode:
*************************************************/

const char* AffinePointTransformNode::className="AffinePointTransform";
...
void AffinePointTransformNode::read(SceneGraphReader& reader)
	{
	/* Read all fields: */
	reader.readField(matrix);
	}

void AffinePointTransformNode::write(SceneGraphWriter& writer) const
	{
	/* Write all fields: */
	writer.writeField(matrix);
	}
...
PointTransformNode::TBox AffinePointTransformNode::calcBoundingBox(const std::vector<Point>& points,const std::vector<int>& pointIndices) const
	{
	TBox result=Box::empty;
	...
	for(std::vector<int>::const_iterator piIt=pointIndices.begin();piIt!=pointIndices.end();++piIt)
		if(*piIt>=0)
			result.addPoint(transform.transform(TPoint(points[*piIt])));
