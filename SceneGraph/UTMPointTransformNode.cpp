...
/**********************************************
Static elements of class UTMPointTransformNode:
**********************************************/

const char* UTMPointTransformNode::className="UTMPointTransform";
...
void UTMPointTransformNode::read(SceneGraphReader& reader)
	{
	/* Read all fields: */
	reader.readSFNode(referenceEllipsoid);
	reader.readField(zone);
	reader.readField(scaleFactor);
	reader.readField(falseNorthing);
	reader.readField(falseEasting);
	reader.readField(degrees);
	}

void UTMPointTransformNode::write(SceneGraphWriter& writer) const
	{
	/* Write all fields: */
	writer.writeSFNode(referenceEllipsoid);
	writer.writeField(zone);
	writer.writeField(scaleFactor);
	writer.writeField(falseNorthing);
	writer.writeField(falseEasting);
...
PointTransformNode::TBox UTMPointTransformNode::calcBoundingBox(const std::vector<Point>& points,const std::vector<int>& pointIndices) const
	{
	TBox result=Box::empty;
	
	/* Transform all used points individually: */
	for(std::vector<int>::const_iterator piIt=pointIndices.begin();piIt!=pointIndices.end();++piIt)
		if(*piIt>=0)
			{
			const Point& p=points[*piIt];
			Geometry::UTMProjection<double>::PPoint geodetic=projection.mapToGeodetic(Geometry::UTMProjection<double>::PPoint(TScalar(p[0]),TScalar(p[1])));
			if(degrees.getValue())
				{
				/* Convert geodetic coordinates from radians to degrees: */
				geodetic[0]*=TScalar(180)/Math::Constants<TScalar>::pi;
			...
			result.addPoint(TPoint(geodetic[0],geodetic[1],TScalar(p[2])));
			}
