			...
			switch(packetKind)
				...
				case 2:
					...
					if(numPacketChannels>numChannels)
						...
						pipe.read(channelPacketBuffer,numChannels);
						...
						{
						/* Read all channels: */
						pipe.read(channelPacketBuffer,numPacketChannels);
						}
...
ViconTarsus::ViconTarsus(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(pipe.read<int>()!=1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to tracking server at %s",pipe.getPeerHostName().c_str());
	if(pipe.read<int>()!=1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to tracking server at %s",pipe.getPeerHostName().c_str());
	...
	for(int bodyIndex=0;bodyIndex<getNumTrackers();++bodyIndex)
		...
		for(int i=0;i<3;++i)
			if(trackerChannelIndices[bodyIndex*6+i]==-1)
				valid=false;
		if(!valid)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Insufficient data to track body %s",trackedBodies[bodyIndex].c_str());
