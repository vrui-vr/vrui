...
RazerHydra::RazerHydra(unsigned int index)
	...
	if(!device.isValid())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Fewer than %u Razer Hydra devices detected on USB bus",index+1);
...
void RazerHydra::setPositionConversionFactor(RazerHydra::Scalar newPositionConversionFactor)
	...
	if(streaming)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot change position conversion factor while streaming");
...
void RazerHydra::pollSensors(RazerHydra::SensorState states[2])
	...
	/* Throw an exception if the device is currently streaming: */
	if(streaming)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot poll sensors while streaming");
...
void RazerHydra::stopStreaming(void)
	...
	if(streaming)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Device is not streaming");
