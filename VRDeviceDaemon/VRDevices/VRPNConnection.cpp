...
VRPNConnection::VRPNConnection(const char* serverName,int serverPort)
	...
	*********************************************************************/
	...
	if(!connected)
		...
		fflush(stdout);
		#endif
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to VRPN server on host %s, port %d",serverName,serverPort);
		}
	...
	*********************************************************************/
	...
	if(dotPtr!=0)
		...
		if(strncmp(message,versionMessage,dotPtr-message)!=0)
			...
			fflush(stdout);
			#endif
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VRPN server on host %s, port %d sent incompatible version message %s",serverName,serverPort,message);
			}
		}
	else
		{
