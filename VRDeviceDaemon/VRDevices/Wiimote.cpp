...
void Wiimote::writePacket(unsigned char packet[],size_t packetSize)
	...
	if(write(dataSocket,packet,packetSize)!=ssize_t(packetSize))
		{
		int error=errno;
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Error %s",strerror(error));
		}
	}
...
void* Wiimote::receiverThreadMethod(void)
	...
	while(true)
		...
		if(packetSize>=2&&packet[0]==0xa1)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to Wiimote");
			}
		}
...
Wiimote::Wiimote(const char* deviceName,Misc::ConfigurationFile& configFile)
	...
	try
		...
		if(btDeviceId<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot get handle to local bluetooth device");
		btSocket=hci_open_dev(btDeviceId);
		if(btSocket<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot connect to local bluetooth device");
		...
		if(isAddress)
			...
			if(hci_read_remote_name(btSocket,&deviceAddress,sizeof(remoteDeviceName),remoteDeviceName,0)>=0)
				...
				if(strncmp(remoteDeviceName,"Nintendo RVL-CNT-01",19)!=0)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Device at address %s is not a Wiimote",deviceName);
				}
			else
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Device at address %s not found",deviceName);
			}
		else
			{
			inquiry_info* iis=0;
			try
				...
				if(numResponses<0)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot scan for nearby bluetooth devices");
				...
				if(!deviceFound)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No Wiimote device found");
	...
	if(controlSocket<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot open control socket to device %s",deviceName);
	...
	if(connect(controlSocket,reinterpret_cast<struct sockaddr*>(&controlSocketAddress),sizeof(controlSocketAddress))<0)
		...
		int myerrno=errno;
		close(controlSocket);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Cannot connect control socket to device %s",deviceName);
		}
	...
	if(dataSocket<0)
		...
		int myerrno=errno;
		close(controlSocket);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Cannot open data socket to device %s",deviceName);
		}
	...
	if(connect(dataSocket,reinterpret_cast<struct sockaddr*>(&dataSocketAddress),sizeof(dataSocketAddress))<0)
		...
		close(controlSocket);
		close(dataSocket);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Cannot connect data socket to device %s",deviceName);
		}
	...
	try
		...
		if(calibration!=0)
			...
			close(controlSocket);
			close(dataSocket);
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot read calibration data from device %s",deviceName);
			}
		}
