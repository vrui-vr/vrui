...
bool PolhemusFastrak::processRecord(void)
	...
	float trans[3];
	devicePort.read(trans,3);
	typedef PositionOrientation::Vector Vector;
	Vector v(trans);
	float rotAngles[3];
	devicePort.read(rotAngles,3);
...
PolhemusFastrak::PolhemusFastrak(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(!readStatusReply())
		...
		writeCommand(devicePort,'S');
		if(!readStatusReply())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Device not responding");
		}
	...
	if(hemisphere=="+X")
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unrecognized hemisphere value %s",hemisphere.c_str());
