...
HIDDevice::HIDDevice(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(deviceFd<0)
		...
		if(deviceFileName!="")
			...
			if(deviceFd<0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open device file \"%s\"",deviceFileName.c_str());
			}
		}
	...
	if(deviceFd<0)
		...
		if(deviceVendorProductId!="")
			...
			if(*colonPtr!=':'||*endPtr!='\0'||vendorId<0||productId<0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed vendorId:productId string \"%s\"",deviceVendorProductId.c_str());
			#ifdef VERBOSE
			...
			if(deviceFd<0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No device with vendorId:productId %04x:%04x found",vendorId,productId);
			}
		}
	...
	if(deviceFd<0)
		...
		if(deviceName!="")
			...
			if(deviceFd<0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No device with name \"%s\" found",deviceName.c_str());
			}
		}
	
	/* Bail out if no device was found: */
	if(deviceFd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No device specified");
	...
	if(ioctl(deviceFd,EVIOCGBIT(0,sizeof(featureTypeBits)),featureTypeBits)<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query device feature types");
	...
	if(featureTypeBits[EV_KEY/8]&(1<<(EV_KEY%8)))
		...
		if(ioctl(deviceFd,EVIOCGBIT(EV_KEY,sizeof(keyBits)),keyBits)<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query device key features");
	...
	if(featureTypeBits[EV_ABS/8]&(1<<(EV_ABS%8)))
		...
		if(ioctl(deviceFd,EVIOCGBIT(EV_ABS,sizeof(absAxisBits)),absAxisBits)<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query device absolute axis features");
	...
	if(featureTypeBits[EV_REL/8]&(1<<(EV_REL%8)))
		...
		if(ioctl(deviceFd,EVIOCGBIT(EV_REL,sizeof(relAxisBits)),relAxisBits)<0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query device relative axis features");
		...
		for(int i=0;i<=ABS_MAX;++i)
			if(absAxisMap[i]>=0)
				...
				if(ioctl(deviceFd,EVIOCGABS(i),&absAxisConf)<0)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query device absolute axis configuration");
