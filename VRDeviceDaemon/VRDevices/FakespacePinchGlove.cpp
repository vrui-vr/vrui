...
FakespacePinchGlove::FakespacePinchGlove(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(devicePort.waitForData(Misc::Time(0.5)))
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Pinch glove device is not responding");
	...
	if(replyBuffer[0]!='0')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot disable time stamps on pinch glove device");
	...
	readReply(256,replyBuffer);
	if(replyBuffer[0]!='1')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set interface version on pinch glove device");
