...
struct SensorRange // Structure to get and set the maximum value ranges of the Oculus Rift's three sensors
	...
	void get(USB::Device& device) // Reads sensor ranges from given USB device
		...
		if(readResult!=size_t(pktBuffer.getSize()))
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Received %u bytes instead of %u",readResult,size_t(pktBuffer.getSize()));
...
struct SensorData // Structure to retrieve raw sensor measurements from the Oculus Rift
	...
	void get(USB::Device& device) // Reads next sensor data packet from given USB device
		...
		if(readSize==62U)
			...
			Vrui::TimePoint now;
			arrivalTimeStamp=Vrui::VRDeviceState::TimeStamp(now.tv_sec*1000000L+(now.tv_nsec+500L)/1000L);
...
OculusRift::OculusRift(VRDevice::Factory* sFactory,VRDeviceManager* sDeviceManager,Misc::ConfigurationFile& configFile)
	...
	if(configFile.hasTag("./deviceSerialNumber"))
		...
		if(!oculus.isValid())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Oculus Rift device with serial number %s not found",serialNumber.c_str());
		...
		switch(oculus.getVendorProductId().productId)
			...
			default:
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Oculus Rift device with serial number %s has unsupported product ID %u",serialNumber.c_str(),oculus.getVendorProductId().productId);
			}
		}
	else
		{
		...
		if(!oculus.isValid())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Oculus Rift device %d not found",oculusIndex);
		...
		switch(oculus.getVendorProductId().productId)
			...
			default:
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Oculus Rift device %d has unsupported product ID %u",oculusIndex,oculus.getVendorProductId().productId);
			}
		}
	}
