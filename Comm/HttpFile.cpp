...
size_t parseChunkHeader(Comm::Pipe& pipe)
	...
	while(digit!='\r')
		digit=pipe.getChar();
	if(pipe.getChar()!='\n')
		throw IO::File::Error("Comm::HttpFile::readData: Malformed HTTP chunk header");
	...
	if(chunked)
		...
		if(unreadSize==0)
			...
			if(pipe->getChar()!='\r'||pipe->getChar()!='\n')
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Malformed HTTP chunk footer"));
...
void HttpFile::init(const HttpFile::URLParts& urlParts,const Misc::Time* timeout)
	...
	if(timeout!=0)
		...
		if(!pipe->waitForData(*timeout))
			throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Timeout while waiting for reply from server %s on port %d",urlParts.serverName.c_str(),urlParts.portNumber));
		}
	...
	if(!reply.isLiteral("HTTP")||!reply.isLiteral('/'))
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Malformed HTTP reply from server %s on port %d",urlParts.serverName.c_str(),urlParts.portNumber));
	reply.skipString();
	...
	if(statusCode!=200)
		...
		if(statusCode==301) // HTTP 301 Moved Permanently
			...
			while(!reply.eof()&&reply.peekc()!='\r')
				...
				if(reply.isString("Location:"))
					...
					throw HttpRedirect(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Resource \"%s\" on server %s on port %d permanently moved to location \"%s\"",urlParts.resourcePath.c_str(),urlParts.serverName.c_str(),urlParts.portNumber,redirectUrl.c_str()),error,redirectUrl);
					}
		...
		throw HttpError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"HTTP error %d (%s) while opening resource \"%s\" on server %s on port %d",statusCode,error.c_str(),urlParts.resourcePath.c_str(),urlParts.serverName.c_str(),urlParts.portNumber),statusCode,error);
		}
...
HttpFile::HttpFile(const char* fileUrl,const Misc::Time* timeout)
	...
	if(urlParts.https)
		...
		#else
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"HTTPS connections not supported due to lack of OpenSSL library");
		#endif
		}
	else
		{
...
HttpFile::~HttpFile(void)
	...
	try
		...
		if(chunked)
			...
			while(pipe->getChar()!='\r')
				...
				while(pipe->getChar()!='\r')
					;
				if(pipe->getChar()!='\n')
					throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Malformed HTTP body trailer"));
				}
			if(pipe->getChar()!='\n')
				throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Malformed HTTP body trailer"));
			}
		...
		Misc::userError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Caught exception \"%s\" while closing file",err.what()).c_str());
		}
