...
TCPSocket::TCPSocket(int portId,int backlog)
	...
	if(bind(socketFd,(struct sockaddr*)&socketAddress,sizeof(IPv4SocketAddress))<0)
		...
		int myerrno=errno;
		close(socketFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Unable to bind socket to port %d",portId);
		}
	...
	if(listen(socketFd,backlog)<0)
		...
		int myerrno=errno;
		close(socketFd);
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Unable to start listening on socket");
		}
	}
...
IPv4SocketAddress TCPSocket::getAddress(void) const
	...
	if(getsockname(socketFd,(struct sockaddr*)&socketAddress,&socketAddressLen)<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Unable to query local socket address");
...
TCPSocket& TCPSocket::connect(const std::string& hostname,int portId)
	...
	if(bind(socketFd,(struct sockaddr*)&socketAddress,sizeof(IPv4SocketAddress))<0)
		...
		close(socketFd);
		socketFd=-1;
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Unable to bind socket");
		}
	...
	if(::connect(socketFd,(const struct sockaddr*)&remoteAddress,sizeof(IPv4SocketAddress))<0)
		...
		close(socketFd);
		socketFd=-1;
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Unable to connect to host %s on port %d",hostname.c_str(),portId);
		}
...
TCPSocket TCPSocket::accept(void) const
	...
	if(newSocketFd<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Unable to accept connection request");
...
IPv4SocketAddress TCPSocket::getPeerAddress(void) const
	...
	if(getpeername(socketFd,(struct sockaddr*)&peerAddress,&peerAddressLen)<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Unable to query remote socket address");
	if(peerAddressLen<sizeof(IPv4SocketAddress))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Returned address has wrong size; %u bytes instead of %u bytes",(unsigned int)peerAddressLen,(unsigned int)sizeof(IPv4SocketAddress));
...
void TCPSocket::shutdown(bool shutdownRead,bool shutdownWrite)
	...
	if(shutdownRead&&shutdownWrite)
		...
		if(::shutdown(socketFd,SHUT_RDWR)!=0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to shut down read and write");
		}
	else if(shutdownRead)
		{
		if(::shutdown(socketFd,SHUT_RD)!=0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to shut down read");
		}
	else if(shutdownWrite)
		{
		if(::shutdown(socketFd,SHUT_WR)!=0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to shut down write");
		}
	}
...
bool TCPSocket::waitForData(long timeoutSeconds,long timeoutMicroseconds,bool throwException) const
	...
	if(throwException&&!dataWaiting)
		throw TimeOut(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Time-out while waiting for data"));
	return dataWaiting;
	}
...
size_t TCPSocket::read(void* buffer,size_t count)
	...
	if(numBytesRead>0||errno==EAGAIN)
		...
		throw PipeError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Connection terminated by peer"));
		}
	else
		{
		/* Consider this a fatal error: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Fatal error");
		}
