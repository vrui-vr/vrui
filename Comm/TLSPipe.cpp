...
SSL_CTX* TLSContext::acquireContext(void)
	...
	if(theContext.context==0)
		...
		if(sslMethod==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to access TLS method");
		theContext.context=SSL_CTX_new(sslMethod);
		if(theContext.context==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to create SSL context");
...
size_t TLSPipe::readData(IO::File::Byte* buffer,size_t bufferSize)
	...
	if(readResult<=0&&sslError!=SSL_ERROR_ZERO_RETURN)
		...
		if(sslError==SSL_ERROR_SYSCALL)
			...
			throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Socket-level error %d (%s)",error,strerror(error)));
			}
		else
			{
			/* Unknown error; probably a bad thing: */
			throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Protocol-level error %d",sslError));
			}
		}
...
TLSPipe::TLSPipe(const char* hostName,int portId)
	...
	if(tcpPipe==0)
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to create TCP connection object"));
	...
	if(ssl==0)
		{
		BIO_vfree(tcpPipe);
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to create SSL connection object"));
		}
	...
	if(SSL_set_cipher_list(ssl,preferredCiphers)==0)
		{
		SSL_free(ssl);
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to set preferred TLS ciphers"));
		}
	...
	if((sslError=BIO_do_connect(tcpPipe))<=0)
		{
		SSL_free(ssl);
		throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to connect to host %s on port %d due to protocol error %d",hostName,portId,sslError));
		}
	if((sslError=SSL_connect(ssl))<=0)
		...
		SSL_free(ssl);
		if(sslError==SSL_ERROR_SYSCALL)
			throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to establish TLS connection with host %s on port %d due to socket-level error %d (%s)",hostName,portId,myerrno,strerror(myerrno)));
		else
			throw OpenError(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to establish TLS connection with host %s on port %d due to protocol error %d",hostName,portId,sslError));
		}
...
void TLSPipe::shutdown(bool read,bool write)
	...
	if(ret<0)
		...
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Protocol error %d",sslError));
		}
	}
...
int TLSPipe::getPortId(void) const
	...
	if(getsockname(fd,reinterpret_cast<struct sockaddr*>(&socketAddress),&socketAddressLen)<0)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to query socket address"));
	...
	if(niResult!=0)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to retrieve port ID due to error %d (%s)",niResult,gai_strerror(niResult)));
...
std::string TLSPipe::getAddress(void) const
	...
	if(niResult!=0)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to retrieve address due to error %d (%s)",niResult,gai_strerror(niResult)));
...
std::string TLSPipe::getHostName(void) const
	...
	if(niResult!=0)
		throw Error(Misc::makeStdErrMsg(__PRETTY_FUNCTION__,"Unable to retrieve host name due to error %d (%s)",niResult,gai_strerror(niResult)));
