...
Opener* Opener::getOpener(void)
	...
	if(result==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Active IO::Opener is not a Comm::Opener");
...
NetPipePtr Opener::openTLSPipe(const char* hostName,int portId)
	...
	#else
	throw Misc::makeStdErr(__PRETTY_FUNCTION__,"OpenSSL library does not exist");
	#endif
	}
