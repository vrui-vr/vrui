...
std::vector<IPSocketAddress> IPSocketAddress::lookupHost(const char* hostName,unsigned short port)
	...
	if(aiResult!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to resolve address %s:%u due to error %d (%s)",hostName,(unsigned int)port,aiResult,gai_strerror(aiResult));
...
unsigned short IPSocketAddress::getPort(void) const
	...
	switch(family)
		...
		default:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid address family");
		}
	}
...
std::string IPSocketAddress::getAddress(void) const
	...
	if(niResult!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to retrieve address due to error %d (%s)",niResult,gai_strerror(niResult));
...
std::string IPSocketAddress::getHostName(void) const
	...
	if(niResult!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unable to retrieve host name due to error %d (%s)",niResult,gai_strerror(niResult));
