...
ifdef DEVEL # Configuration to build Vrui in-tree for development
  INSTALLDIR=$(VRUI_PACKAGEROOT)
  HEADERINSTALLDIR = $(INSTALLDIR)
  LIBINSTALLDIR_DEBUG = $(INSTALLDIR)/$(LIBEXT)/debug
  LIBINSTALLDIR_RELEASE = $(INSTALLDIR)/$(LIBEXT)
  EXECUTABLEINSTALLDIR_DEBUG = $(INSTALLDIR)/$(BINEXT)/debug
  EXECUTABLEINSTALLDIR_RELEASE = $(INSTALLDIR)/$(BINEXT)
  ETCINSTALLDIR = $(VRUI_ETCDIR)
  SHAREINSTALLDIR = $(VRUI_SHAREDIR)
  PKGCONFIGINSTALLDIR = $(VRUI_SCRIPTDIR)
  DOCINSTALLDIR = $(VRUI_DOCDIR)
  UDEVRULEDIR = /usr/lib/udev/rules.d
  INSTALLROOT = $(VRUI_PACKAGEROOT)
  MAKEINSTALLDIR = $(VRUI_MAKEDIR)
else
...
ALL = $(VRUI_MAKEDIR)/StripPackages $(LIBRARIES) $(EXECUTABLES) $(PLUGINS) $(RESOURCES)
...
ifdef SYSTEMINSTALL
	...
	@$(call CONFIG_SETMAKEVAR,$(VRUI_MAKEDIR)/makefile,VRUI_MAKEDIR,$(MAKEINSTALLDIR))
...
SCENEGRAPH_PACKAGES = MYALSUPPORT MYSOUND MYIMAGES MYGLGEOMETRY MYGLSUPPORT MYGLWRAPPERS MYGEOMETRY MYMATH MYIO MYTHREADS MYMISC GL
ifneq ($(SCENEGRAPH_USE_FANCYTEXT),0)
...
ifneq ($(SYSTEM_HAVE_VULKAN),0)
	...
	@cp VRCompositingServer/Config.h VRCompositingServer/Config.h.temp
	@$(call CONFIG_SETSTRINGVAR,VRCompositingServer/Config.h.temp,VRCOMPOSITOR_SHADERDIR,$(SHAREINSTALLDIR)/spirv)
	@if ! diff VRCompositingServer/Config.h.temp VRCompositingServer/Config.h > /dev/null ; then cp VRCompositingServer/Config.h.temp VRCompositingServer/Config.h ; fi
