...
void WidgetManager::moveWidget(PopupBinding* binding,const Transformation& newTransform)
	{
	/* Check if the binding is a primary: */
	if(binding->parent==0)
		{
		...
		{
		/* Set the binding's widget transformation directly: */
		binding->widgetToWorld=newTransform;
		}
	
	/* Call the widget move callbacks: */
	WidgetMoveCallbackData cbData(this,binding->widgetToWorld,binding->topLevelWidget,true);
	widgetMoveCallbacks.call(&cbData);
	
	/* Recurse into the binding's secondaries: */
	moveSecondaryWidgets(binding,binding->widgetToWorld);
	}
...
void WidgetManager::show(Widget* widget)
	...
	if(binding!=0)
		...
		/* Call the widget show/hide callbacks: */
		WidgetShowHideCallbackData cbData(this,true,binding->topLevelWidget);
		widgetShowHideCallbacks.call(&cbData);
		}
	}
...
void WidgetManager::hide(Widget* widget)
	...
	if(binding!=0)
		...
		/* Call the widget show/hide callbacks: */
		WidgetShowHideCallbackData cbData(this,false,binding->topLevelWidget);
		widgetShowHideCallbacks.call(&cbData);
		}
	}
...
void WidgetManager::setPrimaryWidgetTransformation(Widget* widget,const WidgetManager::Transformation& newWidgetToWorld)
	...
	/* Set the binding's widget transformation if the binding exists and is a top-level: */
	if(bPtr!=0&&bPtr->parent==0)
		moveWidget(bPtr,newWidgetToWorld);
	}
