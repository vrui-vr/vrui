...
class Texture:public Widget,public GLObject
	...
	/* Embedded classes: */
	public:
	typedef Misc::Size<2> Size; // Type for texture sizes
	...
	struct DataItem:public GLObject::DataItem
		...
		Size textureSize; // Current width and height of texture
	...
	/* Elements: */
	private:
	Size size; // Current size of the texture
	unsigned int version; // Version number of image data in texture
	GLfloat resolution[2]; // The horizontal and vertical resolution of the image in pixels per GLMotif length unit; if both are zero, image will fit itself into assigned rectangle
	...
	/* Protected methods: */
	virtual void uploadTexture(GLuint textureObjectId,bool npotdtSupported,const Size& textureSize,GLContextData& contextData) const =0; // Called when an image needs to be uploaded to the texture object of the given (padded) texture size; texture object is bound when this method is called
	...
	public:
	Texture(const char* sName,Container* sParent,const Size& sSize,const GLfloat sResolution[2],bool sManageChild =true); // Creates a texture widget of the given image size at the given resolution
	...
	const Size& getSize(void) const // Returns the current texture size
		{
		return size;
		}
	...
	void setSize(const Size& newSize); // Sets a new texture image size; resets region to display the entire image, and requests a resize of the widget
