			...
			if(pt==Misc::PATHTYPE_DIRECTORY)
				...
				if(Misc::hasCaseExtension(entryName,".zip"))
					...
					if(fileNameFilters!=0)
						...
						const char* extPtr="";
						size_t extLen=0;
						for(const char* enPtr=entryName;*enPtr!='\0';++enPtr,++extLen)
							if(*enPtr=='.')
								{
								extPtr=enPtr;
								extLen=0;
						...
						lcExt.clear();
						for(const char* ePtr=extPtr;*ePtr!='\0';++ePtr)
							lcExt.push_back(tolower(*ePtr));
						...
						while(*filterPtr!='\0'&&!passesFilters)
							...
							/* See if it matches: */
							passesFilters=extLen==size_t(filterPtr-extStart)&&memcmp(lcExt.data(),extStart,extLen)==0;
