...
void writeTopLevelPosition(const GLMotif::Widget* topLevelWidget,Misc::ConfigurationFileSection& configFileSection)
	...
	configFileSection.storeValue("./widgetExterior",root->getExterior());
	
	/* Write the root's widget transformation: */
	configFileSection.storeValue("./widgetTransform",root->getManager()->calcWidgetTransformation(root));
	}
...
void readTopLevelPosition(GLMotif::Widget* topLevelWidget,const Misc::ConfigurationFileSection& configFileSection,bool useDefaults)
	...
	Box exterior;
	if(useDefaults)
		exterior=configFileSection.retrieveValue("./widgetExterior",root->getExterior());
	else
	...
	if(useDefaults)
		transform=configFileSection.retrieveValue("./widgetTransform",root->getManager()->calcWidgetTransformation(root));
	else
