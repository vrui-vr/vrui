...
Texture::Texture(const char* sName,Container* sParent)
	:Widget(sName,sParent,false),
	 ...
	 illuminated(false)
	{
	/* Initialize the texture resolution: */
	for(int i=0;i<2;++i)
		resolution[i]=72.0f;
...
Texture::Texture(const char* sName,Container* sParent,const Texture::Size& sSize,const GLfloat sResolution[2],bool sManageChild)
	:Widget(sName,sParent,false),
	 size(sSize),
	 version(1),regionVersion(1),
	 interpolationMode(GL_NEAREST),mipmapLevel(0),settingsVersion(1),
...
Vector Texture::calcNaturalSize(void) const
	...
	if(resolution[0]>0.0f&&resolution[1]>0.0f)
		...
		{
		naturalSize[1]=naturalSize[0]=0.0f;
		}
	naturalSize[2]=0.0f;
...
void Texture::setSize(const Texture::Size& newSize)
	{
	...
	if(isManaged&&resolution[0]>0.0f&&resolution[1]>0.0f)
		{
...
void Texture::setRegion(const GLfloat newRegion[4])
	...
	GLfloat rw=region[2]-region[0];
	GLfloat rh=region[3]-region[1];
	if(resolution[0]>0.0f&&resolution[1]>0.0f)
		{
		/* Scale the region size by resolution: */
		rw/=resolution[0];
		rh/=resolution[1];
		}
	else
		{
		/* Scale the region size to fit the widget's interior: */
