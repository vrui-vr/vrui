...
void DropdownBox::setSpacing(GLfloat newSpacing)
	...
	/* Try adjusting the widget size to accommodate the new setting: */
	if(isManaged)
...
void DropdownBox::clearItems(void)
	...
	/* Resize the widget to accommodate the new list: */
	if(isManaged)
		parent->requestResize(this,calcNaturalSize());
	else
		resize(Box(Vector(0.0f,0.0f,0.0f),calcNaturalSize()));
...
Widget* DropdownBox::addItem(const char* newItem)
	...
	/* Resize the widget to accommodate the new item: */
	if(isManaged)
		parent->requestResize(this,calcNaturalSize());
	else
		resize(Box(Vector(0.0f,0.0f,0.0f),calcNaturalSize()));
