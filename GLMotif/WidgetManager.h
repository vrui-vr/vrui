...
class WidgetManager
	...
	struct WidgetShowHideCallbackData:public CallbackData // Class for widget show or hide events
		{
		/* Elements: */
		public:
		bool show; // Flag if the event was a show event
		Widget* topLevelWidget; // The shown or hidden widget
		
		/* Constructors and destructors: */
		WidgetShowHideCallbackData(WidgetManager* sWidgetManager,bool sShow,Widget* sTopLevelWidget)
			:CallbackData(sWidgetManager),show(sShow),topLevelWidget(sTopLevelWidget)
			{
			}
		};
	...
	Misc::CallbackList& getWidgetShowHideCallbacks(void) // Returns the list of widget show/hide callbacks
		{
		return widgetShowHideCallbacks;
		}
