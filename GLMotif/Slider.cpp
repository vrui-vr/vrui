...
void Slider::decrement(void)
	...
	if(valueIncrement!=0.0)
		...
		newValue=Math::max(Math::floor(value/valueIncrement-1.0e-6)*valueIncrement,valueMin);
		}
	else
		newValue=valueMin;
	
	/* Limit the new value to the originally clicked value: */
	newValue=Math::max(newValue,clickValue);
...
void Slider::increment(void)
	...
	if(valueIncrement!=0.0)
		{
		/* Increment and quantize, then range-check: */
		newValue=Math::min(Math::ceil(value/valueIncrement+1.0e-6)*valueIncrement,valueMax);
		}
	else
		newValue=valueMax;
	
	/* Limit the new value to the originally clicked value: */
	newValue=Math::min(newValue,clickValue);
...
void Slider::pointerButtonDown(Event& event)
	...
	if(picked>=sliderBox.origin[dimension]&&picked<=sliderBox.origin[dimension]+sliderBox.size[dimension])
		...
		if(i<notchPositions.size())
			...
			{
			/* Calculate the value at which the slider shaft was clicked: */
			clickValue=double(picked-(shaftBox.origin[dimension]+sliderLength*0.5f))*(valueMax-valueMin)/double(shaftBox.size[dimension]-sliderLength)+valueMin;
...
void Slider::setMarginWidth(GLfloat newMarginWidth)
	...
	if(isManaged)
		{
		/* Try adjusting the widget size to accommodate the new margin width: */
