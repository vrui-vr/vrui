/***********************************************************************
PhongAppearanceShaderColor.fs - Fragment shader for Phong shading with
...
void accumulateDirectionalLight(in int lightIndex,in vec4 vertexEc,in vec3 normalEc,in vec4 color,inout vec4 ambientDiffuseAccum,inout vec4 specularAccum)
	{
	/* Calculate the light direction: */
	vec3 lightDirEc=normalize(gl_LightSource[lightIndex].position.xyz);
	
	/* Calculate the per-light ambient light term: */
	ambientDiffuseAccum+=gl_LightSource[lightIndex].ambient*color;
	...
	if(nl>0.0)
		...
		ambientDiffuseAccum+=gl_LightSource[lightIndex].diffuse*color*nl;
...
void accumulatePointLight(in int lightIndex,in vec4 vertexEc,in vec3 normalEc,in vec4 color,inout vec4 ambientDiffuseAccum,inout vec4 specularAccum)
	{
	...
	/* Calculate the per-light ambient light term: */
	ambientDiffuseAccum+=gl_LightSource[lightIndex].ambient*color*att;
	...
	if(nl>0.0)
		...
		ambientDiffuseAccum+=gl_LightSource[lightIndex].diffuse*color*(nl*att);
...
void accumulateSpotLight(in int lightIndex,in vec4 vertexEc,in vec3 normalEc,in vec4 color,inout vec4 ambientDiffuseAccum,inout vec4 specularAccum)
	{
	/* Calculate the light direction (works for arbitrary homogeneous weights): */
	vec3 lightDirEc=gl_LightSource[lightIndex].position.xyz*vertexEc.w-vertexEc.xyz*gl_LightSource[lightIndex].position.w;
	float lightDist=length(lightDirEc)/(gl_LightSource[lightIndex].position.w*vertexEc.w);
	...
	for(int lightIndex=0;lightIndex<gl_MaxLights;++lightIndex)
		if(lightEnableds[lightIndex])
			...
			if(gl_LightSource[lightIndex].position.w==0.0)
				accumulateDirectionalLight(lightIndex,position,fragNormal,color,ambientDiffuseColor,specularColor);
			else if(gl_LightSource[lightIndex].spotCosCutoff<-0.001)
				accumulatePointLight(lightIndex,position,fragNormal,color,ambientDiffuseColor,specularColor);
			else
				accumulateSpotLight(lightIndex,position,fragNormal,color,ambientDiffuseColor,specularColor);
			}
	
	/* Assign the final fragment color: */
	gl_FragColor=ambientDiffuseColor+specularColor;
	}
