...
WAVFile::WAVFile(IO::FilePtr sFile)
	...
	if(file->getReadBufferSize()==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not opened for reading");
	...
	if(memcmp(riffTag,"RIFF",4)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not a RIFF file");
	file->skip<Misc::UInt32>(1); // Skip RIFF chunk size
	char waveTag[4];
	file->read(waveTag,4);
	if(memcmp(waveTag,"WAVE",4)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not a WAVE file");
	...
	if(memcmp(fmtTag,"fmt ",4)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File does not have a fmt chunk");
	size_t fmtChunkSize=file->read<Misc::UInt32>();
	if(fmtChunkSize<fmtHeaderSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File has truncated fmt chunk");
	if(file->read<Misc::UInt16>()!=1U) // Can only do linear PCM samples for now
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File does not contain linear PCM samples");
	...
	if(format.bitsPerSample<8||format.bitsPerSample>32||(format.bitsPerSample&0x7)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File has unsupported number of bits per sample");
	...
	if(format.samplesPerFrame<1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File has unsupported number of samples per frame");
	if(bytesPerFrame!=size_t(format.samplesPerFrame)*size_t(format.bytesPerSample)
	   ||bytesPerSecond!=size_t(format.framesPerSecond)*size_t(format.samplesPerFrame)*size_t(format.bytesPerSample))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File has inconsistent fmt chunk");
	...
	if(dataChunkSize==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File does not contain a data chunk");
...
WAVFile::WAVFile(IO::FilePtr sFile,const SoundDataFormat& sFormat)
	...
	if(file->getWriteBufferSize()==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not opened for writing");
	...
	if(file->getWriteBufferSize()!=0)
		...
		if(numAudioFrames!=numPresetAudioFrames)
			...
			if(seekableFile!=0)
				...
				Misc::sourcedUserWarning(__PRETTY_FUNCTION__,"Invalid WAV file was written; number of audio frames does not match WAV header");
				}
			}
		}
	}
