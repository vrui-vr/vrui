...
class ALSAPCMDevice
	...
	/* Embedded classes: */
	public:
	struct PCM // Structure identifying a capture or playback PCM device on the system
		{
		/* Elements: */
		public:
		int cardIndex,deviceIndex; // Indices of the audio card containing the PCM, and the PCM to construct audio device names such as "hw:<cardIndex>,<deviceIndex>"
		std::string name; // Descriptive name of the PCM device
		};
	
	typedef std::vector<PCM> PCMList; // Type for lists of PCM device descriptors
	...
	snd_pcm_format_t pcmSampleFormat;
	...
	/* Constructors and destructors: */
	public:
	static PCMList enumeratePCMs(bool recording); // Returns a list of capture or playback PCM devices on the system
	...
	size_t getAvailableFramesCached(void) // Ditto, but does not round-trip to hardware. Can be used after wake-up from poll or select
		...
		if(result<0)
			throwException(__PRETTY_FUNCTION__,int(result));
		return size_t(result);
		}
