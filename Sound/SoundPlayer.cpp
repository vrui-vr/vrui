...
SoundPlayer::SoundPlayer(const char* inputFileName)
	...
	if(AudioFileOpenURL(inputFileURL,0x1,0,&inputFile)!=noErr)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot open input file %s",inputFileName);
		}
	CFRelease(inputFileURL);
	...
	if(AudioFileGetProperty(inputFile,kAudioFilePropertyDataFormat,&formatSize,&format)!=noErr)
		...
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query sound data format of input file %s",inputFileName);
		}
	...
	if(AudioFileGetProperty(inputFile,kAudioFilePropertyPacketSizeUpperBound,&maxPacketSizeSize,&maxPacketSize)!=noErr)
		{
		AudioFileClose(inputFile);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot query sound data packet size of input file %s",inputFileName);
		}
	...
	if(AudioQueueNewOutput(&format,handleOutputBufferWrapper,this,0,kCFRunLoopCommonModes,0,&queue)!=noErr)
		{
		AudioFileClose(inputFile);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot create playback audio queue");
		}
	...
	if(AudioFileGetPropertyInfo(inputFile,kAudioFilePropertyMagicCookieData,&magicCookieSize,0)==noErr&&magicCookieSize>0)
		...
		if(AudioFileGetPropertyInfo(inputFile,kAudioFilePropertyMagicCookieData,&magicCookieSize,reinterpret_cast<UInt32*>(magicCookie))!=noErr)
			...
			AudioFileClose(inputFile);
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot set the audio queue's magic cookie");
			}
	...
	for(int i=0;i<2;++i)
		...
		if(AudioQueueAllocateBuffer(queue,bufferSize,&buffers[i])!=noErr)
			...
			AudioQueueDispose(queue,true);
			AudioFileClose(inputFile);
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot allocate audio buffer %d",i);
			}
		}
...
void SoundPlayer::start(void)
	...
	if(AudioQueueStart(queue,0)!=noErr)
		{
		active=false;
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot start audio queue");
		}
	}
...
SoundPlayer::SoundPlayer(const char* inputFileName)
	...
	if(Misc::hasCaseExtension(inputFileName,".wav"))
		...
		if(!readWAVHeader())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s is invalid or incompatible WAV file",inputFileName);
		}
	else
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s has unrecognized extension",inputFileName);
