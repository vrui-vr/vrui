...
std::vector<BlobParam> extractBlobs(const Size& size,const PixelParam* image,const ForegroundSelectorParam& foregroundSelector,const typename BlobParam::Creator& blobCreator,unsigned int* blobIdImage =0); // Extracts blobs from the given image; if blobIdImage is !=0, creates per-pixel blob ID array

template <class BlobParam,class PixelParam,class ForegroundSelectorParam,class MergeCheckerParam>
std::vector<BlobParam> extractBlobs(const Size& size,const PixelParam* image,const ForegroundSelectorParam& foregroundSelector,const MergeCheckerParam& mergeChecker,const typename BlobParam::Creator& blobCreator,unsigned int* blobIdImage =0); // Ditto, with merge checker
