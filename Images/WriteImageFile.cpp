...
class PNGWriter
	...
	static void errorFunction(png_structp pngReadStruct,png_const_charp errorMsg) // Called by the PNG library to report an error
		{
		/* Throw an exception: */
		throw Misc::makeStdErr("Images::writePngFile","%s",errorMsg);
		}
	static void warningFunction(png_structp pngReadStruct,png_const_charp warningMsg) // Called by the PNG library to report a recoverable error
		...
		Misc::sourcedUserWarning("Images::writePngFile","%s",warningMsg);
		}
	...
	PNGWriter(IO::File& sSink)
		...
		if((pngWriteStruct=png_create_write_struct(PNG_LIBPNG_VER_STRING,0,errorFunction,warningFunction))==0
		  ...
		  /* Clean up and throw an exception: */
			...
			throw Misc::makeStdErr("Images::writePngFile","Internal error in PNG library");
			}
...
class JPEGExceptionErrorManager:public jpeg_error_mgr
	...
	static void errorExitFunction(j_common_ptr cinfo)
		...
		/* Throw an exception: */
		throw Misc::makeStdErr("Images::writeJpegFile",thisPtr->jpeg_message_table[thisPtr->msg_code],thisPtr->msg_parm.i[0],thisPtr->msg_parm.i[1],thisPtr->msg_parm.i[2],thisPtr->msg_parm.i[3],thisPtr->msg_parm.i[4],thisPtr->msg_parm.i[5],thisPtr->msg_parm.i[6],thisPtr->msg_parm.i[7]);
		}
...
class JPEGWriter:public jpeg_compress_struct
	...
	JPEGWriter(IO::File& destination)
		...
		throw Misc::makeStdErr("Images::writeJpegFile","Unsupported bit depth in JPEG library");
		#endif
...
void tiffErrorFunction(const char* module,const char* fmt,va_list ap)
	...
	throw Misc::makeStdErr("Images::writeTiffFile",fmt,ap);
	}

void tiffWarningFunction(const char* module,const char* fmt,va_list ap)
	{
	/* Log a warning message: */
	Misc::sourcedConsoleWarning("Images::writeTiffFile",fmt,ap);
	}
...
void writeTiffFile(unsigned int width,unsigned int height,const unsigned char* image,const char* imageFileName)
	...
	try
		...
		throw Misc::makeStdErr("Images::writeTiffFile","Caught exception \"%s\" while writing image \"%s\"",err.what(),imageFileName);
		}
	}
...
void writeImageFile(const RGBImage& image,IO::File& file,ImageFileFormat imageFileFormat)
	...
	switch(imageFileFormat)
		...
		#if IMAGES_CONFIG_HAVE_TIFF
		case IFF_TIFF:
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot write TIFF images to already-open files");
		#endif
...
void writeImageFile(const RGBImage& image,const char* imageFileName)
	...
	try
		...
		{
		/* Wrap and re-throw the exception: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot write image file %s due to error %s",imageFileName,err.what());
		}
	}
...
void writeImageFile(const RGBImage& image,const IO::Directory& directory,const char* imageFileName)
	...
	try
		...
		{
		/* Wrap and re-throw the exception: */
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot write image file %s due to error %s",directory.getPath(imageFileName).c_str(),err.what());
		}
	}
