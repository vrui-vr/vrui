...
class Image:public BaseImage
	...
	Image(const Size& sSize,GLenum sFormat) // Creates an uninitialized image of the given size and format
		:BaseImage(sSize,numComponents,sizeof(Scalar),sFormat,GLScalarLimits<Scalar>::type)
		{
		}
	...
	static Image glReadPixels(const Offset& offset,const Size& size,GLenum format) // Returns a new image created by reading from the frame buffer
		{
		/* Create a new image of the requested size: */
		Image result(size,format);
	...
	*****************************************************************************/
	...
	Image& resize(const Size& newSize); // Resamples the image to the given size
	};
