...
class BaseImage
	...
	struct ImageRepresentation // Structure to represent an image to allow non-copy sharing and passing of images
		...
		ImageRepresentation(const Size& sSize,unsigned int sNumChannels,unsigned int sChannelSize,GLenum sFormat,GLenum sScalarType); // Creates a new image representation with uninitialized data
	...
	/* Elements: */
	private:
	static bool useGammaCorrection; // Flag whether images should assume gamma-corrected input images by default
	...
	/* Constructors and destructors: */
	public:
	static bool getUseGammaCorrection(void) // Returns the current value of the gamma correction flag
		{
		return useGammaCorrection;
		}
	static void setUseGammaCorrection(bool newUseGammaCorrection); // Sets the static gamma correction flag
	...
	public:
	BaseImage(const Size& sSize,unsigned int sNumChannels,unsigned int sChannelSize,GLenum sFormat,GLenum sScalarType) // Creates an uninitialized image of the given size and format
		:rep(new ImageRepresentation(sSize,sNumChannels,sChannelSize,sFormat,sScalarType))
		{
		}
	...
	*****************************************************************************/
	...
	const Size& getSize(void) const // Returns image size
		{
		return rep->size;
		}
	...
	BaseImage toUInt8(void) const; // Returns a new image with 8-bit unsigned integer samples; returns itself if the image is already in that format
	...
	GLint getInternalFormat(void) const; // Returns an internal OpenGL texture format compatible with this image
	BaseImage& glReadPixels(const Offset& offset); // Reads the frame buffer contents starting from the given offset into the image
	...
	void glTexSubImage2D(GLenum target,GLint level,const Offset& offset) const; // Uploads an image as a part of a larger OpenGL texture
	void glTexSubImage3D(GLenum target,GLint level,const Offset& offset,GLint zOffset) const; // Uploads an image as a (part of) single slice of an OpenGL 3D texture
	};
