...
class TargaImageFileReader
	...
	Rect rect; // Position and size of image
	...
	/* Methods: */
	const Size& getSize(void) const // Returns the size of the image stored in the Targa image file
		{
		return rect.size;
		}
	unsigned int getSize(int dimension) const // Returns the width or height of the image stored in the Targa image file
		{
		return rect.size[dimension];
		}
