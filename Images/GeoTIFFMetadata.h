...
bool readGeoTIFFMetadata(TIFF* tiff,GeoTIFFMetadata& metadata); // Function to read GeoTIFF metadata from the current directory of an open TIFF file; returns true if reading succeeded
bool writeGeoTIFFMetadata(TIFF* tiff,const GeoTIFFMetadata& metadata); // Function to write GeoTIFF metadata to the current directory of an open TIFF file; returns true if writing succeeded
