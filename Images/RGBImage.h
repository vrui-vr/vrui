...
color depth.
Copyright (c) 2005-2022 Oliver Kreylos
...
class RGBImage:public Image<GLubyte,3>
	...
	RGBImage(const Size& sSize) // Creates an uninitialized image of the given size
		:Base(sSize,GL_RGB)
		{
		}
	...
	static RGBImage glReadPixels(const Offset& offset,const Size& size) // Returns a new image created by reading from the frame buffer
		{
		/* Create a new image of the requested size: */
		RGBImage result(size);
