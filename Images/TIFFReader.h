...
class TIFFReader // Class to read the first image from a TIFF image file
	...
	typedef void (*PixelStreamingCallback)(uint32_t x,uint32_t y,uint32_t width,uint16_t channel,const uint8_t* pixels,void* userData); // Type of callback function receiving a row of samples for the given channel in streaming mode; if channel==uint16(-1), pixel channels are interleaved
	
	/* Elements: */
	private:
	TIFF* tiff; // The TIFF library object
	IO::SeekableFilePtr seekableSource; // A seekable version of the source file
	uint32_t width,height; // Image width and height in pixels
	uint16_t numBits,numSamples,sampleFormat; // Number of bits per sample, samples per pixel, and sample format
	...
	uint32_t rowsPerStrip; // Number of rows per image strip if image data is organized in strips
	uint32_t tileWidth,tileHeight; // Width and height of an image tile if image data is organized in tiles
	...
	/* Methods: */
	uint32_t getWidth(void) const // Returns the image's width in pixels
		{
		return width;
		}
	uint32_t getHeight(void) const // Returns the image's height in pixels
		{
		return height;
		}
	uint16_t getNumBits(void) const // Returns the number of bits per image sample
		{
		return numBits;
		}
	uint16_t getNumSamples(void) const // Returns the number of samples per image pixel
		{
		return numSamples;
		}
	uint16_t getSampleFormat(void) const // Returns the sample format as a libtiff enumerant
		{
		return sampleFormat;
		}
	...
	uint32_t getRowsPerStrip(void) const // Returns the number of image rows per image strip
		{
		return rowsPerStrip;
		}
	uint32_t getTileWidth(void) const // Returns the width of an image tile
		{
		return tileWidth;
		}
	uint32_t getTileHeight(void) const // Returns the height of an image tile
		{
		return tileHeight;
		}
	bool getColorMap(uint16_t*& red,uint16_t*& green,uint16_t*& blue); // Puts pointers to the TIFF file's color map into the provided pointers; returns true if TIFF file does contain a color map; nulls pointers if no color map present
	bool getCMYKColorMap(uint16_t*& cyan,uint16_t*& magenta,uint16_t*& yellow,uint16_t*& black); // Puts pointers to the TIFF file's color map into the provided pointers if the file's color space is CMYK; returns true if TIFF file does contain a color map; nulls pointers if no color map present
	...
	void readRgba(uint32_t* rgbaBuffer); // Reads a complete TIFF image into a 32-bit RGBA buffer
	void readStrips(uint8_t* image,ptrdiff_t rowStride); // Reads a stripped TIFF image into a channel-interleaved image of appropriate size
	uint8_t* createTileBuffer(void); // Returns a new-allocated buffer for temporary storage when reading a single TIFF image tile
	void readTile(uint32_t tileIndexX,uint32_t tileIndexY,uint8_t* tileBuffer,uint8_t* image,ptrdiff_t rowStride); // Reads a single TIFF image tile into a channel-interleaved image of appropriate size; tile buffer must have been created using createTileBuffer method
	void readTiles(uint8_t* image,ptrdiff_t rowStride); // Reads a tiled TIFF image into a channel-interleaved image of appropriate size
	void readImage(uint8_t* image,ptrdiff_t rowStride) // Reads a stripped or tiled TIFF image into a channel-interleaved image of appropriate size
		{
