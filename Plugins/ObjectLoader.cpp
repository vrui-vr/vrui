...
/*******************************************
Methods of class ObjectLoaderBase::DsoError:
*******************************************/

ObjectLoaderBase::DsoError::DsoError(const char* source)
...
void ObjectLoaderBase::closeDso(void* handle)
	{
	/* Close the DSO of the given handle if it is not null: */
	if(handle!=0)
		dlclose(handle);
	}
	...
	for(const char* tPtr=templateStart;*tPtr!='\0';++tPtr)
		...
		if(*tPtr=='/'&&cStart==0&&vStart==0) // Find the last slash before the %s and potential %u conversion
			...
			if(tPtr[1]=='s'&&cStart==0)
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid DSO name template %s",sDsoNameTemplate);
			}
		}
	if(cStart==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid DSO name template %s",sDsoNameTemplate);
