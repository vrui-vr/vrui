						...
						if(numBytesReceived==sizeof(StreamMessage))
							...
							if(pipeState.isValid())
								...
								if(msg->streamPos!=pipeState->streamPos)
									...
									if(packet==0)
										throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Fatal packet loss detected at stream position %u",msgNodeIndex,msg->streamPos);
...
void* Multiplexer::packetHandlingThreadSlave(void)
	...
	while(true)
		...
		if(!havePacket)
			{
			/* Signal an error: */
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Communication error",nodeIndex);
			}
...
Multiplexer::Multiplexer(unsigned int sNumSlaves,unsigned int sNodeIndex,std::string masterHostName,int masterPortNumber,std::string slaveMulticastGroup,int slavePortNumber)
	...
	if(masterEntry==0)
		{
		close(socketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Unable to resolve master %s",nodeIndex,masterHostName.c_str());
		}
	...
	if(slaveEntry==0)
		{
		close(socketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Unable to resolve slave multicast group %s",nodeIndex,slaveMulticastGroup.c_str());
		}
	...
	if(socketFd<0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Unable to create socket",nodeIndex);
	...
	if(bind(socketFd,(struct sockaddr*)&socketAddress,sizeof(struct sockaddr_in))==-1)
		{
		close(socketFd);
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Unable to bind socket to port number %d",nodeIndex,localPortNumber);
		}
	...
	if(nodeIndex==0)
		...
		if(isMulticast(slaveNetAddress))
			...
			if(setsockopt(socketFd,IPPROTO_IP,IP_MULTICAST_IF,&multicastInterfaceAddress,sizeof(struct in_addr))<0)
				...
				int myerrno=errno;
				close(socketFd);
				throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Node %u: Unable to enable multicast",nodeIndex);
				}
			}
		...
		if(isMulticast(slaveNetAddress))
			...
			if(setsockopt(socketFd,IPPROTO_IP,IP_ADD_MEMBERSHIP,&addGroupRequest,sizeof(struct ip_mreq))<0)
				...
				int myerrno=errno;
				close(socketFd);
				throw Misc::makeLibcErr(__PRETTY_FUNCTION__,myerrno,"Node %u: Unable to joint multicast group",nodeIndex);
				}
			}
...
unsigned int Multiplexer::openPipe(void)
	...
	if(sizeof(CreatePipe1Message)+threadId.getNumParts()*sizeof(unsigned int)>Packet::maxRawPacketSize)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Threads nested too deply");
	...
	if(nodeIndex==0)
		...
		{
		#if CLUSTER_CONFIG_DEBUG_MULTIPLEXER
		std::cerr<<"Node "<<nodeIndex<<": Opening pipe for thread ";
		if(threadId.getNumParts()==0)
			std::cerr<<"root";
		else
			{
		...
		#if CLUSTER_CONFIG_DEBUG_MULTIPLEXER
		std::cerr<<", pipe ID "<<newPipeState->pipeId<<"... done"<<std::endl;
		#endif
		}
...
void Multiplexer::closePipe(unsigned int pipeId)
	...
	if(psIt.isFinished())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Node %u: Pipe already closed",nodeIndex);
