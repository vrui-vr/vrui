	...
	if(multiplexer!=0)
		...
		if(Comm::HttpFile::checkHttpPrefix(fileName))
			...
			if(accessMode==IO::File::WriteOnly||accessMode==IO::File::ReadWrite)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Write access to HTTP files not supported");
			
			/* Split the URL into its components: */
			Comm::HttpFile::URLParts urlParts=Comm::HttpFile::splitUrl(fileName);
			
			if(urlParts.https)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"HTTPS connections not supported on clusters");
...
Opener* Opener::getOpener(void)
	{
	/* Get the active opener and cast it to a Cluster::Opener: */
	Opener* result=dynamic_cast<Opener*>(IO::Opener::getOpener());
	if(result==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Active IO::Opener is not a Cluster::Opener");
