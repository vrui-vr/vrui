...
static const char readDataFuncName[]="Cluster::StandardFile::readData";
static const char readErrorString[]="Cannot read from file";
static const char writeDataFuncName[]="Cluster::StandardFile::writeData";
static const char writeDataUpToFuncName[]="Cluster::StandardFile::writeDataUpTo";
static const char writeErrorString[]="Cannot write to file";
static const char constructorFuncName[]="Cluster::StandardFile::StandardFile";
static const char openErrorString[]="Cannot open file %s for %s";
static const char getFdFuncName[]="Cluster::StandardFile::getFd";
static const char getFdErrorString[]="Cannot retrieve descriptor of cluster file";
static const char getSizeFuncName[]="Cluster::StandardFile::getSize";
static const char getSizeErrorString[]="Cannot retrieve file size";
}
...
size_t StandardFileMaster::readData(IO::File::Byte* buffer,size_t bufferSize)
	...
	if(errorType==0)
		...
		/* Throw an exception: */
		if(errorType==1)
			throw SeekError(readDataFuncName,readPos);
		else if(errorType==3)
...
void StandardFileMaster::writeData(const IO::File::Byte* buffer,size_t bufferSize)
	...
	/* Handle errors: */
	if(errorType==1)
		throw SeekError(writeDataFuncName,writePos);
	else if(errorType==2)
		throw WriteError(writeDataFuncName,errorCode);
	else if(errorType==3)
		throw Error(Misc::makeLibcErrMsg(writeDataFuncName,errorCode,writeErrorString));
	}
...
size_t StandardFileMaster::writeDataUpTo(const IO::File::Byte* buffer,size_t bufferSize)
	...
	/* Handle errors: */
	if(errorType==1)
		throw SeekError(writeDataUpToFuncName,writePos);
	else if(errorType==2)
		throw WriteError(writeDataUpToFuncName,errorCode);
	else if(errorType==3)
		throw Error(Misc::makeLibcErrMsg(writeDataUpToFuncName,errorCode,writeErrorString));
...
void StandardFileMaster::openFile(const char* fileName,IO::File::AccessMode accessMode,int flags,int mode)
	...
	/* Check for errors: */
	if(errorCode!=0)
		throw OpenError(Misc::makeLibcErrMsg(constructorFuncName,errorCode,openErrorString,fileName,getAccessModeName(accessMode)));
...
IO::SeekableFile::Offset StandardFileMaster::getSize(void) const
	...
	/* Check for errors: */
	if(statResult<0)
		throw Error(Misc::makeLibcErrMsg(getSizeFuncName,errorCode,getSizeErrorString));
...
IO::SeekableFile::Offset StandardFileSlave::getSize(void) const
	...
	if(isReadCoupled())
		...
		/* Check for errors: */
		if(statResult<0)
			throw Error(Misc::makeLibcErrMsg(getSizeFuncName,int(fileSize),getSizeErrorString));
